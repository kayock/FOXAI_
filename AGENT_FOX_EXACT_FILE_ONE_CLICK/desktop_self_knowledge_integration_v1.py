...
EXACT_FILE_SUFFIXES = {
    ".py", ".md", ".txt", ".json", ".ini", ".bat", ".ps1",
    ".yaml", ".yml", ".log",
}
EXACT_FILE_BLOCKED_NAMES = {
    ".env", "id_rsa", "id_dsa", "credentials.json", "secrets.json",
    "tokens.json", "passwords.txt",
}
EXACT_FILE_BLOCKED_PARTS = {
    ".git", "__pycache__", "credentials", "secrets", "tokens",
}


def _exact_file_result(
    *,
    intercepted: bool,
    status: str,
    ...
    return {
        ...
        "model_bypass": intercepted,
        "ordinary_chat_pass_through": not intercepted,
    }


def _extract_exact_file_path(message: str) -> str | None:
    """Find one literal file path in a natural read/explain question."""
    text = str(message or "").strip()
    if not text or text.lstrip().startswith("/"):
        return None

    asks_about_file = re.search(
        r"(?:\bwhat\s+does\b|\bwhat\s+is\b|\bexplain\b|\bread\b|"
        r"\bopen\b|\breview\b|\bsummarize\b|\bdescribe\b)",
        text,
        flags=re.IGNORECASE,
    )
    if not asks_about_file:
        return None

    quoted = re.search(
        r"""["'](?P<path>[^"']+[\\/][^"']+\.[A-Za-z0-9]{1,8})["']""",
        text,
    )
    if quoted:
...
def _python_file_summary(path: Path, text: str) -> str:
    ...
    try:
        ...
        return (
            ...
            f"It could not be fully parsed because of {error.msg} "
    ...
    if module_doc:
        ...
        parts.append(f"Its module description says: {first_line}")

    if imports:
        shown = ", ".join(imports[:10])
        extra = len(imports) - min(len(imports), 10)
        parts.append(
            f"It imports {shown}"
    ...
    if classes:
        parts.append(
            "Its top-level classes are " + ", ".join(classes[:12]) + "."
        )

    if functions:
        parts.append(
            "Its top-level functions are " + ", ".join(functions[:16]) + "."
        )

    function_set = set(functions)
    if (
        path.name.casefold() == "director.py"
        and {"classify", "direct"}.issubset(function_set)
    ):
        parts.append(
            "In FOXAI, this file is the department selector. Normal messages "
            "remain in Agent Fox chat. An explicit /image command selects Red "
            "Canvas, and an explicit /engineer command selects Engineer. The "
            "direct() function returns the selected destination and payload."
        )
    elif classes or functions:
        parts.append(
            "Those names describe the file's main structure without searching "
            "the rest of FOXAI."
        )
    else:
        parts.append(
            "It contains executable Python statements but no top-level class "
...
def _structured_text_summary(path: Path, text: str) -> str:
    lines = text.splitlines()
    ...
    if suffix == ".json":
        try:
            ...
            return (
                f"This is a JSON file with {len(lines)} lines, but it could not "
                "be parsed as valid JSON."
            )
        if isinstance(parsed, dict):
            keys = [str(key) for key in list(parsed)[:20]]
            return (
                f"This is a JSON object with {len(parsed)} top-level keys. "
                f"The first keys are: {', '.join(keys) if keys else '[none]'}."
            )
        if isinstance(parsed, list):
            return f"This is a JSON list containing {len(parsed)} item(s)."
        return f"This is a JSON value of type {type(parsed).__name__}."
    ...
    if suffix == ".md":
        ...
        result = f"This is a Markdown document with {len(lines)} lines."
        if headings:
            result += " Its headings include: " + "; ".join(headings) + "."
        return result
    ...
    return (
        f"This is a {kind} with {len(lines)} total lines and "
        f"{nonempty} non-empty lines."
    )


def _route_exact_project_file_question(
    message: str,
    *,
    ...
    if value.startswith("\\\\") or value.startswith("//"):
        return _exact_file_result(
            intercepted=True,
            status="evidence_error",
            answer_text=(
                "That is a network path. Agent Fox only opened local FOXAI "
                "project files for this request."
            ),
            diagnostic="exact_file_network_path_denied",
        )
    ...
    try:
        ...
        return _exact_file_result(
            ...
            answer_text=(
                f"I found the file path in your question, but this file does "
                f"not exist: {value}"
            ),
            diagnostic="exact_file_not_found",
        )
        ...
        return _exact_file_result(
            ...
            status="evidence_error",
            answer_text=(
                "I recognized the file request, but the path could not be "
                "resolved safely."
            ),
            diagnostic=f"exact_file_resolution_error:{type(error).__name__}",
        )
    ...
    try:
        ...
        return _exact_file_result(
            ...
            status="evidence_error",
            answer_text=(
                "That file is outside the active FOXAI project, so Agent Fox "
                "did not open it."
            ),
            diagnostic="exact_file_outside_project",
        )

    folded_parts = {part.casefold() for part in resolved.parts}
    if (
        resolved.name.casefold() in EXACT_FILE_BLOCKED_NAMES
        or folded_parts.intersection(EXACT_FILE_BLOCKED_PARTS)
    ):
        return _exact_file_result(
            intercepted=True,
            status="evidence_error",
            answer_text=(
                "That path appears to contain credentials or other protected "
                "material, so Agent Fox did not open it."
            ),
            diagnostic="exact_file_protected_name",
        )
    ...
    if not resolved.is_file():
        return _exact_file_result(
            intercepted=True,
            status="clarification_required",
            answer_text=f"The named path is not a regular file: {resolved}",
            diagnostic="exact_file_not_regular",
        )
    ...
    if suffix not in EXACT_FILE_SUFFIXES:
        return _exact_file_result(
            ...
            status="clarification_required",
            answer_text=(
                f"I found {resolved}, but {suffix or '[no extension]'} is not "
                "an approved text format for this read-only explanation."
            ),
            diagnostic="exact_file_unsupported_type",
        )
    ...
    try:
        ...
        return _exact_file_result(
            intercepted=True,
            status="evidence_error",
            answer_text="The named file could not be measured safely.",
            diagnostic=f"exact_file_stat_error:{type(error).__name__}",
        )
    ...
    if size > EXACT_FILE_MAX_BYTES:
        return _exact_file_result(
            ...
            status="clarification_required",
            answer_text=(
                f"The file is too large for this bounded explanation "
                f"({size} bytes; limit {EXACT_FILE_MAX_BYTES})."
            ),
            diagnostic="exact_file_too_large",
        )
    ...
    try:
        ...
        return _exact_file_result(
            intercepted=True,
            status="evidence_error",
            answer_text="The named file could not be read.",
            diagnostic=f"exact_file_read_error:{type(error).__name__}",
        )
    ...
    if b"\x00" in data:
        return _exact_file_result(
            ...
            status="clarification_required",
            answer_text=(
                "The named file appears to contain binary data, so Agent Fox "
                "did not treat it as readable text."
            ),
            diagnostic="exact_file_binary",
        )

    text = data.decode("utf-8", errors="replace")
    if suffix == ".py":
        summary = _python_file_summary(resolved, text)
    else:
        summary = _structured_text_summary(resolved, text)

    relative = resolved.relative_to(resolved_root)
    answer = (
        f"{relative}\n\n"
        f"{summary}\n\n"
        "Read-only: Agent Fox opened exactly this one FOXAI file and changed "
        "nothing."
    )
    return _exact_file_result(
        intercepted=True,
        status="answered",
        answer_text=answer,
        diagnostic="exact_project_file_answered",
    )
...
def route_desktop_message(
    ...
    *,
    ...
    resource_provider_path: Path | None = None,
    resource_source_dir: Path | None = None,
) -> dict[str, Any]:
    ...
    exact_file_result = _route_exact_project_file_question(normalized)
    if exact_file_result is not None:
    ...
    try:
        ...
        if any(
            value is not None
            for value in (bridge_path, registry_dir, resource_provider_path, resource_source_dir)
        ):
