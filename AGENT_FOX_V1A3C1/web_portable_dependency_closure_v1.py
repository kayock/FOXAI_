...
SCHEMA_PREFIX = "foxai.agent_fox.technical_core.v1a3c1"
...
def parse_reached_sources_once(
    ...
    for source_path in closure["reached_paths"]:
        ...
        expected_digest = str(file_row.get("sha256"))
        expected_size = int(file_row.get("size_bytes") or -1)
        if digest != expected_digest or len(data) != expected_size:
            raise ValueError(f"Reached source drifted from verified immutable index: {source_path}")
...
def validate_output(index_dir: Path) -> None:
    ...
    for row in receipt.get("core_outputs_before_receipt", []):
        ...
        if not path.is_file() or path.stat().st_size != int(row.get("size_bytes") or -1):
...
"""
    ...
    print("V1A3C1_SELF_TEST_OK")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Agent Fox V1A-3C1 Web Portable static dependency closure")
