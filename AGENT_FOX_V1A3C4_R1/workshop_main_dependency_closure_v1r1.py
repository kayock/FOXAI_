...
SCHEMA_PREFIX = "foxai.agent_fox.technical_core.v1a3c4_r1"
...
TARGET_LAUNCHER = r"Z:\FOXAI\Launch FOXAI Workshop.bat"
TARGET_SCRIPT = r"Z:\FOXAI\foxai.py"
TARGET_INTERPRETER_KIND = "command_alias_requires_path_resolution"
EXPECTED_REACHED_NODE_COUNT = 70
EXPECTED_DEPENDENCY_EDGE_COUNT = 205
EXPECTED_PRELIMINARY_UNRESOLVED_COUNT = 15
EXPECTED_ZERO_BYTE_SOURCE = r"Z:\FOXAI\core\__init__.py"
EMPTY_FILE_SHA256 = "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"

OUTPUT_NAMES = (
    "WORKSHOP_MAIN_CONTEXT.json",
    "WORKSHOP_MAIN_FIRST_PARTY_CLOSURE.json",
    "WORKSHOP_MAIN_CONDITIONAL_IMPORTS.json",
    "WORKSHOP_MAIN_PACKAGE_REQUIREMENTS.json",
    "WORKSHOP_MAIN_UNRESOLVED_BRANCHES.json",
    "WORKSHOP_MAIN_RUNTIME_UNCERTAINTY.json",
...
def select_workshop_main_context(source: dict[str, Any]) -> dict[str, Any]:
    matches = [
    ...
    if len(matches) != 1:
        raise ValueError(f"Expected exactly one Workshop Main context, found {len(matches)}")
    context = dict(matches[0])
    context["role"] = "workshop_main_webui"
...
def enrich_closure(closure: dict[str, Any], parsed: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
    ...
    for edge in closure["edges"]:
        ...
        item = {
            **edge,
            ...
            "context_role": "workshop_main_webui",
        }
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.workshop_main_first_party_closure.v1",
        "context_id": context.get("context_id"),
        "context_role": "workshop_main_webui",
...
def build_conditional_report(parsed: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.workshop_main_conditional_imports.v1",
        "context_count": 1,
...
def build_package_requirements(
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.workshop_main_package_requirements.v1",
        "context_id": context.get("context_id"),
        "context_count": 1,
        **groups,
...
def build_unresolved_report(
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.workshop_main_unresolved_branches.v1",
        "record_count": len(records),
        "records": records,
...
def build_core_outputs(
    ...
    context_output = {
        "schema": f"{SCHEMA_PREFIX}.workshop_main_context.v1",
    ...
    coverage = {
        "schema": f"{SCHEMA_PREFIX}.workshop_main_coverage.v1",
        "mission_id": mission_id,
        "result": "workshop_main_dependency_closure_complete",
        ...
        "limitations": [
            ...
            "Only the Workshop Main protected context is covered by this mission.",
        ],
    }
    return {
        "WORKSHOP_MAIN_CONTEXT.json": context_output,
        "WORKSHOP_MAIN_FIRST_PARTY_CLOSURE.json": enriched,
        "WORKSHOP_MAIN_CONDITIONAL_IMPORTS.json": conditional,
        "WORKSHOP_MAIN_PACKAGE_REQUIREMENTS.json": requirements,
        "WORKSHOP_MAIN_UNRESOLVED_BRANCHES.json": unresolved,
        "WORKSHOP_MAIN_COVERAGE.json": coverage,
    }
...
def write_outputs(output_dir: Path, outputs: dict[str, Any], mission_id: str) -> None:
    ...
    coverage = outputs["WORKSHOP_MAIN_COVERAGE.json"]
    receipt = {
        "schema": f"{SCHEMA_PREFIX}.workshop_main_closure_receipt.v1",
        "mission_id": mission_id,
        "result": "workshop_main_dependency_closure_complete",
        "source_outputs_verified_exact": True,
    ...
    (output_dir / "WORKSHOP_MAIN_CLOSURE_RECEIPT.json").write_bytes(receipt_bytes)
...
def build(
    ...
    context = select_workshop_main_context(source)
...
def validate_output(index_dir: Path) -> None:
    required = set(OUTPUT_NAMES) | {"WORKSHOP_MAIN_CLOSURE_RECEIPT.json"}
    ...
    receipt = read_json(index_dir / "WORKSHOP_MAIN_CLOSURE_RECEIPT.json")
    coverage = read_json(index_dir / "WORKSHOP_MAIN_COVERAGE.json")
    closure = read_json(index_dir / "WORKSHOP_MAIN_FIRST_PARTY_CLOSURE.json")
    context = read_json(index_dir / "WORKSHOP_MAIN_CONTEXT.json")
    conditional = read_json(index_dir / "WORKSHOP_MAIN_CONDITIONAL_IMPORTS.json")
    unresolved = read_json(index_dir / "WORKSHOP_MAIN_UNRESOLVED_BRANCHES.json")
    if receipt.get("result") != "workshop_main_dependency_closure_complete":
...
def show_context_requirements(index_dir: Path) -> None:
    context = read_json(index_dir / "WORKSHOP_MAIN_CONTEXT.json")
    requirements = read_json(index_dir / "WORKSHOP_MAIN_PACKAGE_REQUIREMENTS.json")
...
"""
    ...
    assert "WORKSHOP_MAIN_RUNTIME_UNCERTAINTY.json" in OUTPUT_NAMES
    ...
    assert len(set(OUTPUT_NAMES) | {"WORKSHOP_MAIN_CLOSURE_RECEIPT.json"}) == 8
    runtime_payload = serialized_outputs["WORKSHOP_MAIN_RUNTIME_UNCERTAINTY.json"]
    runtime_record = {
        "name": "WORKSHOP_MAIN_RUNTIME_UNCERTAINTY.json",
    ...
    print("V1A3C4_R1_OUTPUT_MANIFEST_SELF_TEST_OK")
    print("V1A3C4_R1_ZERO_BYTE_SELF_TEST_OK")



EXPECTED_CONTEXT_ID = "CTX-68030A15EE97A526"
EXPECTED_PATH_GROUP_ID = "PATHGROUP-266CA7411F12A68D"
EXPECTED_LAUNCHER_LINE = 24
EXPECTED_INTERPRETER_REFERENCE = "python"
EXPECTED_SOURCE_KIND = "nested_shell"
EXPECTED_WORKING_DIRECTORY = r"Z:\FOXAI"
EXPECTED_UNCERTAINTY = [
    "interpreter_command_alias_not_resolved",
    "runtime_sys_path_unavailable",
]

MANAGER_BASELINE_MISSION_ID = "ENG-20260721-224437-0FDDE1"
MANAGER_BASELINE_CONTEXT_ID = "CTX-EEC78B5B382B239D"
MANAGER_BASELINE_PATH_GROUP_ID = "PATHGROUP-25300F3FBF74CEEB"
MANAGER_BASELINE_LAUNCHER_LINE = 12
MANAGER_BASELINE_SCRIPT = r"Z:\FOXAI\System\PortableRuntime\manage_comfyui_normal.py"
MANAGER_BASELINE_RESULT = "workshop_comfyui_manager_dependency_closure_complete"
MANAGER_BASELINE_RECEIPT = "WORKSHOP_COMFYUI_MANAGER_CLOSURE_RECEIPT.json"
MANAGER_BASELINE_CORE_OUTPUTS = (
    "WORKSHOP_COMFYUI_MANAGER_CONTEXT.json",
    "WORKSHOP_COMFYUI_MANAGER_FIRST_PARTY_CLOSURE.json",
    "WORKSHOP_COMFYUI_MANAGER_CONDITIONAL_IMPORTS.json",
    "WORKSHOP_COMFYUI_MANAGER_PACKAGE_REQUIREMENTS.json",
    "WORKSHOP_COMFYUI_MANAGER_UNRESOLVED_BRANCHES.json",
    "WORKSHOP_COMFYUI_MANAGER_COVERAGE.json",
)
...
def select_workshop_main_context(source: dict[str, Any]) -> dict[str, Any]:
    matches = [
        ...
        and win_case(row.get("launcher_path")) == win_case(TARGET_LAUNCHER)
        and win_case(row.get("script_path")) == win_case(TARGET_SCRIPT)
        and str(row.get("interpreter_reference_kind")) == TARGET_INTERPRETER_KIND
    ]
    if len(matches) != 1:
        raise ValueError(f"Expected exactly one Workshop Main context, found {len(matches)}")
    context = dict(matches[0])
    if context.get("path_group_id") != EXPECTED_PATH_GROUP_ID:
        raise ValueError(f"Unexpected Workshop Main path group: {context.get('path_group_id')}")
    if int(context.get("line") or 0) != EXPECTED_LAUNCHER_LINE:
        raise ValueError(f"Unexpected Workshop Main launcher line: {context.get('line')}")
    if str(context.get("interpreter_reference")) != EXPECTED_INTERPRETER_REFERENCE:
        raise ValueError(f"Unexpected interpreter reference: {context.get('interpreter_reference')}")
    if context.get("resolved_interpreter_path") is not None:
        raise ValueError("Workshop Main command alias was falsely resolved to an executable")
    if context.get("runtime_id") is not None:
        raise ValueError("Workshop Main command alias was falsely assigned a runtime ID")
    if context.get("runtime_identity_directly_observed") is not False:
        raise ValueError("Workshop Main runtime identity was falsely labeled directly observed")
    if str(context.get("source_kind")) != EXPECTED_SOURCE_KIND:
        raise ValueError(f"Unexpected source kind: {context.get('source_kind')}")
    if win_case(context.get("working_directory")) != win_case(EXPECTED_WORKING_DIRECTORY):
        raise ValueError(f"Unexpected working directory: {context.get('working_directory')}")
    if [str(value) for value in context.get("arguments", [])] != []:
        raise ValueError(f"Unexpected Workshop Main arguments: {context.get('arguments')}")
    if [str(value) for value in context.get("flags", [])] != []:
        raise ValueError(f"Unexpected Workshop Main flags: {context.get('flags')}")
    if sorted(str(value) for value in context.get("uncertainty", [])) != sorted(EXPECTED_UNCERTAINTY):
        raise ValueError(f"Unexpected runtime uncertainty: {context.get('uncertainty')}")
    context["role"] = "workshop_main_foxai"
    context["context_selected_from_verified_evidence"] = True
    context["context_execution_proven"] = False
    context["runtime_resolution_status"] = "unresolved_command_alias"
    return context


def load_completed_manager_baseline(manager_baseline_dir: Path) -> dict[str, Any]:
    receipt_path = manager_baseline_dir / MANAGER_BASELINE_RECEIPT
    if not receipt_path.is_file():
        raise FileNotFoundError(receipt_path)
    receipt = read_json(receipt_path)
    if receipt.get("mission_id") != MANAGER_BASELINE_MISSION_ID:
        raise ValueError(f"Unexpected manager baseline mission: {receipt.get('mission_id')}")
    if receipt.get("result") != MANAGER_BASELINE_RESULT:
        raise ValueError(f"Unexpected manager baseline result: {receipt.get('result')}")
    if receipt.get("context_id") != MANAGER_BASELINE_CONTEXT_ID:
        raise ValueError(f"Unexpected manager baseline context: {receipt.get('context_id')}")
    if receipt.get("path_group_id") != MANAGER_BASELINE_PATH_GROUP_ID:
        raise ValueError(f"Unexpected manager baseline path group: {receipt.get('path_group_id')}")
    if int(receipt.get("launcher_line") or 0) != MANAGER_BASELINE_LAUNCHER_LINE:
        raise ValueError(f"Unexpected manager baseline launcher line: {receipt.get('launcher_line')}")
    if receipt.get("internal_deterministic_rebuild_match") is not True:
        raise ValueError("Manager baseline deterministic rebuild is not verified")
    if receipt.get("related_pending_context_id") != EXPECTED_CONTEXT_ID:
        raise ValueError("Manager baseline does not identify this Workshop Main context as related pending")
    if receipt.get("related_context_built") is not False:
        raise ValueError("Manager baseline falsely claims the Workshop Main context was built")
    recorded = {
        str(row.get("name")): row
        for row in receipt.get("core_outputs_before_receipt", [])
    }
    checks = []
    for name in MANAGER_BASELINE_CORE_OUTPUTS:
        row = recorded.get(name)
        path = manager_baseline_dir / name
        if row is None:
            raise ValueError(f"Manager baseline receipt omits required output: {name}")
        if not path.is_file():
            raise FileNotFoundError(path)
        size = path.stat().st_size
        digest = sha256_file(path)
        verify_recorded_size_and_hash(size, digest, row, f"Manager baseline output {name}")
        checks.append({"name": name, "size_bytes": size, "sha256": digest, "status": "verified"})
    context_output = read_json(manager_baseline_dir / "WORKSHOP_COMFYUI_MANAGER_CONTEXT.json")
    if context_output.get("context", {}).get("context_id") != MANAGER_BASELINE_CONTEXT_ID:
        raise ValueError("Manager baseline context output changed")
    if win_case(context_output.get("entry_script")) != win_case(MANAGER_BASELINE_SCRIPT):
        raise ValueError("Manager baseline entry script changed")
    return {
        "status": "linked_completed_context_verified",
        "mission_id": MANAGER_BASELINE_MISSION_ID,
        "context_id": MANAGER_BASELINE_CONTEXT_ID,
        "path_group_id": MANAGER_BASELINE_PATH_GROUP_ID,
        "launcher_line": MANAGER_BASELINE_LAUNCHER_LINE,
        "entry_script": MANAGER_BASELINE_SCRIPT,
        "result": MANAGER_BASELINE_RESULT,
        "receipt_path": str(receipt_path),
        "receipt_sha256": sha256_file(receipt_path),
        "core_output_checks": checks,
        "context_built_by_this_mission": False,
        "nodes_or_edges_included": False,
        "cartesian_expansion_used": False,
    }


def build_runtime_uncertainty_report(
    mission_id: str,
    source: dict[str, Any],
    context: dict[str, Any],
) -> dict[str, Any]:
    candidates = []
    for row in sorted(
        source.get("runtime_map", {}).get("records", []),
        key=lambda item: win_case(item.get("executable")),
    ):
        candidates.append({
            "runtime_id": row.get("runtime_id"),
            "executable": row.get("executable"),
            "owner": row.get("owner"),
            "portability_classification": row.get("portability_classification"),
            "host_base_dependent": row.get("host_base_dependent"),
            "probe_status": row.get("probe_status"),
            "site_policy": row.get("site_policy"),
            "candidate_basis": "known runtime identity from prior verified evidence only",
            "launcher_alias_link_proven": False,
            "present_on_live_PATH_proven": False,
            "selected_for_context": False,
        })
    return {
        "schema": f"{SCHEMA_PREFIX}.workshop_main_runtime_uncertainty.v1",
        "mission_id": mission_id,
        "context_id": context.get("context_id"),
        "path_group_id": context.get("path_group_id"),
        "launcher_path": context.get("launcher_path"),
        "launcher_line": context.get("line"),
        ...
        "runtime_id": None,
        "runtime_sys_path": None,
        "uncertainty": list(context.get("uncertainty", [])),
        "runtime_resolution_status": "unresolved_command_alias",
        "runtime_identity_directly_observed": False,
        "single_runtime_proven": False,
        "selected_runtime_candidate_count": 0,
        "proven_alias_candidate_count": 0,
        "known_runtime_identity_count": len(candidates),
        "known_runtime_identities_not_linked_to_alias": candidates,
        "path_group_entries": context.get("effective_path_entries", []),
        "why_no_single_runtime_is_proven": [
            "The launcher invokes the command alias python rather than an exact executable path.",
            "Verified launcher evidence did not capture a PATH or PATHEXT resolution for this invocation.",
            "Runtime sys.path is unavailable because no runtime identity was directly resolved.",
            "The separate manager invocation in the same launcher uses an explicit runtime but does not prove this alias resolves to that runtime.",
            "No live PATH query or command execution is authorized by this mission.",
        ],
        "live_path_queried": False,
        "python_alias_executed": False,
        "runtime_probe_executed": False,
        "runtime_assignment_inferred_from_neighbor_context": False,
    }


_base_build_core_outputs = build_core_outputs
...
def build_core_outputs(
    ...
    outputs = _base_build_core_outputs(
        mission_id, source, context, closure, parsed, protected_checks
    )
    baseline = source["linked_manager_baseline"]
    runtime_uncertainty = build_runtime_uncertainty_report(mission_id, source, context)

    context_output = outputs["WORKSHOP_MAIN_CONTEXT.json"]
    context_output["context_id"] = EXPECTED_CONTEXT_ID
    context_output["launcher_line"] = EXPECTED_LAUNCHER_LINE
    context_output["source_kind"] = EXPECTED_SOURCE_KIND
    context_output["interpreter_reference"] = EXPECTED_INTERPRETER_REFERENCE
    context_output["interpreter_reference_kind"] = TARGET_INTERPRETER_KIND
    context_output["resolved_interpreter_path"] = None
    context_output["runtime_id"] = None
    context_output["runtime_resolution_status"] = "unresolved_command_alias"
    context_output["runtime_uncertainty_output"] = "WORKSHOP_MAIN_RUNTIME_UNCERTAINTY.json"
    context_output["linked_completed_manager_context"] = baseline

    closure_output = outputs["WORKSHOP_MAIN_FIRST_PARTY_CLOSURE.json"]
    closure_output["runtime_resolution_status"] = "unresolved_command_alias"
    closure_output["runtime_id"] = None
    closure_output["linked_manager_context_id"] = baseline["context_id"]
    closure_output["linked_manager_context_nodes_or_edges_included"] = False

    requirements = outputs["WORKSHOP_MAIN_PACKAGE_REQUIREMENTS.json"]
    requirements["runtime_identity_proven"] = False
    requirements["runtime_sys_path_available"] = False
    requirements["interpreter_alias_resolution_status"] = "unresolved"
    requirements["requirements_are_static_candidates_only"] = True

    coverage = outputs["WORKSHOP_MAIN_COVERAGE.json"]
    coverage["result"] = "workshop_main_dependency_closure_complete"
    coverage["context_id"] = EXPECTED_CONTEXT_ID
    coverage["path_group_id"] = EXPECTED_PATH_GROUP_ID
    coverage["launcher_line"] = EXPECTED_LAUNCHER_LINE
    coverage["source_kind"] = EXPECTED_SOURCE_KIND
    coverage["interpreter_reference"] = EXPECTED_INTERPRETER_REFERENCE
    coverage["interpreter_reference_kind"] = TARGET_INTERPRETER_KIND
    coverage["resolved_interpreter_path"] = None
    coverage["runtime_id"] = None
    coverage["runtime_identity_proven"] = False
    coverage["runtime_sys_path_available"] = False
    coverage["runtime_alias_falsely_resolved"] = False
    coverage["known_runtime_identity_count"] = runtime_uncertainty["known_runtime_identity_count"]
    coverage["selected_runtime_candidate_count"] = 0
    coverage["live_path_queried"] = False
    coverage["linked_completed_manager_context_count"] = 1
    coverage["linked_completed_manager_mission_id"] = baseline["mission_id"]
    coverage["linked_completed_manager_context_id"] = baseline["context_id"]
    coverage["linked_completed_manager_receipt_sha256"] = baseline["receipt_sha256"]
    coverage["linked_manager_context_built"] = False
    coverage["linked_manager_context_nodes_or_edges_included"] = False
    coverage["other_protected_contexts_built"] = 0
    coverage["limitations"] = [
        "Static reachability does not prove an import executed or succeeded.",
        "Function-local imports remain deferred candidates without call-graph execution inference.",
        "Dynamic import references are recorded but not executed or resolved by execution.",
        "The python command alias is unresolved; no specific runtime or runtime sys.path is proven.",
        "Known runtime identities are listed only as static identities and are not linked to the command alias.",
        "The completed manager context is hash-linked without merging or rebuilding its closure.",
        "Only the Workshop Main foxai.py protected context is covered by this mission.",
    ]

    outputs["WORKSHOP_MAIN_RUNTIME_UNCERTAINTY.json"] = runtime_uncertainty
    return outputs
...
def write_outputs(output_dir: Path, outputs: dict[str, Any], mission_id: str) -> None:
    ...
    for name in OUTPUT_NAMES:
        ...
        core_outputs.append({
            "name": name,
            "size_bytes": len(first[name]),
            "sha256": sha256_bytes(first[name]),
        })
    coverage = outputs["WORKSHOP_MAIN_COVERAGE.json"]
    runtime_uncertainty = outputs["WORKSHOP_MAIN_RUNTIME_UNCERTAINTY.json"]
    context_output = outputs["WORKSHOP_MAIN_CONTEXT.json"]
    receipt = {
        "schema": f"{SCHEMA_PREFIX}.workshop_main_closure_receipt.v1",
        "mission_id": mission_id,
        ...
        "path_group_id": EXPECTED_PATH_GROUP_ID,
        "launcher_line": EXPECTED_LAUNCHER_LINE,
        ...
        "runtime_sys_path_available": False,
        "single_runtime_proven": False,
        "selected_runtime_candidate_count": 0,
        "known_runtime_identity_count": runtime_uncertainty.get("known_runtime_identity_count"),
        "live_path_queried": False,
        "python_alias_executed": False,
        "source_launcher_record_count": coverage.get("source_launcher_record_count"),
        "source_code_file_count": coverage.get("source_code_file_count"),
        ...
        "source_buffer_ceiling_bytes": SOURCE_BUFFER_CEILING_BYTES,
        "known_good_candidates_all_match": coverage.get("known_good_candidates_all_match"),
        "linked_completed_manager_mission_id": coverage.get("linked_completed_manager_mission_id"),
        "linked_completed_manager_context_id": coverage.get("linked_completed_manager_context_id"),
        "linked_completed_manager_receipt_sha256": coverage.get("linked_completed_manager_receipt_sha256"),
        "linked_manager_context_built": False,
        "linked_manager_context_nodes_or_edges_included": False,
        "cartesian_expansion_used": False,
        "other_protected_contexts_built": 0,
        "imports_or_source_executed": False,
        "interpreter_child_processes": 0,
        ...
        "existing_foxai_source_modified": False,
        "raw_source_content_persisted": False,
        "context_execution_proven": context_output.get("context_execution_proven"),
        "core_outputs_before_receipt": core_outputs,
    }
    receipt_bytes = canonical_json_bytes(receipt)
    if total_pre_receipt + len(receipt_bytes) >= OUTPUT_CEILING_BYTES:
...
def build(
    ...
    v1a3a_r1_dir: Path,
    v1a3b_dir: Path,
    manager_baseline_dir: Path,
    output_dir: Path,
    mission_id: str,
) -> None:
    source = load_verified_sources(v1a2r2_dir, v1a3a_r1_dir, v1a3b_dir)
    source["linked_manager_baseline"] = load_completed_manager_baseline(manager_baseline_dir)
    context = select_workshop_main_context(source)
    catalogs = provider_catalogs(source)
    closure = preliminary_static_closure(source, context, catalogs)
    if len(closure["reached_paths"]) != EXPECTED_REACHED_NODE_COUNT:
        raise ValueError(f"Unexpected preliminary reached-source count: {len(closure['reached_paths'])}")
    if len(closure["edges"]) != EXPECTED_DEPENDENCY_EDGE_COUNT:
        raise ValueError(f"Unexpected preliminary dependency-edge count: {len(closure['edges'])}")
    if len(closure["unresolved"]) != EXPECTED_PRELIMINARY_UNRESOLVED_COUNT:
        raise ValueError(f"Unexpected preliminary unresolved count: {len(closure['unresolved'])}")
    parsed = parse_reached_sources_once(
        project_root_path, project_root_win, source, closure, catalogs
    )
    protected_checks = verify_current_known_good(
        project_root_path, project_root_win, source["known_good"]
    )
    outputs_a = build_core_outputs(
        mission_id, source, context, closure, parsed, protected_checks
    )
    outputs_b = build_core_outputs(
        mission_id, source, context, closure, parsed, protected_checks
    )
    bytes_a = {name: canonical_json_bytes(outputs_a[name]) for name in OUTPUT_NAMES}
    bytes_b = {name: canonical_json_bytes(outputs_b[name]) for name in OUTPUT_NAMES}
    if any(bytes_a[name] != bytes_b[name] for name in OUTPUT_NAMES):
    ...
    print(json.dumps({
        ...
        "preliminary_unresolved_branch_count": len(closure["unresolved"]),
        "runtime_resolution_status": "unresolved_command_alias",
        "linked_manager_context_id": source["linked_manager_baseline"]["context_id"],
    }, sort_keys=True))
...
def verify_inputs(
    ...
    source = load_verified_sources(v1a2r2_dir, v1a3a_r1_dir, v1a3b_dir)
    context = select_workshop_main_context(source)
    baseline = load_completed_manager_baseline(manager_baseline_dir)
    uncertainty = build_runtime_uncertainty_report("verification", source, context)
    print(json.dumps({
        "status": "verified",
        "context_id": context.get("context_id"),
        "path_group_id": context.get("path_group_id"),
        ...
        "known_runtime_identity_count": uncertainty.get("known_runtime_identity_count"),
        "selected_runtime_candidate_count": 0,
        "manager_baseline_mission_id": baseline.get("mission_id"),
        "manager_baseline_context_id": baseline.get("context_id"),
        "manager_baseline_receipt_sha256": baseline.get("receipt_sha256"),
        "manager_context_built": False,
        "source_launcher_record_count": source["v1a3b_coverage"].get("source_launcher_record_count"),
        "source_code_file_count": source["v1a3b_coverage"].get("source_code_file_count"),
    }, sort_keys=True))
...
def verify_topology(
    ...
    if zero_size != 0 or str(zero_row.get("sha256")) != EMPTY_FILE_SHA256:
        raise ValueError("Zero-byte core package source evidence changed")
    uncertainty = build_runtime_uncertainty_report("verification", source, context)
    if uncertainty.get("selected_runtime_candidate_count") != 0:
        raise ValueError("Runtime candidate falsely selected")
    print(json.dumps({
        "status": "verified",
        "context_id": context.get("context_id"),
        "path_group_id": context.get("path_group_id"),
        ...
        "known_runtime_identity_count": uncertainty.get("known_runtime_identity_count"),
        "selected_runtime_candidate_count": 0,
        "linked_manager_context_id": baseline.get("context_id"),
        "linked_manager_context_nodes_or_edges_included": False,
    }, sort_keys=True))
...
def validate_output(index_dir: Path) -> None:
    ...
    present = {path.name for path in index_dir.iterdir() if path.is_file()}
    if present != required:
        raise ValueError(f"Unexpected output set: present={sorted(present)} required={sorted(required)}")
    context_output = read_json(index_dir / "WORKSHOP_MAIN_CONTEXT.json")
    closure = read_json(index_dir / "WORKSHOP_MAIN_FIRST_PARTY_CLOSURE.json")
    conditional = read_json(index_dir / "WORKSHOP_MAIN_CONDITIONAL_IMPORTS.json")
    requirements = read_json(index_dir / "WORKSHOP_MAIN_PACKAGE_REQUIREMENTS.json")
    unresolved = read_json(index_dir / "WORKSHOP_MAIN_UNRESOLVED_BRANCHES.json")
    runtime_uncertainty = read_json(index_dir / "WORKSHOP_MAIN_RUNTIME_UNCERTAINTY.json")
    coverage = read_json(index_dir / "WORKSHOP_MAIN_COVERAGE.json")
    receipt = read_json(index_dir / "WORKSHOP_MAIN_CLOSURE_RECEIPT.json")
    ...
    if receipt.get("internal_deterministic_rebuild_match") is not True:
        raise ValueError("Deterministic rebuild did not match")
    if receipt.get("protected_direct_context_count") != 1 or context_output.get("protected_direct_context_count") != 1:
        raise ValueError("Expected exactly one protected direct context")
    context = context_output.get("context", {})
    if context.get("context_id") != EXPECTED_CONTEXT_ID:
        raise ValueError(f"Unexpected context ID: {context.get('context_id')}")
    if context.get("path_group_id") != EXPECTED_PATH_GROUP_ID:
        raise ValueError(f"Unexpected path group: {context.get('path_group_id')}")
    if int(context.get("line") or 0) != EXPECTED_LAUNCHER_LINE:
        raise ValueError(f"Unexpected launcher line: {context.get('line')}")
    if win_case(context.get("script_path")) != win_case(TARGET_SCRIPT):
        raise ValueError("Unexpected Workshop Main entry script")
    if str(context.get("source_kind")) != EXPECTED_SOURCE_KIND:
        raise ValueError("Unexpected Workshop Main source kind")
    if str(context.get("interpreter_reference")) != EXPECTED_INTERPRETER_REFERENCE:
        raise ValueError("Unexpected interpreter reference")
    if context.get("resolved_interpreter_path") is not None or context.get("runtime_id") is not None:
        raise ValueError("Unresolved command alias was falsely resolved")
    if sorted(context.get("uncertainty", [])) != sorted(EXPECTED_UNCERTAINTY):
        raise ValueError("Runtime uncertainty changed")
    if runtime_uncertainty.get("runtime_resolution_status") != "unresolved_command_alias":
        raise ValueError("Runtime uncertainty status changed")
    if runtime_uncertainty.get("single_runtime_proven") is not False:
        raise ValueError("A single runtime was falsely proven")
    if runtime_uncertainty.get("selected_runtime_candidate_count") != 0:
        raise ValueError("A runtime candidate was falsely selected")
    if runtime_uncertainty.get("proven_alias_candidate_count") != 0:
        raise ValueError("Alias candidate was falsely proven")
    if runtime_uncertainty.get("live_path_queried") is not False:
        raise ValueError("Live PATH query boundary violated")
    if runtime_uncertainty.get("python_alias_executed") is not False:
        raise ValueError("Python alias execution boundary violated")
    candidates = runtime_uncertainty.get("known_runtime_identities_not_linked_to_alias", [])
    if len(candidates) != int(runtime_uncertainty.get("known_runtime_identity_count") or 0):
        raise ValueError("Known runtime identity count mismatch")
    if any(row.get("selected_for_context") is not False or row.get("launcher_alias_link_proven") is not False for row in candidates):
        raise ValueError("Known runtime identity falsely linked to alias")

    baseline = context_output.get("linked_completed_manager_context", {})
    if baseline.get("mission_id") != MANAGER_BASELINE_MISSION_ID:
        raise ValueError("Manager baseline mission changed")
    if baseline.get("context_id") != MANAGER_BASELINE_CONTEXT_ID:
        raise ValueError("Manager baseline context changed")
    if baseline.get("path_group_id") != MANAGER_BASELINE_PATH_GROUP_ID:
        raise ValueError("Manager baseline path group changed")
    if baseline.get("context_built_by_this_mission") is not False:
        raise ValueError("Manager context was falsely rebuilt")
    if baseline.get("nodes_or_edges_included") is not False:
        raise ValueError("Manager closure was merged")

    if coverage.get("other_protected_contexts_built") != 0:
        raise ValueError("Unexpected additional protected context")
    if coverage.get("cartesian_all_imports_by_all_contexts_expansion") is not False:
        raise ValueError("Cartesian expansion was used")
    if coverage.get("linked_manager_context_built") is not False:
        raise ValueError("Manager context was rebuilt")
    if coverage.get("linked_manager_context_nodes_or_edges_included") is not False:
        raise ValueError("Manager nodes or edges were included")
    if int(coverage.get("peak_source_bytes_in_memory") or 0) > SOURCE_BUFFER_CEILING_BYTES:
        raise ValueError("Source buffer ceiling exceeded")
    if int(coverage.get("maximum_reads_per_source_file") or 0) > 1:
        raise ValueError("A source file was read more than once")
    ...
    if coverage.get("zero_byte_indexed_source_accepted") is not True:
        raise ValueError("Zero-byte indexed source was not accepted")
    if closure.get("all_edges_have_exact_source_path_and_line") is not True:
        raise ValueError("Closure edge lacks exact source path and line evidence")
    if conditional.get("conditionally_guarded_imports_labeled_unconditional") != 0:
        raise ValueError("Conditional import mislabeled")
    if unresolved.get("unresolved_branches_labeled_confirmed") != 0:
        raise ValueError("Unresolved branch falsely confirmed")
    if requirements.get("runtime_identity_proven") is not False:
        raise ValueError("Package requirements falsely claim a proven runtime")
    if requirements.get("runtime_sys_path_available") is not False:
        raise ValueError("Package requirements falsely claim runtime sys.path")

    for key in (
        "imports_or_source_executed", "network_used", "packages_installed",
        "models_loaded", "existing_foxai_source_modified",
    ):
        if receipt.get(key) is not False:
            raise ValueError(f"Unsafe receipt flag: {key}")
    if receipt.get("interpreter_child_processes") != 0 or receipt.get("shell_child_processes") != 0:
        raise ValueError("Payload child process boundary violated")
    if receipt.get("runtime_identity_proven") is not False:
        raise ValueError("Receipt falsely claims runtime identity")
    if receipt.get("single_runtime_proven") is not False:
        raise ValueError("Receipt falsely claims a single runtime")
    if receipt.get("live_path_queried") is not False:
        raise ValueError("Receipt falsely claims or used live PATH query")
    if receipt.get("linked_manager_context_built") is not False:
        raise ValueError("Receipt falsely claims manager context was built")
    if receipt.get("linked_manager_context_nodes_or_edges_included") is not False:
        raise ValueError("Receipt falsely includes manager closure")
    ...
    print(json.dumps({
        ...
        "closure_edge_count": closure.get("edge_count"),
        "unresolved_branch_count": unresolved.get("record_count"),
        "runtime_resolution_status": runtime_uncertainty.get("runtime_resolution_status"),
        "known_runtime_identity_count": runtime_uncertainty.get("known_runtime_identity_count"),
        "selected_runtime_candidate_count": 0,
        "linked_manager_context_id": baseline.get("context_id"),
...
def show_context_requirements(index_dir: Path) -> None:
    ...
    print(json.dumps({
        ...
        "linked_manager_context": context.get("linked_completed_manager_context"),
        "coverage_summary": {
            "closure_node_count": coverage.get("closure_node_count"),
            "closure_edge_count": coverage.get("closure_edge_count"),
            "unresolved_branch_count": coverage.get("unresolved_branch_count"),
            "source_files_read_count": coverage.get("source_files_read_count"),
            "maximum_reads_per_source_file": coverage.get("maximum_reads_per_source_file"),
            "peak_source_bytes_in_memory": coverage.get("peak_source_bytes_in_memory"),
        },
...
def self_test() -> None:
    ...
    synthetic_source = {
        "runtime_map": {
            "records": [
                {
                    "runtime_id": "portable_demo",
                    "executable": r"Z:\FOXAI\Runtime\Demo\python.exe",
                    "owner": "demo",
                    "portability_classification": "project_root_runtime",
                    "host_base_dependent": False,
    ...
    synthetic_context = {
        ...
        "path_group_id": EXPECTED_PATH_GROUP_ID,
        "launcher_path": TARGET_LAUNCHER,
        "line": EXPECTED_LAUNCHER_LINE,
        "working_directory": EXPECTED_WORKING_DIRECTORY,
        "interpreter_reference": EXPECTED_INTERPRETER_REFERENCE,
        "interpreter_reference_kind": TARGET_INTERPRETER_KIND,
        "resolved_interpreter_path": None,
        "runtime_id": None,
        "uncertainty": list(EXPECTED_UNCERTAINTY),
        "effective_path_entries": [],
    }
    report = build_runtime_uncertainty_report("self-test", synthetic_source, synthetic_context)
    assert report["single_runtime_proven"] is False
    assert report["selected_runtime_candidate_count"] == 0
    assert report["known_runtime_identity_count"] == 1
    assert report["known_runtime_identities_not_linked_to_alias"][0]["selected_for_context"] is False
    assert report["live_path_queried"] is False
    assert report["python_alias_executed"] is False
    print("V1A3C4_R1_RUNTIME_UNCERTAINTY_SELF_TEST_OK")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Agent Fox V1A-3C4-R1 Workshop Main foxai.py static dependency closure with unresolved runtime preservation"
    )
    ...
    verify.add_argument("--manager-baseline-dir", required=True)
    ...
    topology.add_argument("--manager-baseline-dir", required=True)
    ...
    build_parser.add_argument("--manager-baseline-dir", required=True)
    ...
    show = sub.add_parser("show-context-requirements")
    show.add_argument("--index-dir", required=True)
    ...
    if args.command == "self-test":
        ...
        verify_inputs(
            Path(args.v1a2r2_dir), Path(args.v1a3a_r1_dir),
            Path(args.v1a3b_dir), Path(args.manager_baseline_dir),
        )
        ...
        build(
            Path(args.project_root), args.project_root_win,
            Path(args.v1a2r2_dir), Path(args.v1a3a_r1_dir),
            Path(args.v1a3b_dir), Path(args.manager_baseline_dir),
