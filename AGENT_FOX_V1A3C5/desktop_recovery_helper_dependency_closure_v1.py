...
SCHEMA_PREFIX = "foxai.agent_fox.technical_core.v1a3c5"
...
TARGET_SCRIPT = r"Z:\FOXAI\System\PortableRuntime\start_comfyui_quiet.py"
TARGET_INTERPRETER_KIND = "directly_observed_python_executable"
EXPECTED_REACHED_NODE_COUNT = 1
...
OUTPUT_NAMES = (
    "DESKTOP_RECOVERY_HELPER_CONTEXT.json",
    "DESKTOP_RECOVERY_HELPER_FIRST_PARTY_CLOSURE.json",
    "DESKTOP_RECOVERY_HELPER_CONDITIONAL_IMPORTS.json",
    "DESKTOP_RECOVERY_HELPER_PACKAGE_REQUIREMENTS.json",
    "DESKTOP_RECOVERY_HELPER_UNRESOLVED_BRANCHES.json",
    "DESKTOP_RECOVERY_HELPER_COVERAGE.json",
)
...
def verify_receipt_outputs(source_dir: Path, receipt_name: str, required: Iterable[str]) -> dict[str, Any]:
    ...
    for name in required:
        ...
        verified_size, verified_digest = verify_recorded_size_and_hash(
            size, digest, row, f"Source evidence {name}"
        )
        checks.append({"name": name, "size_bytes": verified_size, "sha256": verified_digest, "status": "verified"})
    return {
...
def select_desktop_recovery_helper_context(source: dict[str, Any]) -> dict[str, Any]:
    matches = [
    ...
    if len(matches) != 1:
        raise ValueError(f"Expected exactly one Desktop Recovery ComfyUI Helper context, found {len(matches)}")
    context = dict(matches[0])
    context["role"] = "desktop_recovery_helper"
...
def enrich_closure(closure: dict[str, Any], parsed: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
    ...
    for edge in closure["edges"]:
        ...
        item = {
            **edge,
            ...
            "context_role": "desktop_recovery_helper",
        }
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.desktop_recovery_helper_first_party_closure.v1",
        "context_id": context.get("context_id"),
        "context_role": "desktop_recovery_helper",
...
def build_conditional_report(parsed: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.desktop_recovery_helper_conditional_imports.v1",
        "context_count": 1,
...
def build_package_requirements(
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.desktop_recovery_helper_package_requirements.v1",
        "context_id": context.get("context_id"),
        "context_count": 1,
        **groups,
...
def build_unresolved_report(
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.desktop_recovery_helper_unresolved_branches.v1",
        "record_count": len(records),
        "records": records,
...
def build_core_outputs(
    ...
    context_output = {
        "schema": f"{SCHEMA_PREFIX}.desktop_recovery_helper_context.v1",
    ...
    coverage = {
        "schema": f"{SCHEMA_PREFIX}.desktop_recovery_helper_coverage.v1",
        "mission_id": mission_id,
        "result": "desktop_recovery_helper_dependency_closure_complete",
        ...
        "limitations": [
            ...
            "Only the Desktop Recovery ComfyUI Helper protected context is covered by this mission.",
        ],
    }
    return {
        "DESKTOP_RECOVERY_HELPER_CONTEXT.json": context_output,
        "DESKTOP_RECOVERY_HELPER_FIRST_PARTY_CLOSURE.json": enriched,
        "DESKTOP_RECOVERY_HELPER_CONDITIONAL_IMPORTS.json": conditional,
        "DESKTOP_RECOVERY_HELPER_PACKAGE_REQUIREMENTS.json": requirements,
        "DESKTOP_RECOVERY_HELPER_UNRESOLVED_BRANCHES.json": unresolved,
        "DESKTOP_RECOVERY_HELPER_COVERAGE.json": coverage,
    }
...
def write_outputs(output_dir: Path, outputs: dict[str, Any], mission_id: str) -> None:
    ...
    coverage = outputs["DESKTOP_RECOVERY_HELPER_COVERAGE.json"]
    receipt = {
        "schema": f"{SCHEMA_PREFIX}.desktop_recovery_helper_closure_receipt.v1",
        "mission_id": mission_id,
        "result": "desktop_recovery_helper_dependency_closure_complete",
        "source_outputs_verified_exact": True,
    ...
    (output_dir / "DESKTOP_RECOVERY_HELPER_CLOSURE_RECEIPT.json").write_bytes(receipt_bytes)
...
def build(
    ...
    context = select_desktop_recovery_helper_context(source)
...
def validate_output(index_dir: Path) -> None:
    required = set(OUTPUT_NAMES) | {"DESKTOP_RECOVERY_HELPER_CLOSURE_RECEIPT.json"}
    ...
    receipt = read_json(index_dir / "DESKTOP_RECOVERY_HELPER_CLOSURE_RECEIPT.json")
    coverage = read_json(index_dir / "DESKTOP_RECOVERY_HELPER_COVERAGE.json")
    closure = read_json(index_dir / "DESKTOP_RECOVERY_HELPER_FIRST_PARTY_CLOSURE.json")
    context = read_json(index_dir / "DESKTOP_RECOVERY_HELPER_CONTEXT.json")
    conditional = read_json(index_dir / "DESKTOP_RECOVERY_HELPER_CONDITIONAL_IMPORTS.json")
    unresolved = read_json(index_dir / "DESKTOP_RECOVERY_HELPER_UNRESOLVED_BRANCHES.json")
    if receipt.get("result") != "desktop_recovery_helper_dependency_closure_complete":
...
def show_context_requirements(index_dir: Path) -> None:
    context = read_json(index_dir / "DESKTOP_RECOVERY_HELPER_CONTEXT.json")
    requirements = read_json(index_dir / "DESKTOP_RECOVERY_HELPER_PACKAGE_REQUIREMENTS.json")
...
"""
    ...
    print("V1A3C5_ZERO_BYTE_SELF_TEST_OK")




EXPECTED_CONTEXT_ID = "CTX-FCE6D44FA8CBAF96"
EXPECTED_PATH_GROUP_ID = "PATHGROUP-26C1A7D4F9C70C86"
EXPECTED_ARGUMENTS = ["--root", r"Z:\FOXAI", "--source", "desktop"]
EXPECTED_FLAGS = ["-I", "-B", "-S"]
EXPECTED_RUNTIME = r"Z:\FOXAI\Runtime\Desktop\python\python.exe"
LINKED_BASELINE_MISSION_ID = "ENG-20260721-222209-FFD6EF"
LINKED_BASELINE_CONTEXT_ID = "CTX-E47F4DAA05CBCD6B"
LINKED_BASELINE_PATH_GROUP_ID = "PATHGROUP-25300F3FBF74CEEB"
PENDING_CONTEXT_ID = "CTX-5A02B9D4A8E26D64"
PENDING_PATH_GROUP_ID = "PATHGROUP-529C568E9980095F"
PENDING_LAUNCHER_LINE = 54
PENDING_SCRIPT = r"Z:\FOXAI\foxai.py"


def select_desktop_recovery_helper_context(source: dict[str, Any]) -> dict[str, Any]:
    matches=[row for row in source["effective_contexts"].get("contexts",[])
        if row.get("context_id")==EXPECTED_CONTEXT_ID]
    if len(matches)!=1: raise ValueError(f"Expected exactly one Desktop Recovery helper context, found {len(matches)}")
    context=dict(matches[0])
    checks=(
      ...
      (win_case(context.get("script_path"))==win_case(TARGET_SCRIPT),"script"),
      (win_case(context.get("resolved_interpreter_path"))==win_case(EXPECTED_RUNTIME),"runtime"),
      (int(context.get("line") or 0)==43,"launcher line"),
      ([str(x) for x in context.get("arguments",[])]==EXPECTED_ARGUMENTS,"arguments"),
      ([str(x) for x in context.get("flags",[])]==EXPECTED_FLAGS,"flags"),
      (context.get("runtime_id")=="portable_desktop","runtime id"),
      (context.get("runtime_portability_classification")=="project_root_runtime","runtime classification"),
      (win_case(context.get("working_directory"))==win_case(r"Z:\FOXAI"),"working directory"),
    )
    for ok,label in checks:
      if not ok: raise ValueError(f"Unexpected Desktop Recovery helper {label}: {context.get(label.replace(' ','_'))}")
    ignored={str(x.get('name')):x for x in context.get('ignored_environment',[])}
    if set(ignored)!={"PYTHONHOME","PYTHONPATH"}: raise ValueError(f"Unexpected ignored environment: {sorted(ignored)}")
    if "script_directory_suppressed_by_isolated_or_safe_path_policy" not in context.get("uncertainty",[]):
      raise ValueError("Expected isolated-path uncertainty missing")
    context["role"]="desktop_recovery_comfyui_helper"
    context["context_selected_from_verified_evidence"]=True
    context["context_execution_proven"]=False
    return context


def select_pending_gui_context(source: dict[str, Any]) -> dict[str, Any]:
    matches=[row for row in source["effective_contexts"].get("contexts",[]) if row.get("context_id")==PENDING_CONTEXT_ID]
    if len(matches)!=1: raise ValueError(f"Expected one pending Desktop Recovery GUI context, found {len(matches)}")
    row=dict(matches[0])
    if row.get("path_group_id")!=PENDING_PATH_GROUP_ID: raise ValueError("Pending GUI path group changed")
    if win_case(row.get("launcher_path"))!=win_case(TARGET_LAUNCHER): raise ValueError("Pending GUI launcher changed")
    if int(row.get("line") or 0)!=PENDING_LAUNCHER_LINE: raise ValueError("Pending GUI line changed")
    if win_case(row.get("script_path"))!=win_case(PENDING_SCRIPT): raise ValueError("Pending GUI script changed")
    if str(row.get("interpreter_reference"))!="%PYTHONW%": raise ValueError("Pending GUI interpreter reference changed")
    if str(row.get("interpreter_reference_kind"))!="pythonw_static_sibling_of_observed_python": raise ValueError("Pending GUI interpreter kind changed")
    return {
      "context_id":row.get("context_id"),"path_group_id":row.get("path_group_id"),
      "launcher_path":win_norm(row.get("launcher_path")),"launcher_line":int(row.get("line") or 0),
      "interpreter_reference":row.get("interpreter_reference"),
      "interpreter_reference_kind":row.get("interpreter_reference_kind"),
      "resolved_interpreter_path":win_norm(row.get("resolved_interpreter_path")),
      "pythonw_identity_directly_observed":row.get("pythonw_identity_directly_observed"),
      "entry_script":win_norm(row.get("script_path")),"uncertainty":row.get("uncertainty",[]),
      "status":"pending_v1a3c6","built_by_this_mission":False,
    }


def verify_linked_v1a3c2_baseline(source_dir: Path) -> dict[str, Any]:
    names=(
      "WEB_WITH_COMFYUI_HELPER_CONTEXT.json","WEB_WITH_COMFYUI_HELPER_FIRST_PARTY_CLOSURE.json",
      "WEB_WITH_COMFYUI_HELPER_CONDITIONAL_IMPORTS.json","WEB_WITH_COMFYUI_HELPER_PACKAGE_REQUIREMENTS.json",
      "WEB_WITH_COMFYUI_HELPER_UNRESOLVED_BRANCHES.json","WEB_WITH_COMFYUI_HELPER_COVERAGE.json")
    receipt_path=source_dir/"WEB_WITH_COMFYUI_HELPER_CLOSURE_RECEIPT.json"
    if not receipt_path.is_file(): raise FileNotFoundError(receipt_path)
    receipt=read_json(receipt_path)
    if receipt.get("mission_id")!=LINKED_BASELINE_MISSION_ID: raise ValueError("Linked V1A-3C2 mission changed")
    if receipt.get("result")!="web_with_comfyui_helper_dependency_closure_complete": raise ValueError("Linked V1A-3C2 incomplete")
    if receipt.get("internal_deterministic_rebuild_match") is not True: raise ValueError("Linked V1A-3C2 nondeterministic")
    recorded={str(x.get('name')):x for x in receipt.get('core_outputs_before_receipt',[])}
    verified=[]
    for name in names:
      p=source_dir/name; row=recorded.get(name)
      if row is None or not p.is_file(): raise FileNotFoundError(p)
      size,digest=verify_recorded_size_and_hash(p.stat().st_size,sha256_file(p),row,f"Linked V1A-3C2 {name}")
      verified.append({"name":name,"size_bytes":size,"sha256":digest})
    context=read_json(source_dir/"WEB_WITH_COMFYUI_HELPER_CONTEXT.json").get("context",{})
    if context.get("context_id")!=LINKED_BASELINE_CONTEXT_ID or context.get("path_group_id")!=LINKED_BASELINE_PATH_GROUP_ID:
      raise ValueError("Linked V1A-3C2 context identity changed")
    return {"baseline_mission_id":LINKED_BASELINE_MISSION_ID,"baseline_context_id":LINKED_BASELINE_CONTEXT_ID,
      "baseline_path_group_id":LINKED_BASELINE_PATH_GROUP_ID,"baseline_entry_script":TARGET_SCRIPT,
      "baseline_source_argument":"webui","baseline_receipt_sha256":sha256_file(receipt_path),
      "verified_output_count":len(verified),"verified_outputs":verified,
      "linked_context_closure_rebuilt":False,"linked_context_records_duplicated":False}


def decorate_outputs(outputs: dict[str, Any], linked: dict[str, Any], pending: dict[str, Any]) -> dict[str, Any]:
    c=outputs["DESKTOP_RECOVERY_HELPER_CONTEXT.json"]
    c["linked_completed_same_entry_script_context"]=linked
    c["pending_desktop_recovery_gui_context"]=pending
    c["linked_context_closure_rebuilt"]=False
    c["pending_gui_context_built"]=False
    cov=outputs["DESKTOP_RECOVERY_HELPER_COVERAGE.json"]
    cov.update({"linked_v1a3c2_baseline_verified":True,"linked_v1a3c2_mission_id":linked["baseline_mission_id"],
      "linked_context_closure_rebuilt":False,"linked_context_records_duplicated":False,
      "pending_gui_context_id":pending["context_id"],"pending_gui_context_built":False,
      "runtime_id_verified":"portable_desktop","runtime_portability_classification_verified":"project_root_runtime",
      "ignored_environment_names_verified":["PYTHONHOME","PYTHONPATH"],
      "isolated_path_uncertainty_preserved":True,"zero_byte_safe_size_handling_inherited":True})
    cov["limitations"]=["Static reachability does not prove an import executed or succeeded.",
      "Function-local imports remain deferred candidates without call-graph execution inference.",
      "Dynamic import references are recorded but not executed.",
      "Only the Desktop Recovery start_comfyui_quiet.py helper context is built.",
      "V1A-3C2 is hash-linked without merging or duplicating its context.",
      "The Desktop Recovery pythonw GUI context remains pending for V1A-3C6."]
    closure=outputs["DESKTOP_RECOVERY_HELPER_FIRST_PARTY_CLOSURE.json"]
    closure["linked_context_nodes_or_edges_included"]=False
    closure["pending_gui_context_nodes_or_edges_included"]=False
    return outputs


def build(project_root_path: Path,project_root_win: str,v1a2r2_dir: Path,v1a3a_r1_dir: Path,v1a3b_dir: Path,baseline_dir: Path,output_dir: Path,mission_id: str)->None:
    source=load_verified_sources(v1a2r2_dir,v1a3a_r1_dir,v1a3b_dir)
    context=select_desktop_recovery_helper_context(source)
    pending=select_pending_gui_context(source)
    linked=verify_linked_v1a3c2_baseline(baseline_dir)
    catalogs=provider_catalogs(source); closure=preliminary_static_closure(source,context,catalogs)
    parsed=parse_reached_sources_once(project_root_path,project_root_win,source,closure,catalogs)
    checks=verify_current_known_good(project_root_path,project_root_win,source["known_good"])
    a=decorate_outputs(build_core_outputs(mission_id,source,context,closure,parsed,checks),linked,pending)
    b=decorate_outputs(build_core_outputs(mission_id,source,context,closure,parsed,checks),linked,pending)
    if {n:canonical_json_bytes(a[n]) for n in OUTPUT_NAMES}!={n:canonical_json_bytes(b[n]) for n in OUTPUT_NAMES}:
      raise ValueError("Desktop Recovery helper deterministic rebuild failed")
    write_outputs(output_dir,a,mission_id)


def verify_inputs(v1a2r2_dir: Path,v1a3a_r1_dir: Path,v1a3b_dir: Path,baseline_dir: Path)->None:
    source=load_verified_sources(v1a2r2_dir,v1a3a_r1_dir,v1a3b_dir)
    c=select_desktop_recovery_helper_context(source); p=select_pending_gui_context(source); l=verify_linked_v1a3c2_baseline(baseline_dir)
    print(json.dumps({"status":"verified","context_id":c["context_id"],"path_group_id":c["path_group_id"],
      "launcher_line":c["line"],"arguments":c["arguments"],"flags":c["flags"],"runtime_id":c["runtime_id"],
      "runtime_portability_classification":c["runtime_portability_classification"],
      "ignored_environment_names":sorted(x["name"] for x in c["ignored_environment"]),"uncertainty":c["uncertainty"],
      "linked_baseline_mission_id":l["baseline_mission_id"],"linked_context_closure_rebuilt":False,
      "pending_gui_context_id":p["context_id"],"pending_gui_context_built":False},sort_keys=True))


def verify_topology(v1a2r2_dir: Path,v1a3a_r1_dir: Path,v1a3b_dir: Path,baseline_dir: Path)->None:
    source=load_verified_sources(v1a2r2_dir,v1a3a_r1_dir,v1a3b_dir); c=select_desktop_recovery_helper_context(source)
    closure=preliminary_static_closure(source,c,provider_catalogs(source))
    if len(closure["reached_paths"])!=1 or len(closure["edges"])!=0 or len(closure["unresolved"])!=1:
      raise ValueError("Unexpected Desktop Recovery helper topology")
    u=closure["unresolved"][0]
    if str(u.get("module"))!="psutil" or int(u.get("line") or 0)!=38 or win_case(u.get("source_path"))!=win_case(TARGET_SCRIPT):
      raise ValueError(f"Unexpected unresolved helper branch: {u}")
    l=verify_linked_v1a3c2_baseline(baseline_dir); p=select_pending_gui_context(source)
    print(json.dumps({"status":"verified","context_id":c["context_id"],"path_group_id":c["path_group_id"],
      "reached_source_count":1,"dependency_edge_count":0,"preliminary_unresolved_branch_count":1,
      "preliminary_unresolved":closure["unresolved"],"linked_baseline_mission_id":l["baseline_mission_id"],
      "linked_context_closure_rebuilt":False,"pending_gui_context_id":p["context_id"],"pending_gui_context_built":False},sort_keys=True))


def validate_output(index_dir: Path)->None:
    required=set(OUTPUT_NAMES)|{"DESKTOP_RECOVERY_HELPER_CLOSURE_RECEIPT.json"}; present={p.name for p in index_dir.iterdir() if p.is_file()}
    if present!=required: raise ValueError(f"Unexpected Desktop Recovery helper output set: {sorted(present)}")
    receipt=read_json(index_dir/"DESKTOP_RECOVERY_HELPER_CLOSURE_RECEIPT.json")
    context_output=read_json(index_dir/"DESKTOP_RECOVERY_HELPER_CONTEXT.json"); c=context_output.get("context",{})
    coverage=read_json(index_dir/"DESKTOP_RECOVERY_HELPER_COVERAGE.json")
    closure=read_json(index_dir/"DESKTOP_RECOVERY_HELPER_FIRST_PARTY_CLOSURE.json")
    conditional=read_json(index_dir/"DESKTOP_RECOVERY_HELPER_CONDITIONAL_IMPORTS.json")
    unresolved=read_json(index_dir/"DESKTOP_RECOVERY_HELPER_UNRESOLVED_BRANCHES.json")
    if receipt.get("result")!="desktop_recovery_helper_dependency_closure_complete": raise ValueError("Unexpected receipt result")
    if receipt.get("internal_deterministic_rebuild_match") is not True: raise ValueError("Determinism failed")
    if c.get("context_id")!=EXPECTED_CONTEXT_ID or c.get("path_group_id")!=EXPECTED_PATH_GROUP_ID: raise ValueError("Context changed")
    if int(c.get("line") or 0)!=43 or [str(x) for x in c.get("arguments",[])]!=EXPECTED_ARGUMENTS: raise ValueError("Launcher line/arguments changed")
    if c.get("runtime_id")!="portable_desktop" or c.get("runtime_portability_classification")!="project_root_runtime": raise ValueError("Runtime evidence changed")
    linked=context_output.get("linked_completed_same_entry_script_context",{}); pending=context_output.get("pending_desktop_recovery_gui_context",{})
    if linked.get("baseline_mission_id")!=LINKED_BASELINE_MISSION_ID or linked.get("linked_context_closure_rebuilt") is not False: raise ValueError("Linked baseline changed")
    if pending.get("context_id")!=PENDING_CONTEXT_ID or pending.get("built_by_this_mission") is not False: raise ValueError("Pending GUI separation changed")
    if closure.get("node_count")!=1 or closure.get("edge_count")!=0: raise ValueError("Closure topology changed")
    if coverage.get("preliminary_unresolved_branch_count")!=1 or coverage.get("pending_gui_context_built") is not False: raise ValueError("Coverage changed")
    if int(coverage.get("maximum_reads_per_source_file") or 0)>1 or int(coverage.get("peak_source_bytes_in_memory") or 0)>SOURCE_BUFFER_CEILING_BYTES: raise ValueError("Read/memory ceiling violated")
    if closure.get("all_edges_have_exact_source_path_and_line") is not True: raise ValueError("Edge evidence incomplete")
    if conditional.get("conditionally_guarded_imports_labeled_unconditional")!=0 or unresolved.get("unresolved_branches_labeled_confirmed")!=0: raise ValueError("Classification error")
    rows=unresolved.get("records",[])
    if not any(str(x.get("module"))=="psutil" and int(x.get("line") or 0)==38 for x in rows): raise ValueError("psutil unresolved record missing")
    for key in ("imports_or_source_executed","network_used","packages_installed","models_loaded","existing_foxai_source_modified"):
      if receipt.get(key) is not False: raise ValueError(f"Unsafe receipt flag {key}")
    if receipt.get("interpreter_child_processes")!=0 or receipt.get("shell_child_processes")!=0: raise ValueError("Child process boundary violated")
    for row in receipt.get("core_outputs_before_receipt",[]):
      p=index_dir/str(row.get("name")); verify_recorded_size_and_hash(p.stat().st_size,sha256_file(p),row,f"Generated output {p}")
    ...
    print(json.dumps({"status":"valid","context_id":EXPECTED_CONTEXT_ID,"path_group_id":EXPECTED_PATH_GROUP_ID,
      "reached_source_count":1,"dependency_edge_count":0,"preliminary_unresolved_branch_count":1,
      "linked_baseline_mission_id":LINKED_BASELINE_MISSION_ID,"pending_gui_context_id":PENDING_CONTEXT_ID,
      "linked_context_closure_rebuilt":False,"pending_gui_context_built":False,"total_output_bytes":total},sort_keys=True))


def show_context_requirements(index_dir: Path)->None:
    print(json.dumps({"context":read_json(index_dir/"DESKTOP_RECOVERY_HELPER_CONTEXT.json"),
      "requirements":read_json(index_dir/"DESKTOP_RECOVERY_HELPER_PACKAGE_REQUIREMENTS.json"),
      "unresolved":read_json(index_dir/"DESKTOP_RECOVERY_HELPER_UNRESOLVED_BRANCHES.json")},indent=2,sort_keys=True))


def main(argv: list[str]|None=None)->int:
    parser=argparse.ArgumentParser(description="Agent Fox V1A-3C5 bounded Desktop Recovery ComfyUI helper static closure")
    sub=parser.add_subparsers(dest="command",required=True); sub.add_parser("self-test")
    for name in ("verify-inputs","verify-topology"):
      p=sub.add_parser(name); p.add_argument("--v1a2r2-dir",required=True); p.add_argument("--v1a3a-r1-dir",required=True); p.add_argument("--v1a3b-dir",required=True); p.add_argument("--v1a3c2-dir",required=True)
    p=sub.add_parser("build"); p.add_argument("--project-root",default=PROJECT_ROOT_DEFAULT); p.add_argument("--project-root-win",default=PROJECT_ROOT_DEFAULT)
    p.add_argument("--v1a2r2-dir",required=True); p.add_argument("--v1a3a-r1-dir",required=True); p.add_argument("--v1a3b-dir",required=True); p.add_argument("--v1a3c2-dir",required=True); p.add_argument("--output-dir",required=True); p.add_argument("--mission-id",required=True)
    ...
    elif a.command=="verify-inputs": verify_inputs(Path(a.v1a2r2_dir),Path(a.v1a3a_r1_dir),Path(a.v1a3b_dir),Path(a.v1a3c2_dir))
    elif a.command=="verify-topology": verify_topology(Path(a.v1a2r2_dir),Path(a.v1a3a_r1_dir),Path(a.v1a3b_dir),Path(a.v1a3c2_dir))
    elif a.command=="build": build(Path(a.project_root),a.project_root_win,Path(a.v1a2r2_dir),Path(a.v1a3a_r1_dir),Path(a.v1a3b_dir),Path(a.v1a3c2_dir),Path(a.output_dir),a.mission_id)
    elif a.command=="validate-output": validate_output(Path(a.index_dir))
    elif a.command=="show-context-requirements": show_context_requirements(Path(a.index_dir))
    return 0
