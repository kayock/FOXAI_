...
def redact_text(value: Any) -> Any:
    ...
    for marker in PROFILE_MARKERS:
        if re.fullmatch(r"[A-Za-z0-9._-]+", marker):
            ...
            result = re.sub(re.escape(marker), lambda _m: "C:\\Users\\<REDACTED>", result, flags=re.IGNORECASE)
    result = re.sub(r"(?i)C:\\Users\\[^\\\s\"']+", lambda _m: "C:\\Users\\<REDACTED>", result)
...
def _privacy_assertions(payloads: Iterable[bytes]) -> None:
    ...
    for marker in PROFILE_MARKERS:
        ...
        if encoded and encoded in lower:
            raise AssertionError("profile marker leaked into minimal-load evidence")
    forbidden = (
        b'"process_arguments"',
