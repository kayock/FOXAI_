...
BASELINE_SHA256 = "7fcbddeae22904af7f9aa75e9546e3e28721d455222fbfc42c27c5186ba45180"
CANDIDATE_SHA256 = "ecccf3b4a780d9de6ef2aa56522c6b65d06035c42a4a9050d72b95df530c40d0"
DIFF_SHA256 = "41efcd8d4ee744a962d24005924c7f6e1dd1d140b0410121a16229ad88348b00"
SERVER_SHA256 = "238931aaa46446448696c5000ae9b744f44d171fd491e0d41b3562b8d9fddd81"
APPLICATION_REGISTRY_SHA256 = "6338e10b813460ee421e4cbf3d9d74fd82d5f24178347e35f4318ef3c4ef9022"
FLEET_REGISTRY_SHA256 = "18745be73f67e073c002bb645a3c0eaad0a3090ebee1b3bb547ddcc2f147bdb6"
SERVICE_REGISTRY_SHA256 = "cc798df061a27a51c4ea1f64b3757d2a92724a9a5768e4c190846966efe0251b"
LIVE_VERIFY_RECEIPT_SHA256 = "694194c92ce47b1bb9ffc8aaf32cd8d4d8367eaf809feb1215ede8797f2b4954"
PREVIEW_RECEIPT_SHA256 = "66caf6faed169d3980aa17f0d30eea83f8147e9fe1314f5c9c77c3cb678647b3"

UNCHANGED_CONTRACTS = {
    "core/server.py": SERVER_SHA256,
    "Config/application_registry.json": APPLICATION_REGISTRY_SHA256,
    "Config/fleet_registry.json": FLEET_REGISTRY_SHA256,
    "core/service_registry.py": SERVICE_REGISTRY_SHA256,
}
...
def find_root(start: Path) -> Path:
    for candidate in (start, *start.parents):
        if (
            ...
            return candidate
    raise ApplyError(
        r"FOXAI root not found. Extract the complete EMI1A folder directly inside Z:\FOXAI."
    )
...
def load_verifier(package: Path):
    ...
    module_name = "emi1a_approved_preview_verifier"
...
def validate_live_receipt(package: Path) -> dict[str, Any]:
    ...
    inventory = checks.get("live_inventory_preview") or {}
    conditions = {
        "state":
        ...
        "state_file_absent":
            inventory.get("state_file_exists") is False,
        "state_file_unchanged":
            inventory.get("state_file_unchanged") is True,
        "projector_separate":
            inventory.get("projectors_separate") is True,
    }
...
def unchanged_contracts(root: Path) -> dict[str, Any]:
    checks = []
    for relative, expected in UNCHANGED_CONTRACTS.items():
        path = root / relative
        actual = sha256(path) if path.is_file() else None
        checks.append({
            "path": relative,
    ...
    if not all(item["ok"] for item in checks) or not state_absent:
        raise ApplyError(
            "An explicitly unchanged registry, server, service, or "
            "extension-state contract is not in the approved state."
        )
    return {
        "passed": True,
        "files": checks,
        "extension_state_absent": state_absent,
    }
...
def verification_suite(
    ...
    result = {
        ...
        "source_snapshot":
            verifier.source_snapshot_grounding(preview),
        "exact_artifacts":
            verifier.exact_artifacts(preview),
        "node_and_browser":
            verifier.node_and_browser(preview),
        "backend_regression":
            verifier.fake_backend_harness(preview),
        "live_inventory":
            verifier.backend_harness(preview, root),
        "legacy_controls_unchanged":
            verifier.legacy_controls_unchanged(preview),
        "static_contract":
...
def install_candidate(root: Path, package: Path) -> dict[str, Any]:
    ...
    stage = target.with_name(
        f".{target.name}.emi1.{os.getpid()}.new"
    )
    try:
...
def rollback(
    ...
    target = root / TARGET
    stage = target.with_name(
        f".{target.name}.emi1.rollback.{os.getpid()}"
    )
    try:
...
def write_report(output: Path, receipt: dict[str, Any]) -> None:
    lines = [
        "# Extension Manager Inventory & Health Phase 1",
        "",
        ...
        "",
        "Registries, service registry, server, and extension state are "
...
def main() -> int:
    ...
    receipt: dict[str, Any] = {
        "action":
            "extension_manager_inventory_health_phase1_transactional_apply",
    ...
    try:
        ...
        print()
        print("=" * 72)
        print("FOXAI EXTENSION MANAGER — INVENTORY & HEALTH")
        print("PHASE 1 TRANSACTIONAL APPLY")
        print("=" * 72)
        print()
        print("Preflight verified.")
        print("Allowed live target:", TARGET)
        print("Explicitly unchanged:")
        print("  core/server.py")
        print("  Config/application_registry.json")
        print("  Config/fleet_registry.json")
        print("  core/service_registry.py")
        print("  Config/extension_state.json remains absent")
        ...
        if entered != APPROVAL_PHRASE:
            ...
            if protected_changes:
                raise ApplyError(
                    "A protected non-target, registry, state file, "
