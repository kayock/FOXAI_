...
def root(start):
 ...
 raise VerifyError(r'FOXAI root not found. Extract EMS21P_R2 directly inside Z:\FOXAI.')
...
def exact(pkg):
 ...
 baseline_text=b.read_text(encoding='utf-8')
 candidate_text=c.read_text(encoding='utf-8')
 diff_text=d.read_text(encoding='utf-8')
 compile(baseline_text,str(b),'exec');compile(candidate_text,str(c),'exec')
 base_lines=baseline_text.splitlines(keepends=True)
 diff_lines=diff_text.splitlines(keepends=True)
 output=[];source_position=0;index=0;hunks=0
 while index<len(diff_lines):
  ...
  if not match:raise VerifyError('Malformed unified-diff hunk header.')
  old_start=int(match.group(1));old_count=int(match.group(2) or '1')
  new_count=int(match.group(4) or '1')
  target_position=old_start-1
  if target_position<source_position:raise VerifyError('Overlapping unified-diff hunks.')
  output.extend(base_lines[source_position:target_position]);source_position=target_position
  old_seen=0;new_seen=0;index+=1;hunks+=1
  while index<len(diff_lines) and not diff_lines[index].startswith('@@ '):
   ...
   if hunk_line.startswith('\\ No newline at end of file'):
    index+=1
    continue
   if not hunk_line:raise VerifyError('Malformed empty unified-diff line.')
   marker=hunk_line[0];content=hunk_line[1:]
   if marker==' ':
    if source_position>=len(base_lines) or base_lines[source_position]!=content:
     raise VerifyError('Unified-diff context does not match baseline.')
    output.append(content);source_position+=1;old_seen+=1;new_seen+=1
   elif marker=='-':
    if source_position>=len(base_lines) or base_lines[source_position]!=content:
     raise VerifyError('Unified-diff deletion does not match baseline.')
    source_position+=1;old_seen+=1
   elif marker=='+':
    output.append(content);new_seen+=1
   else:
    raise VerifyError('Unexpected unified-diff marker.')
   index+=1
  if old_seen!=old_count or new_seen!=new_count:
   raise VerifyError('Unified-diff hunk counts do not match.')
 output.extend(base_lines[source_position:])
 reconstructed=''.join(output)
 reconstructed_sha=hashlib.sha256(reconstructed.encode('utf-8')).hexdigest()
 if reconstructed!=candidate_text or reconstructed_sha!=CAND_SHA:
  raise VerifyError('Pure-Python diff reconstruction did not match candidate.')
 return {'passed':True,'checks':checks,'python_compile':True,'diff_has_hunks':'@@' in diff_text,'diff_reconstructs_candidate':True,'reconstructed_sha256':reconstructed_sha,'hunks':hunks,'external_patch_required':False}
def applied(pkg):
