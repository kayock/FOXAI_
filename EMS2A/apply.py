...
BASELINE_SHA256 = "ecccf3b4a780d9de6ef2aa56522c6b65d06035c42a4a9050d72b95df530c40d0"
CANDIDATE_SHA256 = "5cffd8b594c5f91b983b391758fa295b151ca82c97ce8ddfcb085cacdd76a548"
DIFF_SHA256 = "86e65fec472f6b5701af24de7e683a81a8340726f1a6fc460feeb0f33a5bdb51"
LIVE_VERIFY_RECEIPT_SHA256 = "40976770263a7ff7e86b12d1b9a0dd5d1f27e4eca23520c95c155270f6438a30"
PREVIEW_RECEIPT_SHA256 = "0567879aff94735ea251a83ab44f1641a368248f83fe5efdfc5a7f276cb6db85"
...
REQUIRED_LIVE_CHECKS = {
    "package_manifest",
    "phase1_applied_receipt",
    "source_snapshot",
    "exact_artifacts",
    "live_baselines",
    "node_and_browser",
    "backend_regression",
    "live_inventory_state_preview",
    "legacy_controls",
    "static_contract",
    "boundary_watch",
}
...
def find_root(start: Path) -> Path:
    for candidate in (start, *start.parents):
        if (
            ...
            return candidate
    raise ApplyError(
        r"FOXAI root not found. Extract the complete EMS2A folder directly inside Z:\FOXAI."
    )
...
def load_verifier(package: Path):
    ...
    module_name = "ems2a_approved_preview_verifier"
...
def validate_live_receipt(package: Path) -> dict[str, Any]:
    ...
    live_state = (
        checks.get("live_inventory_state_preview") or {}
    ).get("result") or {}
    conditions = {
        "state":
        ...
        "no_installer_apply":
            data.get("installer_apply_capability_present") is False,
        "runtime_controls_present":
            data.get("runtime_state_apply_present_in_candidate") is True,
        "exact_scope":
        ...
        "state_file_absent":
            live_state.get("state_file_exists") is False,
        "state_preview_read_only":
            live_state.get("state_file_unchanged") is True,
    }
...
def validate_preview_receipt(package: Path) -> dict[str, Any]:
    ...
    conditions = {
        ...
        "no_deletes":
            data.get("delete_operations") == [],
        "candidate_unchanged":
            data.get("candidate_and_diff_unchanged") is True,
        "verifier_revision":
            data.get("verifier_revision") == 2,
    }
    if not all(conditions.values()):
...
def verification_suite(
    ...
    result = {
        ...
        "source_snapshot":
            verifier.grounding(preview),
        "exact_artifacts":
            verifier.exact_artifacts(preview),
        "node_and_browser":
            verifier.node_browser(preview),
        "backend_regression":
            verifier.backend(preview),
        "live_inventory_state_preview":
            verifier.backend(preview, root),
        "legacy_controls":
            verifier.legacy_controls(preview),
        "static_contract":
...
def install_candidate(root: Path, package: Path) -> dict[str, Any]:
    ...
    stage = target.with_name(
        f".{target.name}.ems2.{os.getpid()}.new"
    )
    try:
...
def rollback(
    ...
    target = root / TARGET
    stage = target.with_name(
        f".{target.name}.ems2.rollback.{os.getpid()}"
    )
    try:
...
def write_report(output: Path, receipt: dict[str, Any]) -> None:
    lines = [
        "# Extension Manager Safe State Controls Phase 2",
        "",
...
def main() -> int:
    ...
    receipt: dict[str, Any] = {
        "action":
            "extension_manager_safe_state_controls_phase2_transactional_source_apply",
    ...
    try:
        ...
        print()
        print("=" * 72)
        print("FOXAI EXTENSION MANAGER — SAFE STATE CONTROLS")
        print("PHASE 2 TRANSACTIONAL SOURCE APPLY")
        print("=" * 72)
        print()
        ...
        print("This apply will NOT enable or disable an extension.")
