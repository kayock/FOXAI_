...
def grounding(package:Path):
 ...
 for item in index.get('files',[]):
  rel=item['grounding_path']; p=package/rel; actual=sha256(p) if p.is_file() else None; checks.append({'path':rel,'expected':item['sha256'],'actual':actual,'ok':actual==item['sha256']})
 if not checks or not all(x['ok'] for x in checks): raise VerifyError('Grounding snapshot failed.')
 return {'passed':True,'files':checks,'provided_extension_state':index.get('provided_extension_state',False),'extension_state_absence_expected':True}
