...
def find_root() -> Path:
    ...
    raise RuntimeError(
        "Could not locate FOXAI root with core/security_containment.py, "
        "Config/engineering_airlock_policy.csv, and env/python/python.exe."
    )
...
def preview(root: Path) -> dict:
    ...
    return {
        "schema": "foxai.engineering.workshop.v1_2_2.policy-preview",
...
def apply(root: Path) -> dict:
    ...
    backup_zip = backup_dir / "before_engineering_workshop_v1_2_2_policy.zip"
    ...
    try:
        ...
        validation_code = (
            ...
            "assert enforcer is not None, 'Casbin enforcer unavailable in WebUI runtime'\n"
            ...
            "print('DECISION=' + str(decision.to_dict()))\n"
        ...
        if result.returncode != 0:
            raise RuntimeError(
                "WebUI-runtime Casbin validation failed.\n"
                + result.stdout
                + result.stderr
            )

        receipt = {
            "schema": "foxai.engineering.workshop.v1_2_2.policy-receipt",
            "result": "installed_verified",
            "timestamp": utc_stamp(),
            "target_root": str(root),
            "diagnosis": (
                "V1.2.1 used Runtime/Desktop Python for validation, but FOXAI "
                "WebUI runs env/python/python.exe. The former lacks the active "
                "Casbin environment, causing a false validation failure and "
                "automatic rollback."
            ),
            "backup": {
...
def main() -> int:
    ...
    print("Engineering Workshop V1.2.2 Casbin policy installed and verified.")
