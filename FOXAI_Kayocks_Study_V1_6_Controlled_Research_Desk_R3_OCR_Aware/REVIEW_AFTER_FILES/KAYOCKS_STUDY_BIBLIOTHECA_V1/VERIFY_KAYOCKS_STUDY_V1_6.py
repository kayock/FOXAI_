...
def select_white_bread_candidate(rows):
    """Prefer the best searchable page-7 copy without altering either PDF."""
    ...
    return max(
        ...
        key=lambda item: (
            ...
            int(item.get("is_ocr_copy") or 0),
            int(item.get("page_text_chars") or len(str(item.get("text") or ""))),
...
def main() -> int:
    ...
    synthetic_selected=select_white_bread_candidate([
        {
            "id":1,
            "title":"Nelson Family Recipe Book",
            "text_status":"image_only_or_low_text",
            "is_ocr_copy":0,
            "page_text_chars":4,
            "text_chars":40,
            "text":"scan",
        ...
        {
            "id":2,
            "title":"Nelson Family Recipe Book OCR searchable",
            "text_status":"searchable_ocr_copy",
            "is_ocr_copy":1,
            "page_text_chars":110,
            "text_chars":165317,
            "text":"White Bread Bake at 375 for 20 minutes then 350 for 25 minutes.",
        },
    ])
    check(
        "white_bread_candidate_prefers_ocr",
        synthetic_selected and int(synthetic_selected.get("id") or 0)==2,
        synthetic_selected,
    )
    ...
    try:
        ...
        candidate_rows=conn.execute("""
            SELECT d.id,d.title,d.path,d.rel_path,d.text_status,d.is_ocr_copy,
                   d.text_chars,d.size_bytes,p.text,p.text_chars AS page_text_chars
            FROM pages p JOIN documents d ON d.id=p.document_id
            WHERE p.page_number=7
              AND (
                LOWER(d.title) LIKE '%nelson%family%recipe%'
                OR LOWER(d.related_stem) LIKE '%nelson%family%recipe%'
              )
            ORDER BY d.id
        """).fetchall()
        row=select_white_bread_candidate(candidate_rows)
        check(
            "white_bread_page_7_present",
            bool(row),
            {
                "candidate_count":len(candidate_rows),
                "message":"Nelson Family Recipe Book page 7 was not found" if not row else "found",
            },
        )
        check(
            "white_bread_searchable_copy_selected",
            str(row.get("text_status") or "") in ("searchable_ocr_copy","searchable","partially_searchable"),
            {
                "selected_id":row.get("id"),
                "selected_title":row.get("title"),
                "selected_status":row.get("text_status"),
                "selected_is_ocr_copy":bool(row.get("is_ocr_copy")),
                "selected_page_text_chars":row.get("page_text_chars"),
                "candidates":[
                    {
                        "id":item["id"],
                        "title":item["title"],
                        "status":item["text_status"],
                        "is_ocr_copy":bool(item["is_ocr_copy"]),
                        "page_text_chars":item["page_text_chars"],
                    }
        ...
        check(
            ...
            first_stage and second_stage,
            {
                ...
                "note":"The source page contains the two timed stages; 45 minutes is their derived total and need not appear as literal PDF text.",
            },
        )
        check(
            "white_bread_total_minutes",
            20+25==45,
            {"stage_minutes":[20,25],"derived_total_minutes":20+25},
        )
