...
function renderSources(items,target='resultList'){
  q(target).innerHTML=items.length?items.map(item=>{
    ...
    const actions=research
      ...
      : `<button class=secondary onclick="openPdf(${item.document_id},${item.page_number})">Open page ${item.page_number}</button><button class=secondary onclick="askFromPage(${item.document_id},${item.page_number})">Ask from this page</button>`;
