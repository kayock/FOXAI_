...
.bookcover:before{content:"";position:absolute;inset:0;background:radial-gradient(circle at 82% 12%,#ffffff2c,transparent 27%),linear-gradient(90deg,#0005 0 5%,transparent 5% 100%);pointer-events:none}
...
<div class="shell">
  ...
  <section id="libraryHome" class="card libraryhome">
    <div class="titleline">
      <div><div class="eyebrow">Visual Library Browser</div><h2>Browse the Bibliotheca</h2><div class="muted">Cover-shaped tiles use existing indexed records. No original file or cover image is changed.</div></div>
...
function coverMarkup(item,detail=false){
  return `<div class="${detail?'detailcover':'bookcover'}" style="${deterministicCoverStyle(item)}"><div class=covermark>${esc(titleInitials(item.title))}</div><div><div class=coverlabel>${esc(item.shelf||'Bibliotheca')}</div><div class=covertext>${esc(item.title||'Untitled document')}</div></div></div>`;
}
...
function filteredLibraryDocuments(){
  ...
  return libraryDocuments.filter(item=>{
    ...
    return [item.title,item.rel_path,item.shelf].some(value=>String(value||'').toLowerCase().includes(query));
  });
}
function tileMarkup(item){
  return `<button class=booktile onclick="openDocumentDetail(${item.id})" aria-label=${JSON.stringify(`Open details for ${item.title||'document'}`)}>${coverMarkup(item)}<div class=tiletitle>${esc(item.title)}</div><div class=tilemeta>${fmt(item.page_count)} pages · ${esc(statusLabel(item.text_status))}</div></button>`;
}
function listRowMarkup(item){
  return `<button class=libraryrow onclick="openDocumentDetail(${item.id})"><div class=listcover style="${deterministicCoverStyle(item)}">${esc(titleInitials(item.title))}</div><div><div class=rowtitle>${esc(item.title)}</div><div class=rowmeta>${esc(item.shelf)} · ${esc(item.rel_path)}</div></div><div class=rowstatus>${fmt(item.page_count)} pages<br>${esc(statusLabel(item.text_status))}</div></button>`;
}
...
function renderLibraryHome(){
  ...
  q('librarySummary').innerHTML=`<div class=stat><span class=small>Documents</span><b>${fmt(libraryDocuments.length)}</b></div><div class=stat><span class=small>Shelves</span><b>${fmt(libraryShelves.filter(s=>s.name!=='Research').length)}</b></div><div class=stat><span class=small>Searchable pages</span><b>${fmt(libraryDocuments.reduce((sum,item)=>sum+Number(item.indexed_pages||0),0))}</b></div><div class=stat><span class=small>Showing</span><b>${fmt(filtered.length)}</b></div>`;
  ...
  for(const item of filtered){if(!groups.has(item.shelf))groups.set(item.shelf,[]);groups.get(item.shelf).push(item)}
...
async function refreshLibraryHome(){
  const [shelfData,documentData]=await Promise.all([api('/api/shelves'),api('/api/documents?include_review=0')]);
  libraryShelves=(shelfData.shelves||[]).filter(item=>item.name!=='Research');
  libraryDocuments=documentData.documents||[];
...
function openDocumentDetail(id){
  const item=libraryDocuments.find(document=>Number(document.id)===Number(id));
  if(!item)return;
  q('documentDetailBody').innerHTML=`<div class=detailbody>${coverMarkup(item,true)}<div><div class=eyebrow>${esc(item.shelf||'Bibliotheca')}</div><h2>${esc(item.title)}</h2><div class=detailmeta><div><b>Pages</b>${fmt(item.page_count)}</div><div><b>Text status</b>${esc(statusLabel(item.text_status))}</div><div><b>Indexed pages</b>${fmt(item.indexed_pages)}</div><div><b>Source type</b>PDF${item.is_ocr_copy?' · OCR copy':''}</div></div><div class=detailpath>${esc(item.rel_path)}</div><div class=controls><button onclick='openPdf(${item.id},1,${JSON.stringify(item.title)},${JSON.stringify(`${item.title}, p. 1`)})'>Open PDF</button><button class=secondary onclick='searchThisDocument(${item.id})'>Search This Document</button><button class=secondary onclick='askThisDocument(${item.id})'>Ask Agent Fox</button></div></div></div>`;
  q('documentDetailDialog').showModal();
}
