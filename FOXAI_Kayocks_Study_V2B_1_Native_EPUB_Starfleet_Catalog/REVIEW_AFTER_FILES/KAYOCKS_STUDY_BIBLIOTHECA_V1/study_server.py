...
def scan_ebook_catalog(paths: AppPaths, candidates: list[Path], *, progress_offset: int = 0) -> dict:
    ...
    try:
        known = {
            ...
            for row in conn.execute("SELECT path,size_bytes,mtime_ns FROM ebooks").fetchall()
        }
...
<div class="shell">
  <section class="hero">
    <div class="eyebrow">Kayock's Study · Bibliotheca V2B.1</div>
    ...
    <div class="controls">
      <button onclick="startIndex()">Scan for New Books</button>
  ...
  <section id="libraryHome" class="card libraryhome">
    <div class="titleline">
      ...
      <div class="controls"><button onclick="startIndex()">Scan for New Books</button><span class="pill ok">Local · Offline</span></div>
    </div>
...
function tileMarkup(item){
  ...
  return `<button class=booktile onclick="openLibraryItemDetail(${JSON.stringify(item.source_kind||'pdf')},${item.id})" aria-label=${JSON.stringify(`Open details for ${item.title||'document'}`)}>${coverMarkup(item)}<div class=tiletitle>${esc(item.title)}</div><div class=tilemeta>${meta}</div></button>`;
}
function listRowMarkup(item){
  ...
  return `<button class=libraryrow onclick="openLibraryItemDetail(${JSON.stringify(item.source_kind||'pdf')},${item.id})">${thumb}<div><div class=rowtitle>${esc(item.title)}</div><div class=rowmeta>${meta}</div></div><div class=rowstatus>${status}</div></button>`;
}
...
function openLibraryItemDetail(kind,id){
  const item=libraryDocuments.find(entry=>entry.source_kind===kind&&Number(entry.id)===Number(id));
  if(!item)return;
  if(kind==='epub'){
    q('documentDetailBody').innerHTML=`<div class=detailbody>${coverMarkup(item,true)}<div><div class=eyebrow>${esc(item.collection||item.shelf||'EPUB')}</div><h2>${esc(item.title)}</h2><div class=detailmeta><div><b>Author</b>${esc(item.creator||'Unknown')}</div><div><b>Format</b>${esc(item.format||'EPUB')}</div><div><b>Chapters</b>${fmt(item.chapter_count)}</div><div><b>Metadata status</b>${esc(ebookStatusLabel(item.status))}</div><div><b>Navigation</b>${item.has_navigation?'Available':'Not declared'}</div><div><b>Language</b>${esc(item.language||'Not listed')}</div></div><div class=detailpath>${esc(item.rel_path)}</div><div class=reviewnote>Cataloged safely. Chapter reading, search, citations, progress, and read-aloud arrive in V2B.2.</div></div></div>`;
  }else{
