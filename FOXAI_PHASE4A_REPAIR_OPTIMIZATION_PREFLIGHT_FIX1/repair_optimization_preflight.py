...
def host_python_comfy(root: Path):
    ...
    # The proven ComfyUI launcher starts ordinary host Python without -s.
    # The preflight controller itself disables user-site packages, so that
    # controller-only setting must not leak into this host-backend probe.
    env.pop("PYTHONNOUSERSITE", None)

    result["probe_contract"] = {
        "matches_working_launcher": True,
        "python_s_flag_used": False,
        "pythonhome_cleared": True,
        "pythonpath_cleared": True,
        "controller_user_site_block_removed": True,
...
def main():
    ...
    receipt = {
        "action": "foxai_phase4a_repair_optimization_read_only_preflight_fix1",
