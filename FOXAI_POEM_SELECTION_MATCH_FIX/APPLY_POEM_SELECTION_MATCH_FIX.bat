...
title FOXAI Poem Selection Match Fix

for %%I in ("%~dp0..") do set "FOXAI_ROOT=%%~fI"
set "PAYLOAD=%~dp0payload"
set "PYTHON=%FOXAI_ROOT%\Runtime\Desktop\python\python.exe"
set "VERIFY=%~dp0verify_selection_match_fix.py"

echo ============================================================
echo FOXAI POEM SELECTION MATCH FIX
echo ============================================================
echo Changes only:
echo   core\foxai_web.py
echo.
echo Fixes the false:
echo   captured passage no longer matches the poem
echo error caused by whitespace being stripped before checking
echo browser character positions.
echo.
echo My Poems, Eric Voice, copyright fields, and the full workshop
echo remain included.
echo.
...
set "BACKUP=%FOXAI_ROOT%\Backups\PoemSelectionMatchFix\%STAMP%"
...
mkdir "%FOXAI_ROOT%\Backups\PoemSelectionMatchFix" 2>nul
> "%FOXAI_ROOT%\Backups\PoemSelectionMatchFix\LATEST.txt" echo %BACKUP%

echo.
echo ============================================================
echo POEM SELECTION MATCH FIX APPLIED AND VERIFIED
echo ============================================================
echo Backup:
echo   %BACKUP%
echo.
echo Restart WebUI and repeat the same selection test.
echo.
pause
exit /b 0
...
:verify_failed
echo ERROR: Selection fix verification failed. Restoring backup...
goto :rollback
