...
title Roll Back FOXAI Poem Selection Match Fix

for %%I in ("%~dp0..") do set "FOXAI_ROOT=%%~fI"
set "LATEST=%FOXAI_ROOT%\Backups\PoemSelectionMatchFix\LATEST.txt"

if not exist "%LATEST%" (
    echo No recorded selection-fix backup was found.
    pause
    exit /b 2
)
