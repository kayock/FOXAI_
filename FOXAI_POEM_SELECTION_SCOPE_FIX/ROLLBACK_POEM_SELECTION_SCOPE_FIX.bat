...
title Roll Back FOXAI Poem Selection Scope Fix

for %%I in ("%~dp0..") do set "FOXAI_ROOT=%%~fI"
set "LATEST=%FOXAI_ROOT%\Backups\PoemSelectionScopeFix\LATEST.txt"

if not exist "%LATEST%" (
    echo No recorded scope-fix backup was found.
    pause
    exit /b 2
)
