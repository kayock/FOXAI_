...
SCHEMA = "foxai.repair_bay.guarded_handoff.v2"
...
SUPPORTED_FINDINGS: dict[str, dict[str, Any]] = {
    ...
    "root_launcher_inventory": {
        "repair_kind": "prepare_reversible_launcher_archive_plan",
        "label": "Prepare a reversible launcher archive plan",
        "backup_required": True,
        "path_required": False,
    },
}
...
def _finding_paths(root: Path, report: dict[str, Any], finding: dict[str, Any]) -> list[str]:
    ...
    if finding_id == "root_launcher_inventory":
        ...
        for item in inventory.get("obsolete_looking_candidates") or []:
            ...
            if not name or name.casefold() in protected_names:
                continue
...
def _command_text(packet: dict[str, Any]) -> str:
    ...
    if packet.get("eligible"):
        path_text = ", ".join(paths) if paths else "No path may be changed until Engineer resolves it from verified evidence."
        evidence_text = " | ".join(evidence[:6]) or "No additional evidence supplied."
        return (
            f"/engineer workshop begin Repair Bay Plan — {_safe_title(finding.get('title'))} :: "
            "Prepare a reviewable exact repair plan only. This authorizes read-only inspection and plan preparation, "
            "not implementation. Do not apply, move, rename, delete, install, restart, run repair commands, or use the network. "
            f"Finding ID: {_clean(finding.get('id'), 80)}. Severity: {_clean(finding.get('severity'), 30)}. "
            f"Plain explanation: {_clean(packet.get('plain_english_explanation'), 600)}. "
            f"Proposed action: {_clean(packet.get('proposed_action'), 600)}. "
            f"Affected FOXAI paths: {_clean(path_text, 1200)}. "
            f"Technical evidence: {_clean(evidence_text, 1400)}. "
            "The plan must use exact before hashes, a targeted snapshot, explicit later APPLY approval, validation, rollback protection, "
            "and a final receipt. Unsupported or uncertain paths must remain advisory."
        )
    return (
        f"/engineer Analyze this Repair Bay finding read-only and explain safe options. Do not prepare or apply a repair unless the "
        f"finding becomes clearly bounded by verified evidence. Finding ID: {_clean(finding.get('id'), 80)}. "
        f"Title: {_safe_title(finding.get('title'))}. Severity: {_clean(finding.get('severity'), 30)}. "
        f"Explanation: {_clean(packet.get('plain_english_explanation'), 700)}. "
        f"Evidence: {_clean(' | '.join(packet.get('technical_evidence') or []) or 'No additional evidence.', 1500)}"
    )
