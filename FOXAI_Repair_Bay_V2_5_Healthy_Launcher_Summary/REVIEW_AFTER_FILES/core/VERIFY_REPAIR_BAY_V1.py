...
def main() -> int:
    ...
    for marker in (
        ...
        "preparedRepairMission",
        "storePreparedRepairMission",
        "syncPreparedRepairMissionUI",
        "repairHandoffCalmOpenButton",
    ...
    assert 'id=repairHandoffCalmActions' in repair_section
    assert 'id=repairHandoffCalmOpenButton' in repair_section
    assert 'onclick="cancelPreparedRepairHandoff()"' in repair_section
    ...
    assert "preparedRepairMissionIsValid" in handoff_function
    assert r"Problem:\s*$" in web_text

    transfer_block = web_text.split("/* REPAIR_BAY_MISSION_TRANSFER_V2_2_BROWSER_START */", 1)[1].split("/* REPAIR_BAY_MISSION_TRANSFER_V2_2_BROWSER_END */", 1)[0]
    assert "preparedRepairMission" in transfer_block
    assert "storePreparedRepairMission(packet)" in transfer_block
    assert "success&&openConsole" in transfer_block
    assert transfer_block.index("repairHandoffBusy=false") < transfer_block.index("if(success&&openConsole)openRepairHandoffInMission()")
    assert "Already prepared — Open Mission Console" in transfer_block
    ...
    result = {
        ...
        "prepared_handoff_lifecycle_verified": True,
