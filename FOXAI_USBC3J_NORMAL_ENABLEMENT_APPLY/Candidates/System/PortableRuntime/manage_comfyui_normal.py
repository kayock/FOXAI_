#!/usr/bin/env python3
"""FOXAI ComfyUI Safe Normal CPU lifecycle controller.

Installed by USB C3J. C3J itself never executes this controller. The later
controlled lifecycle test gate exercises start/status/stop behavior.

The controller is deliberately conservative:
- portable Python and the isolated ComfyUI target only;
- CPU mode, custom nodes disabled, localhost port 8188;
- controller-owned state with PID, creation time, executable, command and port
  ownership checks;
- no automatic force-kill and no action against an unknown process;
- no automatic log deletion.
"""
...
def command_fingerprint(root: Path, command: list[str]) -> str:
    payload = {
        "policy_id": POLICY_ID,
...
def expected_child_command(root: Path, paths: dict[str, Path]) -> list[str]:
    return [
        str(paths["python"]),
        "-I", "-B", "-S",
        str(paths["activator"]),
...
def verify_owned_state(root: Path, paths: dict[str, Path], psutil, state: dict[str, Any]) -> dict[str, Any]:
    command = expected_child_command(root, paths)
    expected_fingerprint = command_fingerprint(root, command)
    ...
    return {
        ...
        "owned_healthy": child_owned and supervisor_owned and loopback_only and health.get("ok") is True,
    }
...
def status_result(root: Path, paths: dict[str, Path], psutil) -> dict[str, Any]:
    ...
    if verification["owned_healthy"]:
        ...
        if verification["child_owned"] or verification["supervisor_owned"]:
            ...
            message = "A partially matching controller instance exists but all ownership and health checks do not agree."
        else:
            effective = "CONFLICT"
...
def run_supervisor(root: Path, paths: dict[str, Path], psutil, open_browser: bool, source: str) -> int:
    current = status_result(root, paths, psutil)
    if current["state"] == "HEALTHY":
        print(json.dumps({"ok": True, "state": "HEALTHY", "message": "ComfyUI is already running."}))
        return 0
    if current["state"] in {"CONFLICT", "STARTING", "STOPPING", "STALE"}:
    ...
    with stdout_path.open("ab", buffering=0) as stdout_handle, stderr_path.open("ab", buffering=0) as stderr_handle:
        ...
        state = {
            ...
            "command": command,
            "command_fingerprint": command_fingerprint(root, command),
            "listen": "127.0.0.1",
            "port": 8188,
        ...
        while time.monotonic() < deadline:
            ...
            health_rows.append({"observed": utc_now(), "owned_healthy": snapshot["owned_healthy"], "health": snapshot["health"]})
        ...
        state.update({"status": "HEALTHY", "healthy": utc_now()})
        atomic_json(paths["state"], state)
...
def spawn_supervisor(root: Path, paths: dict[str, Path], psutil, source: str) -> dict[str, Any]:
    current = status_result(root, paths, psutil)
    if current["state"] == "HEALTHY":
        return {"ok": True, "state": "HEALTHY", "message": "ComfyUI is already running.", "status": current}
    if current["state"] in {"CONFLICT", "STARTING", "STOPPING", "STALE"}:
        return {"ok": False, "state": current["state"], "message": current["message"], "status": current}
    command = [
        str(paths["python"]), "-I", "-B", "-S", str(paths["manager"]),
        "--root", str(root), "start", "--no-browser", "--source", source,
    ]
    creationflags = 0
    if os.name == "nt":
    ...
    while time.monotonic() < deadline:
        ...
        if last["state"] == "HEALTHY":
            return {
                "ok": True,
                "state": "HEALTHY",
                "message": "ComfyUI started and passed controller ownership and health checks.",
...
def main() -> int:
    ...
    try:
        ...
        if args.action == "spawn":
            result = spawn_supervisor(root, paths, psutil, args.source)
            emit(result, args.json)
            return 0 if result.get("ok") else 12
        return run_supervisor(root, paths, psutil, not args.no_browser, args.source)
