#!/usr/bin/env python3
"""USB C4B controlled allowlisted custom-node import, health verification, and stop.

Stdlib only. This is the first approved live-start gate. It binds to the exact
accepted C4A Airlock evidence and sealed C3 baseline, starts ComfyUI through a
dedicated audited launcher on localhost CPU mode with every custom node disabled
except one exact hash-locked whitelist entry, verifies registration, health,
process/network/audit state, then stops it before returning.
"""
...
SUCCESS_CLASSIFICATION = "C4B_ALLOWLISTED_NODE_VERIFIED_STOPPED_READY_FOR_C4C_WEBUI_PROFILE_REVIEW"
BLOCKED_CLASSIFICATION = "C4B_BLOCKED_FAIL_CLOSED"
    ...
    for record in manifest.get("files", []):
        ...
        rows.append(
            verify_exact_file(
                ...
                record["sha256"],
                f"C4B package file {record['path']}",
            )
        )
...
def verify_c4a_evidence(root: Path, package_root: Path, output: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    binding = load_json(package_root / "EXPECTED_C4A_BINDING.json")
    base = (root / Path(binding["c4a_relative_output"])).resolve(strict=True)
    evidence_path = base / "evidence_integrity.json"
    if sha256_file(evidence_path) != binding["evidence_integrity_sha256"]:
        raise GateError("Exact C4A evidence-integrity manifest hash changed", 21)
    evidence = load_json(evidence_path)
    verified_rows = []
    for row in evidence.get("files", []):
        path = base / Path(row["file"])
        verified_rows.append(
            verify_exact_file(path, int(row["size_bytes"]), row["sha256"], f"C4A evidence {row['file']}")
        )
    exact_files = (
        ("classification.json", "classification_sha256"),
        ("receipt.json", "receipt_sha256"),
        ("custom_node_airlock_inventory.json", "inventory_sha256"),
        ("comfyui_custom_node_controls.json", "controls_sha256"),
        ("PROPOSED_NEXT_GATE/APPROVED_NODES_WEBUI_CONTRACT.json", "profile_contract_sha256"),
        ("baseline_verification.json", "baseline_verification_sha256"),
    )
    for name, key in exact_files:
        if sha256_file(base / Path(name)) != binding[key]:
            raise GateError(f"Exact C4A {name} hash changed", 21)
    ...
    if not classification.get("verified") or classification.get("mode") != binding["expected_classification"]:
        raise GateError("C4A success classification is not exact", 21)
    receipt = load_json(base / "receipt.json")
    if not receipt.get("verified") or receipt.get("launch_performed") or receipt.get("live_change"):
        raise GateError("C4A receipt does not preserve the reviewed no-launch/no-change state", 21)
    inventory = load_json(base / "custom_node_airlock_inventory.json")
    node = binding["node"]
    active = [row for row in inventory.get("files", []) if row.get("kind") == "active_python"]
    if len(active) != 1:
        raise GateError("C4A did not record exactly one active Python custom node", 21)
    row = active[0]
    if row.get("path") != node["filename"] or int(row.get("size_bytes", -1)) != int(node["size_bytes"]) or row.get("sha256") != node["sha256"]:
        raise GateError("C4A approved-node identity is not exact", 21)
    controls = load_json(base / "comfyui_custom_node_controls.json")
    if not controls.get("verified") or not controls.get("controls", {}).get("disable_all_custom_nodes") or not controls.get("controls", {}).get("whitelist_custom_nodes"):
        raise GateError("C4A did not verify the required native whitelist controls", 21)
    profile = load_json(base / "PROPOSED_NEXT_GATE/APPROVED_NODES_WEBUI_CONTRACT.json")
    candidates = profile.get("candidate_nodes", [])
    if len(candidates) != 1 or candidates[0].get("path") != node["filename"] or candidates[0].get("sha256") != node["sha256"]:
        raise GateError("C4A profile contract does not bind the exact approved node", 21)
    baseline_dir = (root / Path(binding["baseline"]["relative_path"])).resolve(strict=True)
    for record in binding["baseline"]["files"]:
        verify_exact_file(baseline_dir / Path(record["path"]), int(record["size_bytes"]), record["sha256"], f"sealed baseline {record['path']}")
    result = {
        "verified": True,
        "base": str(base),
        ...
        "approved_node": node,
        "native_whitelist_verified": True,
    }
    write_json(output / "c4a_input_verification.json", result)
...
def http_probe(url: str, timeout: float = 2.0) -> dict[str, Any]:
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "FOXAI-C4B/1.0"})
...
def create_review_zip(output: Path) -> Path:
    review = output / "UPLOAD_THIS_C4B_REVIEW.zip"
    if review.exists():
        review.unlink()
...
def write_evidence_integrity(output: Path) -> dict[str, Any]:
    rows = []
    for path in sorted(output.rglob("*")):
        if not path.is_file() or path.name in {"evidence_integrity.json", "UPLOAD_THIS_C4B_REVIEW.zip"}:
            continue
        rows.append(
            {
...
def verify_approved_node(root: Path, binding: dict[str, Any], output: Path, phase: str) -> dict[str, Any]:
    node = binding["node"]
    result = verify_exact_file(
        root / Path(node["relative_path"]),
        int(node["size_bytes"]),
        node["sha256"],
        "approved custom node",
    )
    result.update({
        "phase": phase,
        "filename": node["filename"],
        "class_key": node["class_key"],
        "display_name": node["display_name"],
        "approved": True,
    })
    write_json(output / f"approved_node_{phase}.json", result)
    return result


def node_registration_from_probe(probe: dict[str, Any], binding: dict[str, Any]) -> dict[str, Any]:
    node = binding["node"]
    data = probe.get("json")
    entry = None
    if isinstance(data, dict):
        if node["class_key"] in data and isinstance(data[node["class_key"]], dict):
            entry = data[node["class_key"]]
        elif data.get("name") == node["class_key"]:
            entry = data
    display = entry.get("display_name") if isinstance(entry, dict) else None
    verified = bool(probe.get("ok") and entry is not None)
    if display not in (None, "", node["display_name"]):
        verified = False
    return {
        "verified": verified,
        "class_key": node["class_key"],
        "expected_display_name": node["display_name"],
        "reported_display_name": display,
        "probe": probe,
        "entry": entry,
    }


def _audit_host(value: Any) -> str:
    if isinstance(value, (tuple, list)) and value:
        return str(value[0]).strip("[]").casefold()
    return str(value).strip("[]").casefold()


def _audit_port(value: Any) -> int | None:
...
def parse_audit_evidence(path: Path, binding: dict[str, Any]) -> dict[str, Any]:
    if not path.is_file() or path.is_symlink():
        raise GateError("C4B audit log is missing or unsafe", 24)
    events = []
    parse_errors = []
    ...
    expected_tail = binding["node"]["relative_path"].replace("\\", "/").casefold()
    node_exec = []
    for row in events:
        if row.get("decision") != "observed_custom_node_execution":
            continue
        event_args = row.get("args")
        if isinstance(event_args, dict):
            filename = str(event_args.get("filename", ""))
        else:
            filename = str(event_args or "")
        normalized_filename = filename.replace("\\", "/").casefold()
        if normalized_filename.endswith(expected_tail):
            node_exec.append(row)
    binds = [row for row in events if row.get("event") == "socket.bind"]
    listener_binds = []
    self_pipe_binds = []
    invalid_binds = []
    for row in binds:
        address = row.get("args")
        host, port = _audit_host(address), _audit_port(address)
        if host in {"127.0.0.1", "localhost"} and port == 8188:
            listener_binds.append(row)
        elif host in {"127.0.0.1", "localhost", "::1"} and port == 0:
            self_pipe_binds.append(row)
        else:
            invalid_binds.append(row)
    subprocess_events = [row for row in events if row.get("event") in {"subprocess.Popen", "os.system", "os.startfile", "os.spawn"}]
    result = {
        "verified": (
            not parse_errors
            and not denied
            and bool(node_exec)
            and bool(listener_binds)
            and not invalid_binds
            and not subprocess_events
        ),
        "event_count": len(events),
        "parse_errors": parse_errors,
        "denied_events": denied,
        "approved_node_execution_events": node_exec,
        "socket_bind_events": binds,
        "contract_listener_bind_events": listener_binds,
        "event_loop_self_pipe_bind_events": self_pipe_binds,
        "invalid_socket_bind_events": invalid_binds,
        "process_launch_events": subprocess_events,
        "events": events,
    }
    if not result["verified"]:
        raise GateError("C4B audit evidence did not prove an isolated allowlisted node import", 24)
    return result
...
def main() -> int:
    ...
    output = package_root / "TEST_OUTPUT" / run_id()
    ...
    try:
        print(f"[C4B] Evidence folder: {output}", flush=True)
        ...
        if not approval.get("approved") or approval.get("leave_running"):
            raise GateError("C4B operator approval is absent or requests leaving the process running", 21)
        write_json(output / "operator_approval.json", approval)
        binding, _ = verify_c4a_evidence(root, package_root, output)
        verify_live_files(root, binding, output, "before")
        verify_target(root, binding, output, "before")
        verify_approved_node(root, binding, output, "before")
        ...
        if port_open():
            raise GateError("Port 127.0.0.1:8188 is already in use; C4B did not launch", 22)
        ...
        wrapper = (package_root / "System" / "NodeTest" / "c4b_audited_allowlisted_launcher.py").resolve(strict=True)
        audit_path = output / "child_audit.jsonl"
        command = [
            str(portable), "-I", "-B", "-S", str(wrapper),
            "--root", str(root), "--audit", str(audit_path), "--", *launch["arguments"],
        ]
        env = os.environ.copy()
        ...
        print("[C4B] Starting ComfyUI with exactly one hash-locked allowlisted node...", flush=True)
        ...
        write_json(
            output / "launch_receipt.json",
            {
                ...
                "whitelist_custom_nodes": [binding["node"]["filename"]],
                "approved_node_sha256": binding["node"]["sha256"],
                "listen": "127.0.0.1",
                "port": 8188,
        ...
        while time.monotonic() < deadline:
            ...
            if root_probe.get("ok"):
                ...
                node_probe = http_probe(launch["node_info_url"])
                registration = node_registration_from_probe(node_probe, binding)
                if registration["verified"]:
                    health_successes.append(
                        {"utc": utc_now(), "root": root_probe, "system_stats": stats_probe, "node_registration": registration}
                    )
        ...
        final_registration = health["checks"][-1]["node_registration"]
        write_json(output / "approved_node_registration.json", final_registration)
        if not final_registration.get("verified"):
            raise GateError("Approved custom node did not register exactly", 24)
        write_json(
        ...
        print("[C4B] Health gate passed. Stopping ComfyUI now...", flush=True)
        ...
        audit_result = parse_audit_evidence(audit_path, binding)
        write_json(output / "child_audit_summary.json", audit_result)
        ...
        verify_approved_node(root, binding, output, "after")
        ...
        classification = {
            ...
            "disable_all_custom_nodes": True,
            "whitelisted_custom_nodes": [binding["node"]["filename"]],
            "approved_node_registered": True,
            "approved_node_audit_verified": True,
            "external_network_observed": False,
            "launcher_change": False,
            "runtime_target_change": False,
            "next_gate": "C4C WebUI approved-node profile integration review requires fresh approval",
        }
        ...
        if output.exists():
            ...
            receipt = {
                "action": "foxai_usbc4b_allowlisted_node_lifecycle_test",
            ...
            report = [
                "# FOXAI USB C4B — Allowlisted Custom-Node Lifecycle Test",
                "",
                f"- Classification: `{classification['mode']}`",
                f"- Verified: `{exit_code == 0}`",
                f"- ComfyUI started: `{classification.get('comfyui_started')}`",
                f"- Approved node registered: `{classification.get('approved_node_registered')}`",
                f"- Approved node audit verified: `{classification.get('approved_node_audit_verified')}`",
            ...
            print(f"[C4B] Review package: {review}", flush=True)
    return exit_code
