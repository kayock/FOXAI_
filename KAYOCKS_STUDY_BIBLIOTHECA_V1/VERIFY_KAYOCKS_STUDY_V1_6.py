...
def main() -> int:
    ...
    degree_samples=[
        ...
        "Bake at 375° for 20 minutes, then turn heat to 350° and bake 25 minutes longer.",
        "Bake at 375 for 20 minutes, then turn heat to 350 and bake 25 minutes longer.",
    ]
    check(
        "white_bread_degree_symbol_numeric_matching",
        all(
            re.search(
                r"(?<!\d)375(?:\s*[°º])?(?!\d).*?(?<!\d)20(?!\d)",
                sample,
            )
            and re.search(
                r"(?<!\d)350(?:\s*[°º])?(?!\d).*?(?<!\d)25(?!\d)",
                sample,
            )
    ...
    try:
        ...
        first_stage=bool(re.search(
            r"(?<!\d)375(?:\s*[°º])?(?!\d).*?(?<!\d)20(?!\d)",
            normalized,
        ))
        second_stage=bool(re.search(
            r"(?<!\d)350(?:\s*[°º])?(?!\d).*?(?<!\d)25(?!\d)",
            normalized,
        ))
        check(
