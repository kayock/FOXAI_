...
_QUESTION_STOP_WORDS = {
    ...
    "degrees", "degree", "instructions", "long",
}
...
def requested_subject(question: str) -> str:
    cleaned = _PAGE_PATTERN.sub(" ", str(question or ""))
    words = [
        token.casefold()
...
def recipe_title_intent(question: str) -> bool:
    ...
    return bool(tokens & title_cues) and not bool(tokens & ingredient_cues)
...
def public_source(item: dict) -> dict:
    keys = (
        "document_id", "title", "rel_path", "shelf", "page_number",
        "snippet", "citation", "text_status", "is_ocr_copy",
        "detected_heading", "match_role",
    )
...
def resolve_question_sources(
    ...
    if resolved_page and document_id:
        ...
        if effective_shelf.casefold() == "recipes" and subject:
            sources = recipe_heading_sources(
                ...
                subject,
                document_id=document_id,
                shelf=effective_shelf,
                status=status,
                limit=MAX_ASK_SOURCES,
            )
            if sources:
                mode = "recipe_heading"
                ...
                warning = (
                    f"No recipe heading matching “{subject}” was found in the selected "
                    "scope. Ingredient-only occurrences were deliberately withheld."
                )
            else:
                sources = search_pages(
                    paths,
                    query,
                    document_id=document_id,
                    shelf=effective_shelf,
        ...
        if resolved_page and not document_id:
            warning = (
                f"Page {resolved_page} was named, but no single document was selected; "
                "the Bibliotheca used normal cited search instead."
            )

    annotated = []
    for source in sources:
        item = dict(source)
        if item.get("shelf", "").casefold() == "recipes":
            analysis = recipe_page_analysis(item.get("text", ""), subject)
    ...
    if recipe_sources and subject:
        title_matches = [
            item for item in recipe_sources
            if item.get("match_role") in ("title_exact", "title_related")
        ]
        if title_matches and mode != "exact_page":
            annotated = title_matches + [
                item for item in annotated
                if item not in recipe_sources
            ]
        ...
        if len(headings) > 1:
            warning = (
                "Multiple recipe headings match this question: "
                + ", ".join(headings[:6])
                + ". Agent Fox must keep their instructions separate."
            )
        elif mode == "exact_page" and recipe_sources:
            item = recipe_sources[0]
            if item.get("match_role") == "ingredient_only":
                warning = (
                    f"The phrase “{subject}” appears as ingredient wording on this page, "
                    f"not as the detected recipe title “{item.get('detected_heading') or 'unknown'}”."
                )
        elif not title_matches and any(
            item.get("match_role") == "ingredient_only"
            for item in recipe_sources
        ):
            headings = [
                item.get("detected_heading") for item in recipe_sources
                if item.get("detected_heading")
            ]
            warning = (
                f"The words “{subject}” were found in ingredient text, not as a recipe title."
                + (f" Nearby heading: {headings[0]}." if headings else "")
            )
    ...
    if not sources:
        ...
        return {
            ...
            "message": (
                ...
                or (
                    "The Bibliotheca found no matching indexed pages."
                    + page_note
                    + " Try different wording or index an OCR/searchable copy."
                )
            ),
            "sources": [],
...
<title>Kayock's Study — The Bibliotheca V1.5</title>
<style>
:root{
...
.duplicates{grid-column:span 12}
...
.groundingnote{border:1px solid #36dbff42;background:#36dbff09;border-radius:14px;padding:10px;margin:9px 0}.groundingwarn{border-color:#ffd16666;background:#ffd1660c}.askgrid{grid-template-columns:repeat(4,1fr)}.checkline{display:flex;gap:9px;align-items:center;color:var(--muted);font-size:13px;margin:8px 0}.checkline input{width:auto}
...
<div class="shell">
  <section class="hero">
    <div class="eyebrow">Kayock's Study · Bibliotheca V1.3</div>
...
async function refreshAll(){
  ...
  await Promise.all([refreshStatus(),refreshDocuments(),refreshDuplicates()]);
}
...
function renderSources(items,target='resultList'){
  q(target).innerHTML=items.length?items.map(item=>`
    <div class=result>
      <div class=titleline>
        <b>${esc(item.title)}</b>
        <span class="${statusClass(item.text_status)} small">${esc(statusLabel(item.text_status))}</span>
      </div>
      <div><span class=pill>${esc(item.shelf||'')}</span>${item.detected_heading?`<span class="pill ok">Heading: ${esc(item.detected_heading)}</span>`:''}</div>
      <div class=path>${esc(item.rel_path)}</div>
      <div class=snippet>${esc(item.snippet)}</div>
      <div class=citation>${esc(item.citation)}</div>
      <div class=controls>
        <button class=secondary onclick="openPdf(${item.document_id},${item.page_number})">Open page ${item.page_number}</button>
        <button class=secondary onclick="askFromPage(${item.document_id},${item.page_number})">Ask from this page</button>
        <button class=secondary onclick="navigator.clipboard.writeText(${JSON.stringify(item.citation)})">Copy citation</button>
      </div>
    </div>`).join(''):'<div class=empty>No matching indexed pages were found.</div>';
}
...
async function askFox(){
  ...
  if(!question)return;
  const sourceRefs=q('askUseResults').checked
    ? lastSearchResults.slice(0,12).map(item=>({document_id:item.document_id,page_number:item.page_number}))
    : [];
  const exactPage=q('askPage').value?Number(q('askPage').value):null;
  ...
  try{
    const data=await api('/api/ask',{
      ...
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({
        question,document_id:q('askDoc').value||null,
    ...
    const mode=`<div class=groundingnote><b>Source selection:</b> ${esc(data.selection_mode||'search')}${data.exact_page?` · exact page ${esc(data.exact_page)}`:''}</div>`;
...
refreshAll().catch(error=>{q('progressText').textContent=error.message;});
...
class StudyHandler(BaseHTTPRequestHandler):
    server_version = "KayocksStudy/1.1"
...
def main() -> int:
    ...
    print("=" * 72)
    print("KAYOCK'S STUDY — THE BIBLIOTHECA V1.1")
    print("=" * 72)
    print("URL:", url)
