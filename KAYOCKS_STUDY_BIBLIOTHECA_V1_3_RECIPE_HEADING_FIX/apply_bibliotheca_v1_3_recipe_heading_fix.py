...
SOURCE_SHA256 = "2fa624d5666af570edc57e0a72687757bdf01f6b3a341b2a9a3214d14d00fc62"
PATCHED_SHA256 = "cb2f5701d2fc0d6dc4cdbbc3674e3e4d730b92821528b5f7ce1fe3bbef407032"
PATCH_ID = "foxai.kayocks_study.bibliotheca.v1.3.recipe_heading_fix"
...
REQUIRED_MARKERS = (
    ...
    'mode = "recipe_heading"',
    '"take", "takes"',
    'When source selection is recipe_heading',
)
...
def compile_file(path: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="bibliotheca_v1_3_") as temp:
        py_compile.compile(
            str(path),
...
def apply(root: Path, package: Path) -> int:
    ...
    if not payload.is_file():
        print("ERROR: V1.3 payload is missing:", payload)
        return 4

    if sha256_file(payload) != PATCHED_SHA256:
        print("ERROR: V1.3 payload verification failed.")
        return 5
    ...
    if missing:
        print("ERROR: Required V1.3 markers are missing:", ", ".join(missing))
        return 6
    compile_file(payload)

    live_hash = sha256_file(target)
    if live_hash == PATCHED_SHA256:
        print("Bibliotheca V1.3 is already installed.")
        print("Database:", database_counts(database))
        return 0
    if live_hash != SOURCE_SHA256:
        print("ERROR: Live Study server does not match reviewed V1.2.")
    ...
    server_backup = backup_dir / "study_server_v1_2.py"
    shutil.copy2(target, server_backup)
    if sha256_file(server_backup) != SOURCE_SHA256:
        print("ERROR: V1.2 server backup verification failed.")
        return 8

    database_snapshot = snapshot_database(
        database,
        backup_dir / "bibliotheca_before_v1_3.sqlite3",
    )

    temporary = target.with_name("study_server.py.v1_3.tmp")
    shutil.copy2(payload, temporary)

    try:
        compile_file(temporary)
        if sha256_file(temporary) != PATCHED_SHA256:
            raise RuntimeError("Temporary V1.3 verification failed.")
        ...
        print("ERROR:", exc)
        print("The verified V1.2 server was restored.")
        return 9

    (app_dir / "README_V1_3.txt").write_text(
    ...
    receipt = write_receipt(
        ...
        "apply",
        {
            ...
            "fix": (
                "Recipe retrieval now searches heading candidates before "
                "ordinary ingredient-text ranking."
            ),
        },
    )
    ...
    print()
    print("=" * 72)
    print("KAYOCK'S STUDY — BIBLIOTHECA V1.3 INSTALLED")
    print("=" * 72)
    print("Server:", target)
...
def rollback(root: Path) -> int:
    ...
    backup_base = root / "Backups" / "KayocksStudy" / "BibliothecaV1_3"
    ...
    live_hash = sha256_file(target)
    if live_hash == SOURCE_SHA256:
        print("Bibliotheca V1.2 is already active.")
        return 0
    if live_hash != PATCHED_SHA256:
        print("ERROR: Study server changed after V1.3.")
        print("Rollback stopped to protect newer work.")
        return 21

    backups = sorted(
        (
            item for item in backup_base.glob("*/study_server_v1_2.py")
    ...
    if not backups:
        print("ERROR: No verified V1.2 server backup was found.")
        return 22
    ...
    preserve_dir = (
        ...
        / "Backups"
        / "KayocksStudy"
        / "BibliothecaV1_3Rollback"
        / datetime.now().strftime("%Y%m%dT%H%M%S")
    )
    preserve_dir.mkdir(parents=True, exist_ok=False)
    v13_copy = preserve_dir / "study_server_v1_3.py"
    shutil.copy2(target, v13_copy)
    ...
    try:
        ...
        temporary.unlink(missing_ok=True)
        shutil.copy2(v13_copy, target)
        print("ERROR:", exc)
        print("The V1.3 server was restored.")
        return 23
    ...
    receipt = write_receipt(
        ...
        "rollback",
        {
            ...
            "v1_3_preserved": str(v13_copy),
            "database_changed": False,
        },
    )
    print("Bibliotheca V1.3 was rolled back to V1.2.")
