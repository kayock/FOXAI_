...
SOURCE_SHA256 = "dc63296cbadf376be440140978e225af8b3a6adfa6b860b512e4d7ee9d1afdf8"
PATCHED_SHA256 = "7f67d7294a0feaf396fadc24ff82699b4b84313130bf63b35825451e66c1cd62"
PATCH_ID = "foxai.kayocks_study.bibliotheca.v1.5.live_model_citations"

REQUIRED_MARKERS = (
    'APP_VERSION = "1.5"',
    'def normalize_answer_citations(',
    'Allowed citations — copy these labels exactly:',
    '"citation_normalized_count"',
    '"/api/model/status"',
...
def compile_file(path: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="bibliotheca_v1_5_") as temp:
        py_compile.compile(
            str(path),
...
def apply(root: Path, package: Path) -> int:
    ...
    if not payload.is_file():
        print("ERROR: V1.5 payload is missing:", payload)
        return 4
    if sha256_file(payload) != PATCHED_SHA256:
        print("ERROR: V1.5 payload verification failed.")
        return 5
    ...
    if missing:
        print("ERROR: Required V1.5 markers are missing:", ", ".join(missing))
        return 6
    compile_file(payload)

    live_hash = sha256_file(target)
    if live_hash == PATCHED_SHA256:
        print("Bibliotheca V1.5 is already installed.")
        print("Database:", database_counts(database))
        return 0
    if live_hash != SOURCE_SHA256:
        print("ERROR: Live Study server does not match reviewed V1.4.")
    ...
    server_backup = backup_dir / "study_server_v1_4.py"
    shutil.copy2(target, server_backup)
    if sha256_file(server_backup) != SOURCE_SHA256:
        print("ERROR: V1.4 server backup verification failed.")
        return 8

    database_snapshot = snapshot_database(
        database,
        backup_dir / "bibliotheca_before_v1_5.sqlite3",
    )

    temporary = target.with_name("study_server.py.v1_5.tmp")
    shutil.copy2(payload, temporary)

    try:
        compile_file(temporary)
        if sha256_file(temporary) != PATCHED_SHA256:
            raise RuntimeError("Temporary V1.5 verification failed.")
        ...
        print("ERROR:", exc)
        print("The verified V1.4 server was restored.")
        return 9

    (app_dir / "README_V1_5.txt").write_text(
    ...
    receipt = write_receipt(
        ...
        "apply",
        {
            ...
            "features": [
                "automatic local-model polling every 2.5 seconds",
                "immediate model check when browser tab becomes visible",
                "exact allowed citation labels in the model prompt",
                "safe normalization of unambiguous citation placeholders",
                "visible warning for unresolved citation placeholders",
            ],
        },
    )
    ...
    print()
    print("=" * 72)
    print("KAYOCK'S STUDY — BIBLIOTHECA V1.5 INSTALLED")
    print("=" * 72)
    print("Server:", target)
    ...
    print()
    print("Restart START_KAYOCKS_STUDY.bat. No reindex is needed.")
    return 0
...
def rollback(root: Path) -> int:
    ...
    backup_base = (
        root / "Backups" / "KayocksStudy" / "BibliothecaV1_5"
    )
    ...
    live_hash = sha256_file(target)
    if live_hash == SOURCE_SHA256:
        print("Bibliotheca V1.4 is already active.")
        return 0
    if live_hash != PATCHED_SHA256:
        print("ERROR: Study server changed after V1.5.")
        print("Rollback stopped to protect newer work.")
        return 21

    backups = sorted(
        (
            item for item in backup_base.glob("*/study_server_v1_4.py")
    ...
    if not backups:
        print("ERROR: No verified V1.4 server backup was found.")
        return 22
    ...
    preserve_dir = (
        ...
        / "Backups"
        / "KayocksStudy"
        / "BibliothecaV1_5Rollback"
        / datetime.now().strftime("%Y%m%dT%H%M%S")
    )
    preserve_dir.mkdir(parents=True, exist_ok=False)
    v15_copy = preserve_dir / "study_server_v1_5.py"
    shutil.copy2(target, v15_copy)
    ...
    try:
        ...
        temporary.unlink(missing_ok=True)
        shutil.copy2(v15_copy, target)
        print("ERROR:", exc)
        print("The V1.5 server was restored.")
        return 23
    ...
    receipt = write_receipt(
        ...
        "rollback",
        {
            ...
            "v1_5_preserved": str(v15_copy),
            "database_changed": False,
        },
    )
    print("Bibliotheca V1.5 was rolled back to V1.4.")
