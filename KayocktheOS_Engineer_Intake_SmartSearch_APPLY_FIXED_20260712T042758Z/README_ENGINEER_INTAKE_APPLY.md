...
## Corrected live functional verifier

The first apply attempt passed the important behavior:

- Query normalized to `COMFY_MAIN`
- Scope was `Executable/source evidence`
- `core/foxai_web.py` was found
- all 8 targeted tests passed
- all 15 Phase 1 tests passed

It rolled back because the verifier searched the entire rendered report for the
literal text `.venv/`. The apply bundle's own test source contained that literal,
so SmartSearch found the test file and the verifier mistook its snippet for an
actual vendor-path leak.

This corrected verifier checks returned evidence file paths structurally. It
rejects actual paths beginning with `.venv/` or containing `/site-packages/`,
while allowing snippets or test source to mention those strings.

The reviewed `core\engineer_agent.py` candidate is unchanged.
