@echo off
setlocal
title KayocktheOS Portable Python Compatibility Apply
cd /d "%~dp0"
...
"%PYTHON%" -u "%~dp0tools\apply_portable_fix.py"
set "RC=%ERRORLEVEL%"
echo.
if "%RC%"=="0" (
    echo Portable Python compatibility fix verified.
) else (
    echo The fix was not applied, was cancelled, or was rolled back.
)
pause
exit /b %RC%
