...
class LlamaServer:
    ...
    def ensure_running(
        ...
        target = self._target(model, host, port, context, threads)
                    ...
                    if state_active:
                        if not self._same_model(state, target):
                            result = self._conflict_result(
                                target,
                                state,
                                "A different shared model is already starting "
                                "on this endpoint.",
                            )
                            self.last_result = result
                            return result
    ...
    def _attach_to_healthy_runtime(self, state, target):
        if state and self._same_endpoint(state, target):
            ...
            if (
                ...
                and process_match is not False
            ):
                state["status"] = "ready"
                state["updated_at"] = self._now()
                state = self._register_client(state)
    ...
    def _conflict_result(self, target, state, reason):
        return RuntimeResult(
            ...
            "Stop the active runtime or select the same model.",
            target["model_name"],
            self._int_or_none(state.get("server_pid")),
            False,
            {
    ...
    def _target(self, model, host, port, context, threads):
    ...
    def _same_model(self, state, target):
        ...
        if state_path:
            return self._normalized_path(state_path) == self._normalized_path(
                target["model_path"]
            )
        return str(state.get("model_name", "")).casefold() == (
