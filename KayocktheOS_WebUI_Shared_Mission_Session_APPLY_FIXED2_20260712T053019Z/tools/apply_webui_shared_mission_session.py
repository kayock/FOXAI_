...
print("Verifier corrections:")
print("  1. Portable isolated Python no longer relies on PYTHONPATH.")
print("  2. /api/chat/reset is tested with GET, matching the live WebUI route.")
print("The approved candidate source files are unchanged.")
...
try:
    ...
    py_compile.compile(
        ...
        cfile=str(verification_dir / "run_test_bootstrap.pyc"),
        doraise=True,
    )
    add_check("portable_python_compile_web_session_and_bootstrap", True, str(verification_dir))
    add_check(
        "portable_python_isolated_mode_test_bootstrap",
        True,
        "Tests insert bundle/payload directly into sys.path; PYTHONPATH is not relied upon.",
    )

    env = os.environ.copy()
    env["PYTHONDONTWRITEBYTECODE"] = "1"
    bootstrap = bundle / "tools" / "run_test_bootstrap.py"
    payload_dir = bundle / "payload"

    def portable_test_command(test_path: Path, *test_args: str) -> list[str]:
        return [
            sys.executable,
            "-S",
            str(bootstrap),
            str(payload_dir),
            str(test_path),
            *[str(item) for item in test_args],
        ]
    ...
    run_test(
        ...
        portable_test_command(tests["mission_session"]),
        120,
        "Ran 6 tests",
    )
    run_test(
        "webui_routing_archive_source_tests_11",
        portable_test_command(tests["webui_static"], str(live_web)),
        120,
        "Ran 11 tests",
    )
    run_test(
        "phase1_security_regression_tests_15",
        portable_test_command(tests["phase1"]),
        120,
        "Ran 15 tests",
    )
    run_test(
        "engineer_intake_regression_tests_8",
        portable_test_command(tests["engineer_intake"], str(live_engineer)),
        120,
        "Ran 8 tests",
    )
