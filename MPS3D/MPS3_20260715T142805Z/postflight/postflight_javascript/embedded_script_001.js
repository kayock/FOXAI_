...
let activeProject=null, curLib='', missionData=null, modelCatalog=[], selectedProfileId=''; const chat=document.getElementById('chatLog');
...
async function send(){let text=q('input').value.trim();if(!text)return;let browserStarted=performance.now();let d=null;q('input').value='';logline('user','ERIC',text);think(true);try{d=await (await fetch('/api/chat/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text})})).json();if(d.answer)logline('fox',d.speaker||q('ap').textContent.toUpperCase(),d.answer);if(!d.ok)logline('bad','ERROR',d.message||'Mission turn failed verification.')}catch(e){logline('bad','ERROR',String(e))}think(false);let browserMs=performance.now()-browserStarted;let modelMs=Number(d?.timing?.model_ms||0);q('ms').textContent=modelMs>0?`READY • ${(browserMs/1000).toFixed(1)}s total • ${(modelMs/1000).toFixed(1)}s model`:`READY • ${(browserMs/1000).toFixed(1)}s`;loadMemory();refresh()}
...
initDepartmentNav();loadModels();loadProf();loadProjects();loadMemory();loadBridge();loadRecoveryDashboard();loadFoxSentryIncidents();refresh();setInterval(refresh,8000);setInterval(loadBridge,10000)
