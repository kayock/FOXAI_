...
class StatusClarityPreviewTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.package = Path(__file__).resolve().parents[1]
        cls.path = cls.package / "candidate/core/foxai_web.py"
    ...
    def test_three_operator_labels_present(self):
        for expected in (
            "<div class=lab>Engine</div><div id=rt>",
            "<div class=lab>Model source</div><div id=msrc>",
            "<div class=lab>Network use</div><div id=nuse>",
        ):
    ...
    def test_backend_network_mapping_present(self):
        self.assertIn(
            "active_source_type=='LAN_OPENAI_COMPATIBLE'",
            self.text,
        )
        self.assertIn("chat_network_use='LAN'", self.text)
        self.assertIn(
            "active_source_type=='ONLINE_PROVIDER'",
            self.text,
        )
    ...
    def test_frontend_fields_are_separate(self):
        self.assertIn(
            "q('rt').textContent=s.chat_engine_state",
            self.text,
        )
        self.assertIn(
            "q('msrc').textContent=s.chat_online?",
            self.text,
        )
        self.assertIn(
            "q('nuse').textContent=s.chat_network_use||'NONE'",
            self.text,
        )

    def test_local_profiles_report_no_network(self):
        self.assertIn(
            "else:\n        chat_network_use='NONE'",
            self.text,
        )

    def test_quick_status_uses_running_stopped(self):
        self.assertIn(
            "Chat ${s.chat_online?'Running':'Stopped'}",
            self.text,
        )
        self.assertIn(
            "ComfyUI ${s.comfy_online?'Running':'Stopped'}",
            self.text,
        )
