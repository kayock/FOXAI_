...
def main() -> int:
    ...
    try:
        ...
        desktop_shortcut = resolve_shortcut(
            ROOT / desktop_expected["filename"]
        )
        web_shortcut = resolve_shortcut(
            ROOT / web_expected["filename"]
        )
        ...
        checks = {
            ...
            "desktop_target": (
                str(desktop_shortcut.get("TargetPath") or "").casefold()
                == desktop_expected["target"].casefold()
            ),
            "desktop_working_directory": (
                str(
                    desktop_shortcut.get("WorkingDirectory") or ""
                ).casefold()
                == desktop_expected["working_directory"].casefold()
            ),
            "desktop_icon": (
                str(desktop_shortcut.get("IconLocation") or "").casefold()
                == desktop_expected["icon"].casefold()
            ),
            "web_resolved": web_shortcut.get("resolved") is True,
            "web_target": (
                str(web_shortcut.get("TargetPath") or "").casefold()
                == web_expected["target"].casefold()
            ),
            "web_working_directory": (
                str(
                    web_shortcut.get("WorkingDirectory") or ""
                ).casefold()
                == web_expected["working_directory"].casefold()
            ),
            "web_icon": (
                str(web_shortcut.get("IconLocation") or "").casefold()
                == web_expected["icon"].casefold()
            ),
        }
        if not all(checks.values()):
            raise RuntimeError(
                "One or both protected root shortcuts no longer match."
            )

        desktop_icon = Path(desktop_expected["icon"])
        web_icon = Path(web_expected["icon"])
        icons = {
            "desktop": {
                "path": str(desktop_icon),
                "exists": desktop_icon.is_file(),
                "sha256": (
                    sha256(desktop_icon)
                    if desktop_icon.is_file()
                    else None
                ),
            },
            "web": {
                "path": str(web_icon),
                "exists": web_icon.is_file(),
                "sha256": (
                    sha256(web_icon)
                    if web_icon.is_file()
                    else None
        ...
        if not all(item["exists"] for item in icons.values()):
            raise RuntimeError(
                "One or both USB-owned shortcut icons are missing."
            )
