...
def verify_quarantine_sources(selected, upload_dir: Path):
    ...
    print(f"Stage 1/4: hashing {total} exact quarantine source files...", flush=True)
    ...
    for kind, records, source_root in (
        ...
        for item in records:
            ...
            entries.append({
                ...
                "expected_sha256": expected,
                "actual_sha256": actual,
                "verified": source.is_file() and actual == expected,
            })
            if current == 1 or current % 250 == 0 or current == total:
                print(f"  Source hash: {current}/{total}", flush=True)

    failed = [item for item in entries if not item["verified"]]
    ...
    if failed:
        raise RuntimeError(f"{len(failed)} quarantined source file(s) failed hash verification.")

    return entries

def build_runtime_manifest(selected, source_entries):
    manifest = selected["manifest"]
    return {
        "format": "foxai_portable_desktop_runtime_manifest_v1",
        ...
        "desktop_package_file_count": len(manifest.get("desktop_package_files") or []),
        "runtime_size_bytes": sum(x.get("size_bytes", 0) for x in manifest.get("runtime_files") or []),
        "desktop_package_size_bytes": sum(x.get("size_bytes", 0) for x in manifest.get("desktop_package_files") or []),
        "desktop_distributions": manifest.get("desktop_distributions") or [],
        "files": [
            {
                "kind": item["kind"],
...
def make_report(receipt, plan):
    ...
    lines = [
        ...
        f"- Live apply performed: **False**",
        f"- Live files modified: **False**",
        f"- Apply capability present: **False**",
        "",
        "## Verified Phase 3C source",
        "",
        f"- Quarantine run: `{plan.get('phase3c_run')}`",
        f"- Source files hash-verified: **{counts.get('source_file_count')}**",
...
def main():
    ...
    receipt = {
        "action": "foxai_pdr_phase3d_preview_first_exact_apply_plan",
    ...
    try:
        ...
        source_entries = verify_quarantine_sources(selected, upload_dir)

        runtime_manifest = build_runtime_manifest(selected, source_entries)
        ...
        plan_core = {
            "format": "foxai_pdr_phase3d_exact_apply_plan_v1",
