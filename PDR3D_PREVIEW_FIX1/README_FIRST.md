...
The first preview stopped safely because 58 imported `.pyc` files had changed after Phase 3C verification. Those files are disposable Python bytecode caches, not package source. The failed run confirmed all 58 mismatches were under `__pycache__` and all protected FOXAI files and shortcuts remained unchanged.

FIX1 excludes every `__pycache__`, `.pyc`, and `.pyo` entry from the deterministic manifest and live plan. It will verify and plan only stable source, resource, DLL, extension, metadata, and runtime files. The diagnostic launcher now sets `PYTHONDONTWRITEBYTECODE=1`.

FIX1 also repairs the report writer so a fail-closed run always produces a usable report even when no exact plan was created.
