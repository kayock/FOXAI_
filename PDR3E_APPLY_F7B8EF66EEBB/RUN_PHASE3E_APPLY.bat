...
echo ============================================================
echo FOXAI Portable Desktop Runtime Phase 3E-A
echo Approved Separate Launcher Apply
echo ============================================================
echo.
echo APPROVED PLAN:
echo F7B8EF66EEBB
...
echo START_FOXAI_DESKTOP_PORTABLE.bat
echo.
echo It will NOT:
...
"%PY%" -s "%~dp0phase3e_apply.py" --root "%FOXAI_ROOT%" --bundle "%~dp0."
set "RC=%ERRORLEVEL%"

echo.
if "%RC%"=="0" (
    echo Phase 3E-A launcher apply completed and verified.
    echo Do not run the new launcher yet.
    echo Upload the newest APPLY_OUTPUT\...\UPLOAD_THIS folder first.
) else (
    echo Phase 3E-A stopped with exit code %RC%.
