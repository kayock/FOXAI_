...
EXPECTED_PLAN_ID = "9418899ab4f89bc9445589015422b8015220c0040bb77c63e864ca7ccdc95dc6"
EXPECTED_APPROVAL_PHRASE = "APPROVE PDR3F 9418899AB4F8"
EXPECTED_DESTINATION_NAME = "START_FOXAI_WORKSHOP_PORTABLE.bat"
...
def validate_plan(plan, preview_receipt, root: Path, bundle: Path):
    ...
    checks = {
        "format": plan.get("format") == "foxai_pdr_phase3f_combined_launcher_plan_v1",
        ...
        "action_kind": action.get("kind") == "combined_portable_workshop_launcher",
        ...
        "payload_hash": sha256_file(payload) == action.get("expected_sha256"),
        "payload_size": (
    ...
    checks["actual_destination_absent"] = not (
        root / EXPECTED_DESTINATION_NAME
    ).exists()
...
def make_report(receipt):
    lines = [
        "# FOXAI Portable Desktop Runtime Phase 3F-A",
        "## Approved Combined Workshop Launcher Apply",
        "",
    ...
    if receipt.get("state") == "combined_launcher_applied_verified":
        lines += [
            "## Applied file",
            "",
            ...
            "- The new combined launcher was not run.",
            "",
            "## Next gate",
            "",
            "Upload this receipt for review before running the combined launcher.",
        ]
    else:
        lines += [
            "## Stop information",
...
def main():
    ...
    receipt = {
        "action": "foxai_pdr_phase3f_approved_combined_launcher_apply",
    ...
    try:
        plan = json.loads(
            (bundle / "EXACT_APPROVED_COMBINED_LAUNCHER_PLAN.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        if not validation["passed"]:
            ...
            raise RuntimeError(
                "Approved combined launcher plan validation failed: "
                + ", ".join(failed)
            )
        ...
        if not protected_before["passed"]:
            raise RuntimeError("Protected FOXAI state failed before combined apply.")

        exact_expected = {
            "START_FOXAI_DESKTOP_PORTABLE.bat": (
                plan["portable_foxai_launcher"]["sha256"]
            ),
            "foxai.py": (
                next(
                    item["expected_sha256"]
                    for item in preview_receipt["exact_required_files"]["files"]
                    if item["path"] == "foxai.py"
                )
            ),
            "ComfyUI/main.py": (
                next(
            ...
            "System/PortableRuntime/verify_desktop_runtime.py": (
                next(
                    item["expected_sha256"]
                    for item in preview_receipt["exact_required_files"]["files"]
                    if item["path"] == "System/PortableRuntime/verify_desktop_runtime.py"
                )
            ),
        }
        exact_checks = []
        for relative, expected in exact_expected.items():
            path = root / Path(relative)
            actual = sha256_file(path)
            exact_checks.append({
                "path": relative,
        ...
        receipt["exact_required_files_before"] = {
            "files": exact_checks,
            "passed": all(item["matches_expected"] for item in exact_checks),
        }
        if not receipt["exact_required_files_before"]["passed"]:
            raise RuntimeError("Required launcher/runtime files changed after preview.")

        if destination.exists():
            raise RuntimeError("Combined launcher destination is no longer absent.")
        ...
        if (
            ...
            raise RuntimeError("Staged combined launcher failed verification.")
        ...
        if destination.exists():
            raise RuntimeError("Combined launcher destination appeared during staging.")
        ...
        if (
            ...
            raise RuntimeError(
                "Live combined launcher verification failed; add-only file rolled back."
            )
        ...
        if not protected_after["passed"]:
            if destination.is_file() and sha256_file(destination) == action["expected_sha256"]:
                ...
                receipt["net_live_changes"] = False
            raise RuntimeError(
                "Protected FOXAI state failed after apply; launcher rolled back."
            )
        ...
        for relative, expected in exact_expected.items():
            path = root / Path(relative)
            actual = sha256_file(path)
            exact_checks_after.append({
                "path": relative,
                "expected_sha256": expected,
                "actual_sha256": actual,
                "matches_expected": actual == expected,
            })
        receipt["exact_required_files_after"] = {
            "files": exact_checks_after,
            "passed": all(item["matches_expected"] for item in exact_checks_after),
        }
        if not receipt["exact_required_files_after"]["passed"]:
            if destination.is_file() and sha256_file(destination) == action["expected_sha256"]:
                destination.unlink()
                receipt["new_launcher_added"] = False
                receipt["net_live_changes"] = False
            raise RuntimeError(
                "Required launcher/runtime files changed during apply; launcher rolled back."
            )
        ...
        receipt["state"] = "combined_launcher_applied_verified"
        ...
        try:
            preview_receipt_path = bundle / "PREVIEW_RECEIPT.json"
            if preview_receipt_path.is_file():
                preview_receipt = json.loads(
                    preview_receipt_path.read_text(encoding="utf-8")
                )
        ...
        (upload / "UPLOAD_INSTRUCTIONS.txt").write_text(
            ...
            "Do not run the new combined launcher until this receipt is reviewed.\n",
            encoding="utf-8",
        )

        print()
        print("Phase 3F-A state:", receipt["state"])
        ...
        if receipt.get("failure"):
            ...
            print("The new combined launcher was not run. Upload the receipt first.")
