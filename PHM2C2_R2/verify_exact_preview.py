...
def main() -> int:
    ...
    try:
        ...
        test_runner = (
            ...
            "start_dir=sys.argv[2],pattern='test_model_sources.py');"
        ...
        combined_test_output = (tests.stdout or '') + (tests.stderr or '')
        if (
            tests.returncode != 0
            or 'Ran 10 tests' not in combined_test_output
            or '\nOK' not in combined_test_output
        ):
            raise RuntimeError(
                'Candidate model-source tests failed: '
                + combined_test_output[-4000:]
            )
        receipt['checks']['model_source_tests'] = {
            'passed': True,
            'tests': 10,
            'stdout': tests.stdout,
            'stderr': tests.stderr,
            'candidate_path_inserted_explicitly': True,
            'pythonpath_environment_required': False,
        }
