...
def is_bytecode_cache(relative_path: str) -> bool:
    ...
    return "__pycache__" in parts and relative_path.endswith(".pyc")


def split_runtime_records(root: Path) -> dict:
    all_records = tree_records(root)
    owned = [
        item for item in all_records
        if not is_bytecode_cache(item["path"])
    ]
    bytecode = [
        item for item in all_records
        if is_bytecode_cache(item["path"])
    ]
    unexpected = [
        item for item in all_records
        if item not in owned and item not in bytecode
    ]
    return {
        "all": all_records,
        "owned": owned,
...
def verify_owned_runtime_tree(root: Path, label: str) -> dict:
    split = split_runtime_records(root)
    expected = PLAN["candidate"]
    owned_digest = tree_digest(split["owned"])
    passed = (
        len(split["owned"]) == expected["site_package_file_count"]
        and owned_digest == expected["site_package_tree_digest"]
        and not split["unexpected"]
    )
    result = {
        "passed": passed,
        "label": label,
        "all_file_count": len(split["all"]),
        "owned_file_count": len(split["owned"]),
        "owned_tree_digest": owned_digest,
        "bytecode_cache_count": len(split["bytecode"]),
        "unexpected_file_count": len(split["unexpected"]),
        "bytecode_cache_paths": [
            item["path"] for item in split["bytecode"]
        ],
        "unexpected_paths": [
            item["path"] for item in split["unexpected"]
        ],
    }
    if not passed:
        raise RuntimeError(
            f"{label} wheel-owned runtime tree verification failed."
        )
    return result
...
def verify_post_apply_files() -> dict:
    owned = verify_owned_runtime_tree(
        LIVE_SITE_PACKAGES,
        "Live Runtime/Core/site-packages",
    )
    expected = PLAN["candidate"]
    checks = {
        "owned_runtime": owned,
...
def main() -> int:
    ...
    try:
        ...
        receipt["checks"]["candidate_tree_after_tests"] = (
            verify_owned_runtime_tree(
                candidate_core / "site-packages",
                "Candidate after preflight and Boundary Watch",
            )
        )
        if (
            receipt["checks"]["candidate_tree_after_tests"][
            ...
            raise RuntimeError(
                "Candidate generated bytecode despite suppression."
            )
        ...
        receipt["checks"]["post_test_runtime_files"] = (
            verify_post_apply_files()
        )
