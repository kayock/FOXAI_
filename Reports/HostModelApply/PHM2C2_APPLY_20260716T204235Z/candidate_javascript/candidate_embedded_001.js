...
let activeProject=null, curLib='', missionData=null, modelCatalog=[], modelSourceState=null, selectedProfileId='', activeProfileId='', pendingMissionImage=null, activeMissionImage=null, chatStreamController=null; const chat=document.getElementById('chatLog');
/* MODEL_PROFILE_SELECTOR_PHASE2_REGISTRY_START */
const MODEL_PROFILES=[
 {id:'fast_text',label:'⚡ Fast Text',model:'Qwen3.5-4B-Q4_K_M.gguf',source:'USB',best:'Quick questions, short summaries, simple instructions, and casual chat.',speed:'Fastest verified text profile',vision:'No',reasoning:'Off',status:'BENCHMARK SUPPORTED',note:'10.87 median tok/s; exact instruction and length checks passed. Runtime launches with reasoning off and budget 0.',recommended:true},
 {id:'balanced_text',label:'⚖️ Balanced Text',model:'Qwen3.5-9B-Q4_K_M.gguf',source:'USB',best:'Everyday writing, research discussion, longer answers, and general text work.',speed:'Moderate',vision:'No',reasoning:'Off',status:'BENCHMARK SUPPORTED',note:'5.75 median tok/s; exact instruction and length checks passed. Runtime launches with reasoning off and budget 0.',recommended:false},
 {id:'creative_text',label:'🎭 Creative Text',model:'PsyLLM-8B-Q5_K_M.gguf',source:'USB',best:'Brainstorming, story hooks, dialogue seeds, poetic fragments, and roleplay ideas.',speed:'Moderate',vision:'No',reasoning:'Off',status:'BRAINSTORMING SUPPORTED • LONG-FORM PENDING',note:'All eight creative responses completed without wrapper or reasoning leaks. Story hooks passed exact constraints; seven tasks missed requested length or structure, so strict long-form work remains pending.',recommended:false},
 {id:'host_general_30b',label:'🖥️ Host General 30B',model:'Qwen3-30B-A3B-Q4_K_M.gguf',source:'HOST_PC',best:'Heavier general reasoning and writing when Eric’s approved host library is available.',speed:'Host hardware dependent',vision:'No',reasoning:'Current engine behavior',status:'HOST MODEL AVAILABLE ON APPROVED MACHINE',note:'Registered for DESKTOP-G9ERN9B at C:\KayockModels. FOXAI never copies or alters this file and never silently falls back if it is unavailable.',recommended:false},
 {id:'fast_vision',label:'👁️ Fast Vision',model:'Qwen3VL-8B-Instruct-Q4_K_M.gguf',source:'USB',best:'Quick image understanding, screenshots, visible text, and general image questions.',speed:'Fast visual-language profile',vision:'Yes',reasoning:'Current engine behavior',status:'REAL IMAGE INPUT SUPPORTED • BENCHMARK PASSED',note:'Human-reviewed 5/5 real-image suite; 42.58s median and 6.56 tok/s. About 1.65× faster than Q8 with no observed quality loss on this suite.',recommended:false},
 {id:'quality_vision',label:'🔎 Quality Vision',model:'Qwen3VL-8B-Instruct-Q8_0.gguf',source:'USB',best:'Detailed image analysis, complex scenes, and careful visual reasoning.',speed:'Quality-first / slower',vision:'Yes',reasoning:'Current engine behavior',status:'REAL IMAGE INPUT SUPPORTED • BENCHMARK PASSED',note:'Human-reviewed 5/5 real-image suite; 70.38s median and 4.17 tok/s. Higher precision remains available, but this suite did not prove a quality advantage.',recommended:false}
];
...
function modelForProfile(profile){return profile?modelCatalog.find(item=>item.name===profile.model&&(!profile.source||item.source===profile.source)):null}
function modelProfileForModel(model){return model?MODEL_PROFILES.find(profile=>profile.model===model.name&&(!profile.source||profile.source===model.source)):null}
function profileBadgeClass(profile){return profile.status.includes('PENDING')?'pending':'supported'}
function sourceBadge(model){let host=model?.source==='HOST_PC';return `<span class="sourcebadge ${host?'host':''}">${esc(model?.source_label||model?.source||'UNKNOWN')}</span>`}
function renderModelProfiles(){
 let grid=q('modelProfileGrid');if(!grid)return;
 let selectedPath=q('model')?.value||'';
 grid.innerHTML=MODEL_PROFILES.map(profile=>{
  let model=modelForProfile(profile);
  let selected=Boolean(model&&model.path===selectedPath);
  let availability=model?'':'disabled';
  let recommended=profile.recommended?'<div class=small>Recommended starting profile</div>':'';
  let unavailable=profile.source==='HOST_PC'?'Approved host model unavailable. No fallback has been selected.':'Model file not currently available.';
  return `<button type=button class="modelprofile ${selected?'selected':''}" onclick="selectModelProfile('${profile.id}')" ${availability}><div class=modelprofilehead><span class=modelprofiletitle>${esc(profile.label)}</span><span class="modelprofilebadge ${profileBadgeClass(profile)}">${esc(profile.status)}</span></div>${recommended}<div class=modelprofilebest>${esc(profile.best)}</div><div class=modelprofilemeta><div>Source: ${esc(profile.source||'LOCAL')}</div><div>Speed: ${esc(profile.speed)}</div><div>Vision: ${esc(profile.vision)}</div><div>Reasoning: ${esc(profile.reasoning)}</div><div>Model: ${esc(profile.model)}</div><div>${esc(profile.note)}</div>${model?`<div>${sourceBadge(model)} ${esc(model.path)}</div>`:`<div class=bad>${esc(unavailable)}</div>`}</div></button>`;
 }).join('');
}
function setPendingModelStatus(profile,model,message=''){
 let target=q('modelProfileStatus');if(!target)return;
 if(message){target.textContent=message;return}
 if(profile&&model){
  target.textContent=`Pending selection: ${profile.label}\nSource: ${model.source_label||model.source}\nModel: ${profile.model}\nPath: ${model.path}\nNo engine action has occurred. Use Start Selected Profile when ready.`;
 }else if(model){
  target.textContent=`Pending exact GGUF selection\nSource: ${model.source_label||model.source}\nModel: ${model.name}\nPath: ${model.path}\nNo engine action has occurred. Use Start Selected Profile when ready.`;
 }else{
  target.textContent='No approved GGUF model is selected. No fallback has occurred.';
 }
}
function selectModelProfile(id){
 ...
 if(!profile||!model){
  selectedProfileId='';
  setPendingModelStatus(null,null,profile?.source==='HOST_PC'?'The approved host model is unavailable. FOXAI did not switch to a USB model. Choose a folder or explicitly select an approved USB profile.':'That profile model is unavailable. No fallback has occurred.');
  toast('Profile unavailable. No model was silently substituted.');
  return;
 }
...
function syncProfileFromSelect(){
 ...
 let profile=modelProfileForModel(selected);
...
function renderModelSourceState(){
 let target=q('modelSourceState');if(!target)return;
 let state=modelSourceState;
 if(!state?.ok){target.textContent=state?.message||'Model-source registry is unavailable.';return}
 let machine=state.machine||{},counts=state.counts||{},roots=state.approved_host_roots||[];
 let rootHtml=roots.length?roots.map((root,index)=>`<div class=modelsourceroot><b>${esc(root.scope==='session'?'SESSION':'REMEMBERED')}</b> • ${root.available?'available':'missing'}<br>${esc(root.path)}<br><button type=button onclick="forgetModelSourceRoot(${index})">Forget this folder reference</button></div>`).join(''):'No host folders approved for this computer or session.';
 target.innerHTML=`Machine: ${esc(machine.display_name||machine.name||'Unknown')}<br>Machine key: ${esc(machine.key||'')}<br>Configured: ${machine.configured?'yes':'no'}<br>Fallback policy: ${esc(state.fallback_policy||'ASK_OR_APPROVED_USB')}<br>Models: ${counts.usb||0} USB • ${counts.host_pc||0} HOST PC<br>Online/LAN sources: disabled<br><br>${rootHtml}`;
}
async function chooseHostModelFolder(){
 let d=await api('/api/model-sources/choose-folder',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
 if(d?.selected_path)q('modelSourcePath').value=d.selected_path;
 toast(d?.message||'Folder chooser finished.');
}
async function approveHostModelFolder(remember){
 let path=(q('modelSourcePath')?.value||'').trim();if(!path)return toast('Choose or enter one specific folder first.');
 let wording=remember?'Remember this folder for this computer? FOXAI will save only the path and preferences.':'Approve this folder for the current FOXAI session only?';
 if(!window.confirm(wording+'\n\nNo model file will be copied, moved, renamed, overwritten, or deleted.'))return;
 let d=await api('/api/model-sources/approve-folder',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({path,scope:remember?'remembered':'session',confirm:true})});
 toast(d?.message||'Folder approval finished.');
 if(d?.ok)await loadModels();
}
async function forgetModelSourceRoot(index){
 let root=(modelSourceState?.approved_host_roots||[])[index];if(!root)return toast('Folder reference not found.');
 if(!window.confirm(`Forget FOXAI's reference to this folder?\n\n${root.path}\n\nThe folder and models will remain untouched.`))return;
 let d=await api('/api/model-sources/forget-folder',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({path:root.path,confirm:true})});
 toast(d?.message||'Forget-folder action finished.');
 await loadModels();
}
async function forgetSelectedHostModel(){
 let selected=modelCatalog.find(item=>item.path===q('model').value);
 if(!selected||selected.source!=='HOST_PC')return toast('Select a HOST PC model first.');
 if(!window.confirm(`Forget only the preferred-model reference?\n\n${selected.path}\n\nThe model file will remain untouched and may still appear through its approved folder.`))return;
 let d=await api('/api/model-sources/forget-model',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({path:selected.path,confirm:true})});
 toast(d?.message||'Forget-model action finished.');
 await loadModels();
}
async function forgetMachineModelProfile(){
 let name=modelSourceState?.machine?.name||'this computer';
 if(!window.confirm(`Forget the complete model-source profile for ${name}?\n\nUSB models remain available. Host folders and model files remain untouched.`))return;
 let d=await api('/api/model-sources/forget-machine',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({confirm:true})});
 toast(d?.message||'Forget-computer action finished.');
 await loadModels();
}
async function loadModels(){
 let d=await (await fetch('/api/models')).json();
 modelCatalog=Array.isArray(d.models)?d.models:[];modelSourceState=d.source_state||null;
 let select=q('model'),previous=select.value;
 select.innerHTML=modelCatalog.length?modelCatalog.map(m=>`<option value="${esc(m.path)}">[${esc(m.source_label||m.source)}] ${esc(m.name)}</option>`).join(''):'<option>No approved GGUF models found</option>';
 let saved='';try{saved=localStorage.getItem('kayock.model.profile.pending')||''}catch(e){}
 let savedProfile=modelProfileById(saved),savedTarget=modelForProfile(savedProfile),previousTarget=modelCatalog.find(item=>item.path===previous),target=null,missingSavedHost=Boolean(savedProfile?.source==='HOST_PC'&&!savedTarget);
 if(savedTarget)target=savedTarget;else if(previousTarget)target=previousTarget;else if(!savedProfile)target=modelForProfile(modelProfileById('fast_text'))||modelCatalog[0]||null;
 if(target)select.value=target.path;else if(modelCatalog.length)select.selectedIndex=-1;
 let profile=target?modelProfileForModel(target):null;
 selectedProfileId=profile?.id||'';
 renderModelProfiles();renderModelSourceState();
 if(missingSavedHost)setPendingModelStatus(null,null,'The previously selected host profile is unavailable. FOXAI did not silently choose a USB fallback. Choose a host folder or explicitly select a USB profile.');else setPendingModelStatus(profile,target);
}
...
let extensionStatePreviewData=null;
...
function inventoryItemByStateKey(key){
    key=String(key||'').toLowerCase();
    return (extensionInventoryData?.items||[]).find(item=>String(item.state_control?.key||'').toLowerCase()===key)||null;
}
function extensionStateLabel(control){
    if(!control?.eligible)return 'STATE LOCKED';
    return control.effective_enabled?'ENABLED':'DISABLED';
}
function renderExtensionInventory(){
    ...
    let items=(data.items||[]).filter(item=>{
        ...
        if(search){
            let hay=[
                ...
                item.state_control?.key,item.state_control?.effective_enabled?'enabled':'disabled'
            ].join(' ').toLowerCase();
    ...
    q('extInventoryList').innerHTML=items.map(item=>{
        ...
        let control=item.state_control||null;
        let badges=[
            ...
            control?`<span class=inventorybadge>${esc(extensionStateLabel(control))}</span>`:'',
            control?.override_present?'<span class="inventorybadge attention">OVERRIDE</span>':'',
        ].join('');
        let details=[
        ...
        let stateDetails='';
        if(control){
            stateDetails=`<div class=inventorystate><b>State:</b> ${control.effective_enabled?'Enabled':'Disabled'} • Manifest default: ${control.manifest_default?'Enabled':'Disabled'} • Override: ${control.override_present?(control.override_enabled?'Enabled':'Disabled'):'None'}${control.dependencies?.length?`<br><b>Depends on:</b> ${esc(control.dependencies.join(', '))}`:''}${control.dependents?.length?`<br><b>Dependents:</b> ${esc(control.dependents.join(', '))}`:''}${!control.eligible?`<br><b>Control locked:</b> ${esc(control.lock_reason||'Not an optional extension.')}`:''}</div>`;
        }
        let actions=[];
        ...
        if(control?.eligible){
            let action=control.effective_enabled?'disable':'enable';
            actions.push(`<button onclick="previewExtensionState('${js(item.id)}','${action}')">Preview ${action==='enable'?'Enable':'Disable'}</button>`);
            if(control.override_present)actions.push(`<button onclick="previewExtensionState('${js(item.id)}','restore')">Restore Default</button>`);
        }
        if(item.status==='NEEDS_ATTENTION'||item.status==='MISSING'||control)actions.push(`<button onclick="explainExtensionInventoryItem('${js(item.id)}')">Why?</button>`);
        actions.push(`<button onclick="sendInventoryItemToMission('${js(item.id)}')">Send to Mission</button>`);
        return `<div class="inventorycard ${cls}"><h4>${esc(item.name)}</h4><div class=inventorybadges>${badges}</div><div class=inventorymeta>${esc(details)}</div>${item.description?`<div class=inventoryhealth>${esc(item.description)}</div>`:''}<div class=inventoryhealth><b>Health:</b> ${esc(item.health||'No health detail.')}</div>${stateDetails}<div class=inventoryactions>${actions.join('')}</div></div>`;
...
function renderExtensionInventorySummary(data){
    ...
    q('extInventorySummary').innerHTML=`<div class=inventorygrid>
        ...
        <div class=inventorymetric><div class=label>State Controls</div><div class=value>${esc(summary.state_controls||0)}</div></div>
        <div class=inventorymetric><div class=label>Overrides</div><div class=value>${esc(summary.state_overrides||0)}</div></div>
    </div>`;
...
async function loadExtensionInventory(){
    ...
    q('extInventoryList').textContent='Reading registries, manifests, local health, model metadata, and safe state metadata...';
    try{
...
function explainExtensionInventoryItem(id){
    let item=inventoryItemById(id);
    if(!item||!q('extStatePreview'))return;
    let control=item.state_control||{};
    let detail=[
        `Name: ${item.name}`,
        `Health state: ${item.status}`,
        `Health detail: ${item.health||'No detail.'}`,
        `Verification basis: ${item.verification_basis||'Not specified.'}`,
        `Required: ${item.required}`,
        `Path: ${item.path||'(none)'}`,
        `Manifest: ${item.manifest_path||'(none)'}`,
        control.key?`State key: ${control.key}`:'',
        control.key?`Effective state: ${control.effective_enabled?'Enabled':'Disabled'}`:'',
        control.key?`Manifest default: ${control.manifest_default?'Enabled':'Disabled'}`:'',
        control.override_present?`Override: ${control.override_enabled?'Enabled':'Disabled'}`:'Override: none',
        control.dependencies?.length?`Dependencies: ${control.dependencies.join(', ')}`:'Dependencies: none declared',
        control.dependents?.length?`Dependents: ${control.dependents.join(', ')}`:'Dependents: none declared',
        control.lock_reason?`State-control lock: ${control.lock_reason}`:'',
        item.missing_fields?.length?`Missing manifest fields: ${item.missing_fields.join(', ')}`:'',
        item.missing_tools?.length?`Missing declared tools: ${item.missing_tools.join(', ')}`:'',
        '',
        item.status==='MISSING'?'Safe guidance: This is optional/planned unless marked required. Enabling state does not install missing files.':'Safe guidance: Review the exact health basis. Use a preview-first repair or install milestone for missing software; state controls only change the enabled override.'
    ].filter(value=>value!==undefined&&value!==null).join('\n');
    extensionStatePreviewData=null;
    q('extStatePreview').innerHTML=`<div class=whybox>${esc(detail)}</div>`;
    q('extStatePreview').scrollIntoView?.({behavior:'smooth',block:'start'});
}
function renderExtensionStatePreview(data){
    extensionStatePreviewData=data;
    let host=q('extStatePreview');
    if(!host)return;
    let blockers=(data.blockers||[]).map(value=>`<div class=stateblocker>${esc(value)}</div>`).join('');
    let warnings=(data.warnings||[]).map(value=>`<div class=statewarning>${esc(value)}</div>`).join('');
    let klass=data.can_apply?'statepreview safe':'statepreview blocked';
    let action=String(data.action||'').toUpperCase();
    let approval=data.approval_phrase||'';
    host.innerHTML=`<div class="${klass}"><h4>${esc(action)} — ${esc(data.name||data.key||'Extension')}</h4><div class=stategrid>
        <div class=statebox><div class=label>Current Effective</div><div class=value>${data.current_effective?'Enabled':'Disabled'}</div></div>
        <div class=statebox><div class=label>Manifest Default</div><div class=value>${data.manifest_default?'Enabled':'Disabled'}</div></div>
        <div class=statebox><div class=label>Current Override</div><div class=value>${data.override_present?(data.override_enabled?'Enabled':'Disabled'):'None'}</div></div>
        <div class=statebox><div class=label>Proposed Effective</div><div class=value>${data.proposed_effective?'Enabled':'Disabled'}</div></div>
        <div class=statebox><div class=label>State File Effect</div><div class=value>${esc(data.state_file_effect||'None')}</div></div>
        <div class=statebox><div class=label>Preview Digest</div><div class=value>${esc(data.preview_digest||'')}</div></div>
    </div>${warnings}${blockers}<div class=small>Dependencies: ${esc((data.dependencies||[]).join(', ')||'None declared')}<br>Enabled dependents: ${esc((data.enabled_dependents||[]).join(', ')||'None')}<br>Backup and receipt are mandatory during apply.</div><h4>Exact State Diff</h4><div class=statediff>${esc(data.diff||'(no change)')}</div>${data.can_apply?`<div class=stateapproval><input id=extStateApproval placeholder="Type the exact approval phrase" autocomplete=off><button onclick="applyExtensionStateChange(this)">Apply Verified Change</button><button onclick="cancelExtensionStatePreview()">Cancel</button></div><div class=small>Required phrase: <code>${esc(approval)}</code></div>`:'<div class=stateblocker>No write is permitted from this preview.</div><button onclick="cancelExtensionStatePreview()">Close Preview</button>'}</div>`;
    host.scrollIntoView?.({behavior:'smooth',block:'start'});
}
async function previewExtensionState(target,action){
    let host=q('extStatePreview');
    if(host)host.textContent='Building exact state preview...';
    try{
        let response=await fetch('/api/extensions/state/preview',{
            method:'POST',headers:{'Content-Type':'application/json'},
            body:JSON.stringify({target,action})
        });
        let data=await response.json();
        if(!response.ok||!data?.ok)throw new Error(data?.message||`HTTP ${response.status}`);
        renderExtensionStatePreview(data);
    }catch(error){
        extensionStatePreviewData=null;
        if(host)host.innerHTML=`<div class=stateblocker>${esc(String(error))}</div>`;
    }
}
function cancelExtensionStatePreview(){
    extensionStatePreviewData=null;
    if(q('extStatePreview'))q('extStatePreview').textContent='No state change is pending. Inventory buttons create a read-only exact preview first.';
}
async function applyExtensionStateChange(button=null){
    let data=extensionStatePreviewData;
    if(!data?.can_apply){toast('No approved state preview is active.');return;}
    let approval=String(q('extStateApproval')?.value||'').trim();
    if(button)button.disabled=true;
    try{
        let response=await fetch('/api/extensions/state/apply',{
            method:'POST',headers:{'Content-Type':'application/json'},
            body:JSON.stringify({
                target:data.target,
                action:data.action,
        ...
        extensionStatePreviewData=null;
        q('extStatePreview').innerHTML=`<div class="statepreview safe"><h4>VERIFIED STATE CHANGE</h4><div class=whybox>${esc(result.message||'State change applied.')}\nReceipt: ${esc(result.receipt_path||'')}\nBackup: ${esc(result.backup_path||'')}\nState file: ${esc(result.state_file||'')}</div></div>`;
        toast('Verified extension state change applied.');
        await loadExtensionInventory();
        await loadExtensions();
    }catch(error){
        q('extStatePreview').innerHTML=`<div class=stateblocker>${esc(String(error))}</div><button onclick="cancelExtensionStatePreview()">Close</button>`;
    }finally{
...
function sendInventoryItemToMission(id){
    ...
    if(!item)return;
    go('mission');
    q('input').value=`Review this Extension Manager inventory item.\n\nName: ${item.name}\nStatus: ${item.status}\nCategory: ${item.category}\nRequired: ${item.required}\nVersion: ${item.version}\nPath: ${item.path||'(none)'}\nSource: ${item.source}\nHealth: ${item.health}\nVerification basis: ${item.verification_basis||'Not specified'}\nState: ${item.state_control?(item.state_control.effective_enabled?'Enabled':'Disabled'):'Not state-controlled'}\n\nDo not assume a missing optional component is an error. Recommend only preview-first actions.`;
    toast('Inventory item sent to Mission Console input.');
}

const EXTENSION_MANAGER_HELP={
 quick:{title:'Extension Manager Quick Cheat Sheet',sections:[
  ['SAFE TO CLICK',['Refresh Inventory','Search and filters','Why?','Open Folder','Open UI','Send to Mission','Quick Cheat Sheet and Operator Manual']],
  ['PREVIEW ONLY — NO WRITE',['Preview Enable','Preview Disable','Restore Default preview','Cancel or Close Preview']],
  ['REQUIRES TYPED APPROVAL',['Apply Verified Change writes Config\\extension_state.json only after the exact phrase, preview digest, dependency checks, backup, receipt, and post-write verification.']],
  ['NEVER CHANGES AUTOMATICALLY',['No silent enable or disable','No install, update, removal, or download','Protected core, system, and department items cannot be overridden']],
  ['RECOVERY',['Cancel removes the pending preview','Successful changes show receipt and backup paths','Failed verification rolls the state file back automatically']]
 ]},
 manual:{title:'Extension Manager Operator Manual',sections:[
  ['Inventory & Health',['Verified means an explicit contract was checked. Installed means the file or component exists but was not fully contract-verified. Missing means a planned or optional item is absent. Needs Attention means the inventory found a mismatch or missing declared dependency.']],
  ['Filters and Why?',['Use Search plus Category, Health, and Required/Optional filters to reduce the list. Why? explains health, verification basis, state eligibility, dependencies, and safe repair guidance.']],
  ['Preview versus Apply',['Preview Enable, Preview Disable, and Restore Default are read-only. The preview shows current state, manifest default, current override, proposed state, exact JSON diff, state-file effect, blockers, dependencies, digest, and required phrase. Only Apply Verified Change can write.']],
  ['What Disable Does',['Disable records an operator override and blocks Extension Manager launch eligibility for state-aware components. It does not terminate an already running process, uninstall files, delete data, or rewrite the manifest.']],
  ['Dependencies and Protection',['Disabling is blocked when enabled dependents require the target. Enabling is blocked when declared dependencies are missing or disabled. Required, reserved, core, system, and department entries remain protected.']],
  ['Overrides and Restore Default',['With no extension_state.json file, manifest defaults are used. The first approved override creates the file. Restore Default removes that extension override. When the last override is restored, the file is removed.']],
  ['Evidence and Recovery',['Every successful state write requires a verified backup and JSON receipt. The write is atomic and post-verified. Any failure triggers rollback. Receipt and backup paths appear after a verified change.']],
  ['Advanced Manifest Tools',['This collapsed section is a separate manifest-maintenance view. Its totals count manifest records, not all inventory components. Validation and Suggest Fix are separate from state controls. Protected records show Protected instead of a misleading Disable button.']]
 ]}
};
function openExtensionHelp(mode='quick'){
 const data=EXTENSION_MANAGER_HELP[mode]||EXTENSION_MANAGER_HELP.quick;
 const modal=q('extHelpModal'),body=q('extHelpBody'),title=q('extHelpTitle');
 if(!modal||!body||!title)return;
 title.textContent=data.title;
 body.innerHTML=data.sections.map(([heading,items])=>`<section class=exthelpsection><h4>${esc(heading)}</h4><ul>${items.map(item=>`<li>${esc(item)}</li>`).join('')}</ul></section>`).join('');
 modal.hidden=false;
 modal.scrollTop=0;
}
function closeExtensionHelp(){const modal=q('extHelpModal');if(modal)modal.hidden=true;}
document.addEventListener('keydown',event=>{if(event.key==='Escape')closeExtensionHelp();});
...
function extDashboardFromData(d){
    ...
    q('extDashboard').innerHTML=`<div class=moddash>
        <div class=modbox><div class=label>Manifest Records</div><div class=value>${total}</div></div>
        <div class=modbox><div class=label>Manifest Enabled</div><div class=value>${enabled}</div></div>
        <div class=modbox><div class=label>Manifest Disabled</div><div class=value>${disabled}</div></div>
        <div class=modbox><div class=label>Manifest Valid</div><div class=value>${valid}</div></div>
...
async function loadExtensions(){
    ...
    try{
        ...
        let stateText=d.state_file_exists?`EXISTS — ${d.state_file}`:`NOT CREATED — reserved path: ${d.state_file}`;
        q('extSummary').textContent=`Manifest records: ${d.count}
Manifest enabled: ${d.enabled}
Manifest valid: ${d.valid}
State override file: ${stateText}`;
        extDashboardFromData(d);
        q('extList').innerHTML=(d.items||[]).map(x=>{
            let cls=x.enabled?'':'disabledmod';
            let kind=String(x.kind||'extension').toLowerCase();
            let protectedState=x.manifest==='built-in'||['system','core','department'].includes(kind)||Boolean(x.raw?.required||x.raw?.reserved);
            let missing=(x.missing&&x.missing.length)?`Missing: ${x.missing.join(', ')}`:'Manifest OK';
            let stateButton=protectedState?`<button class=protectedcontrol disabled title="Required, core, system, and department records are protected">Protected</button>`:`<button onclick="toggleExtension('${js(x.key)}',${x.enabled?'false':'true'})">${x.enabled?'Preview Disable':'Preview Enable'}</button>`;
            return `<div class="extcard ${cls}"><h4>🧩 ${esc(x.name)} <span class=small>v${esc(x.version)}</span></h4><span class=exttag>${esc(x.kind||'extension')}</span><span class=exttag>${esc(x.status||'UNKNOWN')}</span><span class=exttag>${x.enabled?'ENABLED':'DISABLED'}</span>${protectedState?'<span class=exttag>PROTECTED</span>':''}<div class=extmeta>${esc(x.description||'')}\nOfficer: ${esc(extOfficerText(x.officer))}\nKey: ${esc(x.key)}\n${esc(missing)}\nManifest: ${esc(x.manifest)}\nFolder: ${esc(x.folder)}</div><div class=extactions>${stateButton}<button onclick="suggestManifestFix('${js(x.key)}','${js(x.manifest)}')">Suggest Fix</button><button onclick="sendExtensionToMission('${js(x.key)}')">Send to Mission</button></div></div>`
...
async function toggleExtension(key,enabled){
    let item=inventoryItemByStateKey(key);
    let target=item?.id||key;
    return previewExtensionState(target,enabled?'enable':'disable');
}
