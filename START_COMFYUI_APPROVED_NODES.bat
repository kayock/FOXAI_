...
"%PYTHON%" -I -B -S "%MANAGER%" --root "%FOXAI_ROOT%" start --profile approved-custom-nodes-cpu --source direct
set "RC=%ERRORLEVEL%"
echo.
if "%RC%"=="0" (
  echo Approved Custom Nodes CPU lifecycle ended cleanly.
) else (
  echo Approved Custom Nodes CPU lifecycle stopped with code %RC%.
)
pause
exit /b %RC%
