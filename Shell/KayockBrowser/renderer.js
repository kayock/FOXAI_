...
function normalizeInput(text) {
  ...
  if (!value || value === 'kayock://home' || value === 'kayock://bridge') return 'kayock://home';
  ...
  if (value === 'kayock://workshop') return 'kayock://workshop';
...
function makeTitle(url) {
  if (url === 'kayock://home' || url === 'kayock://bridge') return 'KayocktheOS Bridge';
  ...
  if (url === 'kayock://workshop') return 'Workshop';
...
function loadTarget(text) {
  ...
  else if (url === 'kayock://workshop') { showWorkshopPanel(); showHome(true); tab.webview.classList.add('hidden'); }
...
document.getElementById('bridgeBtn')?.addEventListener('click', () => loadTarget('kayock://home'));
...
function bridgeDotClass(status) {
  if (status === 'OK' || status === 'enabled' || status === 'ready') return 'ok';
  if (status === 'planned') return 'wait';
  return 'bad';
}

function renderBridgeList(containerId, items, mode) {
  const container = document.getElementById(containerId);
  if (!container) return;
  container.innerHTML = '';
  for (const item of items || []) {
    const row = document.createElement('div');
    const status = mode === 'module' ? item.status : item.status;
    row.className = `bridgeItem ${bridgeDotClass(status)}`;
    if (mode === 'module') {
      row.innerHTML = `<strong>${escapeHtml(item.name || item.id || 'Module')}</strong><span>${escapeHtml(item.status || '')} · ${escapeHtml(item.owner || '')}</span>`;
    } else {
      row.innerHTML = `<strong>${escapeHtml(item.label || 'Check')}</strong><span>${escapeHtml(item.status || '')} · ${escapeHtml(item.path || '')}</span>`;
    }
    container.appendChild(row);
  }
}

async function refreshBridgeDesktop() {
  const badge = document.getElementById('bridgeApiBadge');
  if (!badge) return;

  try {
    const response = await fetch('http://127.0.0.1:8844/api/status', { cache: 'no-store' });
    ...
    const projectName = data?.project?.name || 'KayocktheOS';
    const version = data?.project?.version || '0.2.0';
    const codename = data?.project?.codename || 'Bridge Desktop';
    const operatorName = data?.operator?.display_name || 'Operator';
    const quote = data?.operator?.quote || 'Wonder is a tool. Build with it.';

    const title = document.getElementById('bridgeTitle');
    if (title) title.textContent = `${projectName} v${version}`;

    const greeting = document.getElementById('bridgeGreeting');
    if (greeting) greeting.textContent = `Welcome back, ${operatorName}.`;

    const quoteEl = document.getElementById('bridgeQuote');
    if (quoteEl) quoteEl.textContent = `“${quote}”`;

    const set = (id, value) => { const el = document.getElementById(id); if (el) el.textContent = value; };
    set('bridgeEnabledModules', data?.summary?.enabled_modules ?? '--');
    set('bridgePlannedModules', data?.summary?.planned_modules ?? '--');
    set('bridgeHealthCount', `${data?.summary?.health_ok ?? '--'}/${data?.summary?.health_total ?? '--'}`);
    set('bridgeShellStatus', data?.shell?.status || '--');

    renderBridgeList('bridgeHealthList', data.health || [], 'health');
    renderBridgeList('bridgeModuleList', data.modules || [], 'module');

    const logs = document.getElementById('bridgeLogs');
    if (logs) logs.textContent = (data.recent_logs && data.recent_logs.length) ? data.recent_logs.join('\n') : 'No boot log entries yet.';

    const updated = document.getElementById('bridgeUpdated');
    if (updated) updated.textContent = `Last updated: ${data.timestamp || new Date().toLocaleString()}`;
  } catch (err) {
    badge.textContent = 'CORE API OFFLINE';
    badge.classList.add('offline');
    badge.classList.remove('online');
    const logs = document.getElementById('bridgeLogs');
    if (logs) logs.textContent = 'Core API is offline. Start KayocktheOS with Start_KayocktheOS.bat, then reopen the Bridge.';
  }
}

setInterval(refreshBridgeDesktop, 5000);
