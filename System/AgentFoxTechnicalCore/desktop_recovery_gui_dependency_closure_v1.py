...
SCHEMA_PREFIX = "foxai.agent_fox.technical_core.v1a3c6"
...
TARGET_LAUNCHER = r"Z:\FOXAI\START_FOXAI_DESKTOP_TWO_WINDOW_RECOVERY.bat"
TARGET_SCRIPT = r"Z:\FOXAI\foxai.py"
TARGET_INTERPRETER_KIND = "pythonw_static_sibling_of_observed_python"
EXPECTED_REACHED_NODE_COUNT = 70
EXPECTED_DEPENDENCY_EDGE_COUNT = 205
EXPECTED_PRELIMINARY_UNRESOLVED_COUNT = 1
EXPECTED_ZERO_BYTE_SOURCE = r"Z:\FOXAI\core\__init__.py"
EMPTY_FILE_SHA256 = "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"

OUTPUT_NAMES = (
    "DESKTOP_RECOVERY_GUI_CONTEXT.json",
    "DESKTOP_RECOVERY_GUI_FIRST_PARTY_CLOSURE.json",
    "DESKTOP_RECOVERY_GUI_CONDITIONAL_IMPORTS.json",
    "DESKTOP_RECOVERY_GUI_PACKAGE_REQUIREMENTS.json",
    "DESKTOP_RECOVERY_GUI_UNRESOLVED_BRANCHES.json",
    "DESKTOP_RECOVERY_GUI_RUNTIME_UNCERTAINTY.json",
    "DESKTOP_RECOVERY_GUI_COVERAGE.json",
)
...
def select_desktop_recovery_gui_context(source: dict[str, Any]) -> dict[str, Any]:
    matches = [
    ...
    if len(matches) != 1:
        raise ValueError(f"Expected exactly one Desktop Recovery GUI context, found {len(matches)}")
    context = dict(matches[0])
    context["role"] = "desktop_recovery_gui_webui"
...
def enrich_closure(closure: dict[str, Any], parsed: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
    ...
    for edge in closure["edges"]:
        ...
        item = {
            **edge,
            ...
            "context_role": "desktop_recovery_gui_webui",
        }
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.desktop_recovery_gui_first_party_closure.v1",
        "context_id": context.get("context_id"),
        "context_role": "desktop_recovery_gui_webui",
...
def build_conditional_report(parsed: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.desktop_recovery_gui_conditional_imports.v1",
        "context_count": 1,
...
def build_package_requirements(
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.desktop_recovery_gui_package_requirements.v1",
        "context_id": context.get("context_id"),
        "context_count": 1,
        **groups,
...
def build_unresolved_report(
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.desktop_recovery_gui_unresolved_branches.v1",
        "record_count": len(records),
        "records": records,
...
def build_core_outputs(
    ...
    context_output = {
        "schema": f"{SCHEMA_PREFIX}.desktop_recovery_gui_context.v1",
    ...
    coverage = {
        "schema": f"{SCHEMA_PREFIX}.desktop_recovery_gui_coverage.v1",
        "mission_id": mission_id,
        "result": "desktop_recovery_gui_dependency_closure_complete",
        ...
        "limitations": [
            ...
            "Only the Desktop Recovery GUI protected context is covered by this mission.",
        ],
    }
    return {
        "DESKTOP_RECOVERY_GUI_CONTEXT.json": context_output,
        "DESKTOP_RECOVERY_GUI_FIRST_PARTY_CLOSURE.json": enriched,
        "DESKTOP_RECOVERY_GUI_CONDITIONAL_IMPORTS.json": conditional,
        "DESKTOP_RECOVERY_GUI_PACKAGE_REQUIREMENTS.json": requirements,
        "DESKTOP_RECOVERY_GUI_UNRESOLVED_BRANCHES.json": unresolved,
        "DESKTOP_RECOVERY_GUI_COVERAGE.json": coverage,
    }
...
def write_outputs(output_dir: Path, outputs: dict[str, Any], mission_id: str) -> None:
    ...
    coverage = outputs["DESKTOP_RECOVERY_GUI_COVERAGE.json"]
    receipt = {
        "schema": f"{SCHEMA_PREFIX}.desktop_recovery_gui_closure_receipt.v1",
        "mission_id": mission_id,
        "result": "desktop_recovery_gui_dependency_closure_complete",
        "source_outputs_verified_exact": True,
    ...
    (output_dir / "DESKTOP_RECOVERY_GUI_CLOSURE_RECEIPT.json").write_bytes(receipt_bytes)
...
def build(
    ...
    context = select_desktop_recovery_gui_context(source)
...
def validate_output(index_dir: Path) -> None:
    required = set(OUTPUT_NAMES) | {"DESKTOP_RECOVERY_GUI_CLOSURE_RECEIPT.json"}
    ...
    receipt = read_json(index_dir / "DESKTOP_RECOVERY_GUI_CLOSURE_RECEIPT.json")
    coverage = read_json(index_dir / "DESKTOP_RECOVERY_GUI_COVERAGE.json")
    closure = read_json(index_dir / "DESKTOP_RECOVERY_GUI_FIRST_PARTY_CLOSURE.json")
    context = read_json(index_dir / "DESKTOP_RECOVERY_GUI_CONTEXT.json")
    conditional = read_json(index_dir / "DESKTOP_RECOVERY_GUI_CONDITIONAL_IMPORTS.json")
    unresolved = read_json(index_dir / "DESKTOP_RECOVERY_GUI_UNRESOLVED_BRANCHES.json")
    if receipt.get("result") != "desktop_recovery_gui_dependency_closure_complete":
...
def show_context_requirements(index_dir: Path) -> None:
    context = read_json(index_dir / "DESKTOP_RECOVERY_GUI_CONTEXT.json")
    requirements = read_json(index_dir / "DESKTOP_RECOVERY_GUI_PACKAGE_REQUIREMENTS.json")
...
"""
    ...
    assert "DESKTOP_RECOVERY_GUI_RUNTIME_UNCERTAINTY.json" in OUTPUT_NAMES
    assert len(OUTPUT_NAMES) == 7
    synthetic_outputs = {name: {"name": name, "size_bytes": 0} for name in OUTPUT_NAMES}
    serialized_outputs = {name: canonical_json_bytes(synthetic_outputs[name]) for name in OUTPUT_NAMES}
    assert set(serialized_outputs) == set(OUTPUT_NAMES)
    assert len(set(OUTPUT_NAMES) | {"DESKTOP_RECOVERY_GUI_CLOSURE_RECEIPT.json"}) == 8
    runtime_payload = serialized_outputs["DESKTOP_RECOVERY_GUI_RUNTIME_UNCERTAINTY.json"]
    runtime_record = {
        "name": "DESKTOP_RECOVERY_GUI_RUNTIME_UNCERTAINTY.json",
        "size_bytes": len(runtime_payload),
        "sha256": sha256_bytes(runtime_payload),
    }
    verified_size, verified_hash = verify_recorded_size_and_hash(
        len(runtime_payload), sha256_bytes(runtime_payload), runtime_record,
        "runtime uncertainty output-manifest regression",
    )
    assert verified_size == len(runtime_payload)
    assert verified_hash == runtime_record["sha256"]
    print("V1A3C6_OUTPUT_MANIFEST_SELF_TEST_OK")
    print("V1A3C6_ZERO_BYTE_SELF_TEST_OK")



EXPECTED_CONTEXT_ID = "CTX-5A02B9D4A8E26D64"
EXPECTED_PATH_GROUP_ID = "PATHGROUP-529C568E9980095F"
EXPECTED_LAUNCHER_LINE = 54
EXPECTED_INTERPRETER_REFERENCE = "%PYTHONW%"
EXPECTED_INTERPRETER_PATH = r"Z:\FOXAI\Runtime\Desktop\python\pythonw.exe"
EXPECTED_RUNTIME_ID = "portable_desktop"
EXPECTED_SOURCE_KIND = "direct"
EXPECTED_WORKING_DIRECTORY = r"Z:\FOXAI"
EXPECTED_FLAGS = ["-s"]
EXPECTED_UNCERTAINTY = ["pythonw_runtime_identity_not_directly_probed"]
EXPECTED_UNRESOLVED_MODULE = "fcntl"
EXPECTED_UNRESOLVED_SOURCE = r"Z:\FOXAI\core\security_containment.py"
EXPECTED_UNRESOLVED_LINE = 135

WORKSHOP_BASELINE_MISSION_ID = "ENG-20260721-232230-72B494"
WORKSHOP_BASELINE_CONTEXT_ID = "CTX-68030A15EE97A526"
WORKSHOP_BASELINE_PATH_GROUP_ID = "PATHGROUP-266CA7411F12A68D"
WORKSHOP_BASELINE_RECEIPT = "WORKSHOP_MAIN_CLOSURE_RECEIPT.json"
WORKSHOP_BASELINE_OUTPUTS = (
    "WORKSHOP_MAIN_CONTEXT.json", "WORKSHOP_MAIN_FIRST_PARTY_CLOSURE.json",
    "WORKSHOP_MAIN_CONDITIONAL_IMPORTS.json", "WORKSHOP_MAIN_PACKAGE_REQUIREMENTS.json",
    "WORKSHOP_MAIN_UNRESOLVED_BRANCHES.json", "WORKSHOP_MAIN_RUNTIME_UNCERTAINTY.json",
    "WORKSHOP_MAIN_COVERAGE.json",
)
HELPER_BASELINE_MISSION_ID = "ENG-20260721-233008-E93FBD"
HELPER_BASELINE_CONTEXT_ID = "CTX-FCE6D44FA8CBAF96"
HELPER_BASELINE_PATH_GROUP_ID = "PATHGROUP-26C1A7D4F9C70C86"
HELPER_BASELINE_RECEIPT = "DESKTOP_RECOVERY_HELPER_CLOSURE_RECEIPT.json"
HELPER_BASELINE_OUTPUTS = (
    "DESKTOP_RECOVERY_HELPER_CONTEXT.json", "DESKTOP_RECOVERY_HELPER_FIRST_PARTY_CLOSURE.json",
    "DESKTOP_RECOVERY_HELPER_CONDITIONAL_IMPORTS.json", "DESKTOP_RECOVERY_HELPER_PACKAGE_REQUIREMENTS.json",
    "DESKTOP_RECOVERY_HELPER_UNRESOLVED_BRANCHES.json", "DESKTOP_RECOVERY_HELPER_COVERAGE.json",
)

_base_load_verified_sources = load_verified_sources

def load_verified_sources(v1a2r2_dir: Path, v1a3a_r1_dir: Path, v1a3b_dir: Path) -> dict[str, Any]:
    source = _base_load_verified_sources(v1a2r2_dir, v1a3a_r1_dir, v1a3b_dir)
    source["runtime_map"] = read_json(v1a3a_r1_dir / "PYTHON_RUNTIME_MAP_CORRECTED.json")
    return source


def select_desktop_recovery_gui_context(source: dict[str, Any]) -> dict[str, Any]:
    matches = [row for row in source["effective_contexts"].get("contexts", [])
               if row.get("context_id") == EXPECTED_CONTEXT_ID]
    if len(matches) != 1:
        raise ValueError(f"Expected exactly one Desktop Recovery GUI context, found {len(matches)}")
    context = dict(matches[0])
    checks = [
        (context.get("path_group_id") == EXPECTED_PATH_GROUP_ID, "path group"),
        (win_case(context.get("launcher_path")) == win_case(TARGET_LAUNCHER), "launcher"),
        (int(context.get("line") or 0) == EXPECTED_LAUNCHER_LINE, "launcher line"),
        (win_case(context.get("script_path")) == win_case(TARGET_SCRIPT), "entry script"),
        (str(context.get("interpreter_reference")) == EXPECTED_INTERPRETER_REFERENCE, "interpreter reference"),
        (str(context.get("interpreter_reference_kind")) == TARGET_INTERPRETER_KIND, "interpreter kind"),
        (win_case(context.get("resolved_interpreter_path")) == win_case(EXPECTED_INTERPRETER_PATH), "inferred interpreter path"),
        (str(context.get("runtime_id")) == EXPECTED_RUNTIME_ID, "runtime ID"),
        (context.get("runtime_identity_directly_observed") is False, "runtime direct-observation flag"),
        (context.get("pythonw_identity_directly_observed") is False, "pythonw direct-observation flag"),
        (str(context.get("runtime_portability_classification")) == "project_root_runtime", "runtime portability"),
        (str(context.get("source_kind")) == EXPECTED_SOURCE_KIND, "source kind"),
        (win_case(context.get("working_directory")) == win_case(EXPECTED_WORKING_DIRECTORY), "working directory"),
        ([str(v) for v in context.get("flags", [])] == EXPECTED_FLAGS, "flags"),
        ([str(v) for v in context.get("arguments", [])] == [], "arguments"),
        (sorted(str(v) for v in context.get("uncertainty", [])) == sorted(EXPECTED_UNCERTAINTY), "uncertainty"),
    ]
    failed = [label for ok, label in checks if not ok]
    if failed:
        raise ValueError(f"Desktop Recovery GUI context changed: {failed}")
    context["role"] = "desktop_recovery_gui_foxai"
    context["context_selected_from_verified_evidence"] = True
    context["context_execution_proven"] = False
    context["runtime_resolution_status"] = "statically_inferred_pythonw_sibling_not_directly_probed"
    return context


def load_completed_baseline(source_dir: Path, *, mission_id: str, context_id: str,
                            path_group_id: str, receipt_name: str,
                            output_names: tuple[str, ...], label: str) -> dict[str, Any]:
    receipt_path = source_dir / receipt_name
    if not receipt_path.is_file(): raise FileNotFoundError(receipt_path)
    receipt = read_json(receipt_path)
    if receipt.get("mission_id") != mission_id: raise ValueError(f"{label} mission changed")
    if receipt.get("internal_deterministic_rebuild_match") is not True: raise ValueError(f"{label} not deterministic")
    recorded = {str(row.get("name")): row for row in receipt.get("core_outputs_before_receipt", [])}
    checks=[]
    for name in output_names:
        path=source_dir/name; row=recorded.get(name)
        if row is None or not path.is_file(): raise FileNotFoundError(path)
        size=path.stat().st_size; digest=sha256_file(path)
        verify_recorded_size_and_hash(size,digest,row,f"{label} {name}")
        checks.append({"name":name,"size_bytes":size,"sha256":digest,"status":"verified"})
    context_file=next((name for name in output_names if name.endswith("_CONTEXT.json")),None)
    if context_file:
        context_output=read_json(source_dir/context_file)
        actual=(context_output.get("context") or {}).get("context_id") or context_output.get("context_id")
        if actual != context_id: raise ValueError(f"{label} context changed: {actual}")
    return {"status":"linked_completed_context_verified","mission_id":mission_id,
            "context_id":context_id,"path_group_id":path_group_id,
            "receipt_path":str(receipt_path),"receipt_sha256":sha256_file(receipt_path),
            "verified_outputs":checks,"context_built_by_this_mission":False,
            "nodes_or_edges_included":False,"cartesian_expansion_used":False}


def load_workshop_baseline(path: Path) -> dict[str, Any]:
    return load_completed_baseline(path, mission_id=WORKSHOP_BASELINE_MISSION_ID,
        context_id=WORKSHOP_BASELINE_CONTEXT_ID,path_group_id=WORKSHOP_BASELINE_PATH_GROUP_ID,
        receipt_name=WORKSHOP_BASELINE_RECEIPT,output_names=WORKSHOP_BASELINE_OUTPUTS,label="V1A-3C4-R1")


def load_helper_baseline(path: Path) -> dict[str, Any]:
    return load_completed_baseline(path, mission_id=HELPER_BASELINE_MISSION_ID,
        context_id=HELPER_BASELINE_CONTEXT_ID,path_group_id=HELPER_BASELINE_PATH_GROUP_ID,
        receipt_name=HELPER_BASELINE_RECEIPT,output_names=HELPER_BASELINE_OUTPUTS,label="V1A-3C5")


def build_runtime_uncertainty_report(mission_id: str, source: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
    portable = [row for row in source.get("runtime_map",{}).get("records",[])
                if row.get("runtime_id") == EXPECTED_RUNTIME_ID]
    if len(portable) != 1: raise ValueError(f"Expected one portable_desktop runtime record, found {len(portable)}")
    sibling = dict(portable[0])
    return {
        "schema": f"{SCHEMA_PREFIX}.desktop_recovery_gui_runtime_uncertainty.v1",
        "mission_id": mission_id,
        "context_id": context.get("context_id"), "path_group_id": context.get("path_group_id"),
        "launcher_path": context.get("launcher_path"), "launcher_line": context.get("line"),
        "working_directory": context.get("working_directory"), "flags": list(context.get("flags",[])),
        "interpreter_reference": context.get("interpreter_reference"),
        "interpreter_reference_kind": context.get("interpreter_reference_kind"),
        "statically_inferred_interpreter_path": context.get("resolved_interpreter_path"),
        "runtime_id_from_static_context": context.get("runtime_id"),
        "runtime_portability_classification": context.get("runtime_portability_classification"),
        "runtime_identity_directly_observed": False, "pythonw_identity_directly_observed": False,
        "pythonw_executed": False, "live_runtime_probe_used": False,
        "pythonw_sys_path_directly_observed": False,
        "sibling_python_runtime_record": sibling,
        "sibling_python_runtime_probe_is_not_pythonw_probe": True,
        "static_sibling_inference_preserved": True,
        "single_pythonw_runtime_identity_directly_proven": False,
        "uncertainty": list(context.get("uncertainty",[])),
        "effective_path_entries": list(context.get("effective_path_entries",[])),
        "launcher_environment": dict(context.get("launcher_environment",{})),
        "runtime_path_group_available_from_static_evidence": True,
        "path_group_used_for_static_provider_resolution": True,
        "imports_or_source_executed": False,
    }


_c6_base_build_core_outputs = build_core_outputs

def build_core_outputs(mission_id: str, source: dict[str, Any], context: dict[str, Any],
                       closure: dict[str, Any], parsed: dict[str, Any],
                       protected_checks: list[dict[str, Any]]) -> dict[str, Any]:
    outputs=_c6_base_build_core_outputs(mission_id,source,context,closure,parsed,protected_checks)
    workshop=source["linked_workshop_baseline"]; helper=source["linked_helper_baseline"]
    uncertainty=build_runtime_uncertainty_report(mission_id,source,context)
    c=outputs["DESKTOP_RECOVERY_GUI_CONTEXT.json"]
    c.update({"context_id":EXPECTED_CONTEXT_ID,"launcher_line":EXPECTED_LAUNCHER_LINE,
      "source_kind":EXPECTED_SOURCE_KIND,"interpreter_reference":EXPECTED_INTERPRETER_REFERENCE,
      "interpreter_reference_kind":TARGET_INTERPRETER_KIND,
      "resolved_interpreter_path":EXPECTED_INTERPRETER_PATH,"runtime_id":EXPECTED_RUNTIME_ID,
      "runtime_identity_directly_observed":False,"pythonw_identity_directly_observed":False,
      "runtime_resolution_status":"statically_inferred_pythonw_sibling_not_directly_probed",
      "runtime_uncertainty_output":"DESKTOP_RECOVERY_GUI_RUNTIME_UNCERTAINTY.json",
      "linked_completed_workshop_main_context":workshop,
      "linked_completed_desktop_recovery_helper_context":helper})
    closure_out=outputs["DESKTOP_RECOVERY_GUI_FIRST_PARTY_CLOSURE.json"]
    closure_out.update({"runtime_id":EXPECTED_RUNTIME_ID,
      "runtime_resolution_status":"static_path_group_with_pythonw_identity_uncertainty",
      "linked_context_nodes_or_edges_included":False})
    req=outputs["DESKTOP_RECOVERY_GUI_PACKAGE_REQUIREMENTS.json"]
    req.update({"runtime_identity_directly_proven":False,"runtime_assignment_static":True,
      "runtime_path_group_available":True,"pythonw_sys_path_directly_observed":False,
      "requirements_are_static_candidates_only":True})
    cov=outputs["DESKTOP_RECOVERY_GUI_COVERAGE.json"]
    cov.update({"result":"desktop_recovery_gui_dependency_closure_complete",
      "context_id":EXPECTED_CONTEXT_ID,"path_group_id":EXPECTED_PATH_GROUP_ID,
      "launcher_line":EXPECTED_LAUNCHER_LINE,"source_kind":EXPECTED_SOURCE_KIND,
      "interpreter_reference":EXPECTED_INTERPRETER_REFERENCE,
      "interpreter_reference_kind":TARGET_INTERPRETER_KIND,
      "resolved_interpreter_path":EXPECTED_INTERPRETER_PATH,"runtime_id":EXPECTED_RUNTIME_ID,
      "runtime_identity_directly_observed":False,"pythonw_identity_directly_observed":False,
      "live_runtime_probe_used":False,"pythonw_executed":False,
      "linked_workshop_main_mission_id":workshop["mission_id"],
      "linked_workshop_main_context_id":workshop["context_id"],
      "linked_workshop_main_receipt_sha256":workshop["receipt_sha256"],
      "linked_desktop_recovery_helper_mission_id":helper["mission_id"],
      "linked_desktop_recovery_helper_context_id":helper["context_id"],
      "linked_desktop_recovery_helper_receipt_sha256":helper["receipt_sha256"],
      "linked_contexts_built":False,"linked_context_nodes_or_edges_included":False,
      "other_protected_contexts_built":0,
      "limitations":["Static reachability does not prove an import executed or succeeded.",
        "The pythonw path is statically inferred as the sibling of an observed python.exe; pythonw identity was not directly probed.",
        "Function-local and guarded imports remain conditional or deferred static candidates.",
        "The Workshop Main and Desktop Recovery helper baselines are hash-linked without rebuilding or merging their closures.",
        "Only the Desktop Recovery GUI protected context is covered by this mission."]})
    outputs["DESKTOP_RECOVERY_GUI_RUNTIME_UNCERTAINTY.json"]=uncertainty
    return outputs
...
def write_outputs(output_dir: Path, outputs: dict[str, Any], mission_id: str) -> None:
    first={name:canonical_json_bytes(outputs[name]) for name in OUTPUT_NAMES}
    second={name:canonical_json_bytes(outputs[name]) for name in OUTPUT_NAMES}
    if first != second: raise ValueError("Internal deterministic output comparison failed")
    total=sum(len(v) for v in first.values())
    if total >= OUTPUT_CEILING_BYTES: raise ValueError(f"Output ceiling exceeded: {total}")
    if output_dir.exists(): raise FileExistsError(output_dir)
    output_dir.mkdir(parents=True)
    core=[]
    for name in OUTPUT_NAMES:
        data=first[name]; (output_dir/name).write_bytes(data)
        core.append({"name":name,"size_bytes":len(data),"sha256":sha256_bytes(data)})
    cov=outputs["DESKTOP_RECOVERY_GUI_COVERAGE.json"]
    c=outputs["DESKTOP_RECOVERY_GUI_CONTEXT.json"]
    receipt={"schema":f"{SCHEMA_PREFIX}.desktop_recovery_gui_closure_receipt.v1",
      "mission_id":mission_id,"result":"desktop_recovery_gui_dependency_closure_complete",
      "source_outputs_verified_exact":True,"internal_deterministic_rebuild_match":True,
      "compared_output_count":len(OUTPUT_NAMES),"protected_direct_context_count":1,
      "context_id":EXPECTED_CONTEXT_ID,"path_group_id":EXPECTED_PATH_GROUP_ID,
      "launcher_line":EXPECTED_LAUNCHER_LINE,"source_kind":EXPECTED_SOURCE_KIND,
      "interpreter_reference":EXPECTED_INTERPRETER_REFERENCE,
      "interpreter_reference_kind":TARGET_INTERPRETER_KIND,
      ...
      "runtime_identity_directly_observed":False,"pythonw_identity_directly_observed":False,
      "pythonw_executed":False,"live_runtime_probe_used":False,
      "source_launcher_record_count":cov.get("source_launcher_record_count"),
      "source_code_file_count":cov.get("source_code_file_count"),
      "source_files_read_count":cov.get("source_files_read_count"),
      "preliminary_reached_source_count":cov.get("preliminary_reached_source_count"),
      "preliminary_dependency_edge_count":cov.get("preliminary_dependency_edge_count"),
      "preliminary_unresolved_branch_count":cov.get("preliminary_unresolved_branch_count"),
      "maximum_reads_per_source_file":cov.get("maximum_reads_per_source_file"),
      "peak_source_bytes_in_memory":cov.get("peak_source_bytes_in_memory"),
      "source_buffer_ceiling_bytes":SOURCE_BUFFER_CEILING_BYTES,
      "known_good_candidates_all_match":cov.get("known_good_candidates_all_match"),
      "linked_workshop_main_mission_id":cov.get("linked_workshop_main_mission_id"),
      "linked_workshop_main_context_id":cov.get("linked_workshop_main_context_id"),
      "linked_workshop_main_receipt_sha256":cov.get("linked_workshop_main_receipt_sha256"),
      "linked_desktop_recovery_helper_mission_id":cov.get("linked_desktop_recovery_helper_mission_id"),
      "linked_desktop_recovery_helper_context_id":cov.get("linked_desktop_recovery_helper_context_id"),
      "linked_desktop_recovery_helper_receipt_sha256":cov.get("linked_desktop_recovery_helper_receipt_sha256"),
      "linked_contexts_built":False,"linked_context_nodes_or_edges_included":False,
      "cartesian_expansion_used":False,"other_protected_contexts_built":0,
      "imports_or_source_executed":False,"interpreter_child_processes":0,"shell_child_processes":0,
      "network_used":False,"packages_installed":False,"models_loaded":False,
      "existing_foxai_source_modified":False,"raw_source_content_persisted":False,
      "context_execution_proven":c.get("context_execution_proven"),"core_outputs_before_receipt":core}
    rb=canonical_json_bytes(receipt)
    if total+len(rb) >= OUTPUT_CEILING_BYTES: raise ValueError("Output ceiling exceeded including receipt")
    (output_dir/"DESKTOP_RECOVERY_GUI_CLOSURE_RECEIPT.json").write_bytes(rb)


def build(project_root_path: Path, project_root_win: str, v1a2r2_dir: Path, v1a3a_r1_dir: Path,
          v1a3b_dir: Path, workshop_baseline_dir: Path, helper_baseline_dir: Path,
          output_dir: Path, mission_id: str) -> None:
    source=load_verified_sources(v1a2r2_dir,v1a3a_r1_dir,v1a3b_dir)
    source["linked_workshop_baseline"]=load_workshop_baseline(workshop_baseline_dir)
    source["linked_helper_baseline"]=load_helper_baseline(helper_baseline_dir)
    context=select_desktop_recovery_gui_context(source); catalogs=provider_catalogs(source)
    closure=preliminary_static_closure(source,context,catalogs)
    if len(closure["reached_paths"])!=70 or len(closure["edges"])!=205 or len(closure["unresolved"])!=1:
        raise ValueError("Unexpected Desktop Recovery GUI preliminary topology")
    u=closure["unresolved"][0]
    if str(u.get("module"))!=EXPECTED_UNRESOLVED_MODULE or int(u.get("line") or 0)!=EXPECTED_UNRESOLVED_LINE or win_case(u.get("source_path"))!=win_case(EXPECTED_UNRESOLVED_SOURCE):
        raise ValueError(f"Unexpected unresolved branch: {u}")
    parsed=parse_reached_sources_once(project_root_path,project_root_win,source,closure,catalogs)
    protected=verify_current_known_good(project_root_path,project_root_win,source["known_good"])
    a=build_core_outputs(mission_id,source,context,closure,parsed,protected)
    b=build_core_outputs(mission_id,source,context,closure,parsed,protected)
    if {n:canonical_json_bytes(a[n]) for n in OUTPUT_NAMES}!={n:canonical_json_bytes(b[n]) for n in OUTPUT_NAMES}:
        raise ValueError("Deterministic rebuild comparison failed")
    write_outputs(output_dir,a,mission_id)
    print(json.dumps({"status":"built","output_dir":str(output_dir),"context_id":EXPECTED_CONTEXT_ID,
      "path_group_id":EXPECTED_PATH_GROUP_ID,"preliminary_reached_source_count":70,
      "preliminary_dependency_edge_count":205,"preliminary_unresolved_branch_count":1,
      "unresolved_module":"fcntl","runtime_id":EXPECTED_RUNTIME_ID,
      "pythonw_identity_directly_observed":False},sort_keys=True))


def verify_inputs(v1a2r2_dir: Path,v1a3a_r1_dir: Path,v1a3b_dir: Path,
                  workshop_baseline_dir: Path,helper_baseline_dir: Path) -> None:
    source=load_verified_sources(v1a2r2_dir,v1a3a_r1_dir,v1a3b_dir)
    c=select_desktop_recovery_gui_context(source); w=load_workshop_baseline(workshop_baseline_dir); h=load_helper_baseline(helper_baseline_dir)
    r=build_runtime_uncertainty_report("verification",source,c)
    print(json.dumps({"status":"verified","context_id":c["context_id"],"path_group_id":c["path_group_id"],
      "launcher":c["launcher_path"],"launcher_line":c["line"],"script":c["script_path"],
      "flags":c["flags"],"interpreter_reference":c["interpreter_reference"],
      "resolved_interpreter_path":c["resolved_interpreter_path"],"runtime_id":c["runtime_id"],
      "runtime_identity_directly_observed":False,"pythonw_identity_directly_observed":False,
      "uncertainty":c["uncertainty"],"effective_path_entry_count":len(c.get("effective_path_entries",[])),
      "workshop_baseline_mission_id":w["mission_id"],"helper_baseline_mission_id":h["mission_id"],
      "linked_contexts_built":False,"live_runtime_probe_used":False},sort_keys=True))


def verify_topology(v1a2r2_dir: Path,v1a3a_r1_dir: Path,v1a3b_dir: Path,
                    workshop_baseline_dir: Path,helper_baseline_dir: Path) -> None:
    source=load_verified_sources(v1a2r2_dir,v1a3a_r1_dir,v1a3b_dir)
    c=select_desktop_recovery_gui_context(source); load_workshop_baseline(workshop_baseline_dir); load_helper_baseline(helper_baseline_dir)
    closure=preliminary_static_closure(source,c,provider_catalogs(source))
    if (len(closure["reached_paths"]),len(closure["edges"]),len(closure["unresolved"]))!=(70,205,1):
        raise ValueError("Unexpected exact topology")
    u=closure["unresolved"][0]
    if str(u.get("module"))!="fcntl" or int(u.get("line") or 0)!=135 or win_case(u.get("source_path"))!=win_case(EXPECTED_UNRESOLVED_SOURCE):
        raise ValueError(f"Unexpected unresolved branch: {u}")
    print(json.dumps({"status":"verified","context_id":EXPECTED_CONTEXT_ID,"path_group_id":EXPECTED_PATH_GROUP_ID,
      "reached_source_count":70,"dependency_edge_count":205,"preliminary_unresolved_branch_count":1,
      "preliminary_unresolved":closure["unresolved"],"linked_contexts_built":False},sort_keys=True))


def validate_output(index_dir: Path) -> None:
    required=set(OUTPUT_NAMES)|{"DESKTOP_RECOVERY_GUI_CLOSURE_RECEIPT.json"}
    present={p.name for p in index_dir.iterdir() if p.is_file()}
    if present!=required: raise ValueError(f"Unexpected output set: {sorted(present)}")
    cdoc=read_json(index_dir/"DESKTOP_RECOVERY_GUI_CONTEXT.json"); c=cdoc.get("context",{})
    closure=read_json(index_dir/"DESKTOP_RECOVERY_GUI_FIRST_PARTY_CLOSURE.json")
    cond=read_json(index_dir/"DESKTOP_RECOVERY_GUI_CONDITIONAL_IMPORTS.json")
    req=read_json(index_dir/"DESKTOP_RECOVERY_GUI_PACKAGE_REQUIREMENTS.json")
    unr=read_json(index_dir/"DESKTOP_RECOVERY_GUI_UNRESOLVED_BRANCHES.json")
    ru=read_json(index_dir/"DESKTOP_RECOVERY_GUI_RUNTIME_UNCERTAINTY.json")
    cov=read_json(index_dir/"DESKTOP_RECOVERY_GUI_COVERAGE.json")
    receipt=read_json(index_dir/"DESKTOP_RECOVERY_GUI_CLOSURE_RECEIPT.json")
    if receipt.get("result")!="desktop_recovery_gui_dependency_closure_complete" or receipt.get("internal_deterministic_rebuild_match") is not True: raise ValueError("Receipt invalid")
    if c.get("context_id")!=EXPECTED_CONTEXT_ID or c.get("path_group_id")!=EXPECTED_PATH_GROUP_ID or int(c.get("line") or 0)!=54: raise ValueError("Context identity changed")
    if win_case(c.get("script_path"))!=win_case(TARGET_SCRIPT) or c.get("flags")!=["-s"]: raise ValueError("Context script or flags changed")
    if c.get("runtime_id")!=EXPECTED_RUNTIME_ID or win_case(c.get("resolved_interpreter_path"))!=win_case(EXPECTED_INTERPRETER_PATH): raise ValueError("Static runtime assignment changed")
    if c.get("runtime_identity_directly_observed") is not False or c.get("pythonw_identity_directly_observed") is not False: raise ValueError("pythonw falsely directly observed")
    if ru.get("pythonw_executed") is not False or ru.get("live_runtime_probe_used") is not False or ru.get("pythonw_sys_path_directly_observed") is not False: raise ValueError("Runtime boundary violated")
    if ru.get("static_sibling_inference_preserved") is not True or ru.get("sibling_python_runtime_probe_is_not_pythonw_probe") is not True: raise ValueError("Sibling inference distinction lost")
    if closure.get("node_count")!=70 or closure.get("edge_count")!=205: raise ValueError("Closure topology changed")
    if unr.get("record_count")!=1 or unr.get("unresolved_branches_labeled_confirmed")!=0: raise ValueError("Unresolved count changed")
    u=unr.get("records",[])[0]
    if str(u.get("module"))!="fcntl" or int(u.get("line") or 0)!=135 or win_case(u.get("source_path"))!=win_case(EXPECTED_UNRESOLVED_SOURCE): raise ValueError("Unexpected unresolved branch")
    if cond.get("conditionally_guarded_imports_labeled_unconditional")!=0: raise ValueError("Conditional import mislabeled")
    if closure.get("all_edges_have_exact_source_path_and_line") is not True: raise ValueError("Edge evidence incomplete")
    if int(cov.get("maximum_reads_per_source_file") or 0)>1 or int(cov.get("peak_source_bytes_in_memory") or 0)>SOURCE_BUFFER_CEILING_BYTES: raise ValueError("Read/memory ceiling exceeded")
    if cov.get("preliminary_reached_source_count")!=70 or cov.get("preliminary_dependency_edge_count")!=205 or cov.get("preliminary_unresolved_branch_count")!=1: raise ValueError("Preliminary topology changed")
    if cov.get("linked_contexts_built") is not False or cov.get("linked_context_nodes_or_edges_included") is not False: raise ValueError("Linked contexts merged")
    if req.get("runtime_identity_directly_proven") is not False or req.get("requirements_are_static_candidates_only") is not True: raise ValueError("Requirement certainty overstated")
    recorded={str(r.get("name")):r for r in receipt.get("core_outputs_before_receipt",[])}
    for name in OUTPUT_NAMES:
        p=index_dir/name; row=recorded.get(name)
        if row is None: raise ValueError(f"Receipt omits {name}")
        verify_recorded_size_and_hash(p.stat().st_size,sha256_file(p),row,f"Output {name}")
    total=sum(p.stat().st_size for p in index_dir.iterdir() if p.is_file())
    if total>=OUTPUT_CEILING_BYTES: raise ValueError("Output ceiling exceeded")
    print(json.dumps({"status":"valid","context_id":EXPECTED_CONTEXT_ID,"path_group_id":EXPECTED_PATH_GROUP_ID,
      "closure_node_count":70,"closure_edge_count":205,"unresolved_branch_count":1,
      "unresolved_module":"fcntl","pythonw_identity_directly_observed":False,
      "source_files_read_count":cov.get("source_files_read_count"),"peak_source_bytes_in_memory":cov.get("peak_source_bytes_in_memory"),
      "total_output_bytes":total},sort_keys=True))


def show_context_requirements(index_dir: Path) -> None:
    c=read_json(index_dir/"DESKTOP_RECOVERY_GUI_CONTEXT.json")
    req=read_json(index_dir/"DESKTOP_RECOVERY_GUI_PACKAGE_REQUIREMENTS.json")
    ru=read_json(index_dir/"DESKTOP_RECOVERY_GUI_RUNTIME_UNCERTAINTY.json")
    unr=read_json(index_dir/"DESKTOP_RECOVERY_GUI_UNRESOLVED_BRANCHES.json")
    cov=read_json(index_dir/"DESKTOP_RECOVERY_GUI_COVERAGE.json")
    print(json.dumps({"context":c.get("context"),"runtime_uncertainty":ru,"requirements":req,
      "unresolved_branches":unr.get("records"),"coverage_summary":{"closure_node_count":cov.get("closure_node_count"),
      "closure_edge_count":cov.get("closure_edge_count"),"source_files_read_count":cov.get("source_files_read_count"),
      "peak_source_bytes_in_memory":cov.get("peak_source_bytes_in_memory")}},indent=2,sort_keys=True))


_base_c6_self_test = self_test

def self_test() -> None:
    _base_c6_self_test()
    synthetic_source={"runtime_map":{"records":[{"runtime_id":"portable_desktop","executable":r"Z:\FOXAI\Runtime\Desktop\python\python.exe","portability_classification":"project_root_runtime","probe_status":"probe_succeeded"}]}}
    synthetic_context={"context_id":EXPECTED_CONTEXT_ID,"path_group_id":EXPECTED_PATH_GROUP_ID,
      "launcher_path":TARGET_LAUNCHER,"line":54,"working_directory":EXPECTED_WORKING_DIRECTORY,
      "flags":["-s"],"interpreter_reference":"%PYTHONW%","interpreter_reference_kind":TARGET_INTERPRETER_KIND,
      "resolved_interpreter_path":EXPECTED_INTERPRETER_PATH,"runtime_id":"portable_desktop",
      "runtime_portability_classification":"project_root_runtime","uncertainty":list(EXPECTED_UNCERTAINTY),
      "effective_path_entries":[],"launcher_environment":{}}
    report=build_runtime_uncertainty_report("self-test",synthetic_source,synthetic_context)
    assert report["pythonw_identity_directly_observed"] is False
    assert report["pythonw_executed"] is False
    assert report["live_runtime_probe_used"] is False
    assert report["statically_inferred_interpreter_path"]==EXPECTED_INTERPRETER_PATH
    assert len(OUTPUT_NAMES)==7 and "DESKTOP_RECOVERY_GUI_RUNTIME_UNCERTAINTY.json" in OUTPUT_NAMES
    print("V1A3C6_PYTHONW_UNCERTAINTY_SELF_TEST_OK")
    print("V1A3C6_OUTPUT_MANIFEST_SELF_TEST_OK")


def main(argv: list[str] | None=None) -> int:
    parser=argparse.ArgumentParser(description="Agent Fox V1A-3C6 Desktop Recovery GUI static dependency closure")
    sub=parser.add_subparsers(dest="command",required=True); sub.add_parser("self-test")
    for cmd in ("verify-inputs","verify-topology"):
        p=sub.add_parser(cmd); p.add_argument("--v1a2r2-dir",required=True); p.add_argument("--v1a3a-r1-dir",required=True); p.add_argument("--v1a3b-dir",required=True); p.add_argument("--workshop-baseline-dir",required=True); p.add_argument("--helper-baseline-dir",required=True)
    p=sub.add_parser("build"); p.add_argument("--project-root",default=PROJECT_ROOT_DEFAULT); p.add_argument("--project-root-win",default=PROJECT_ROOT_DEFAULT); p.add_argument("--v1a2r2-dir",required=True); p.add_argument("--v1a3a-r1-dir",required=True); p.add_argument("--v1a3b-dir",required=True); p.add_argument("--workshop-baseline-dir",required=True); p.add_argument("--helper-baseline-dir",required=True); p.add_argument("--output-dir",required=True); p.add_argument("--mission-id",required=True)
    p=sub.add_parser("validate-output"); p.add_argument("--index-dir",required=True)
    p=sub.add_parser("show-context-requirements"); p.add_argument("--index-dir",required=True)
    args=parser.parse_args(argv)
    if args.command=="self-test": self_test()
    elif args.command=="verify-inputs": verify_inputs(Path(args.v1a2r2_dir),Path(args.v1a3a_r1_dir),Path(args.v1a3b_dir),Path(args.workshop_baseline_dir),Path(args.helper_baseline_dir))
    elif args.command=="verify-topology": verify_topology(Path(args.v1a2r2_dir),Path(args.v1a3a_r1_dir),Path(args.v1a3b_dir),Path(args.workshop_baseline_dir),Path(args.helper_baseline_dir))
    elif args.command=="build": build(Path(args.project_root),args.project_root_win,Path(args.v1a2r2_dir),Path(args.v1a3a_r1_dir),Path(args.v1a3b_dir),Path(args.workshop_baseline_dir),Path(args.helper_baseline_dir),Path(args.output_dir),args.mission_id)
    elif args.command=="validate-output": validate_output(Path(args.index_dir))
    elif args.command=="show-context-requirements": show_context_requirements(Path(args.index_dir))
    return 0
