...
SCHEMA_PREFIX = "foxai.agent_fox.technical_core.v1a3c2"
...
TARGET_LAUNCHER = r"Z:\FOXAI\START_FOXAI_WEB_WITH_COMFYUI.bat"
...
OUTPUT_NAMES = (
    "WEB_WITH_COMFYUI_HELPER_CONTEXT.json",
    "WEB_WITH_COMFYUI_HELPER_FIRST_PARTY_CLOSURE.json",
    "WEB_WITH_COMFYUI_HELPER_CONDITIONAL_IMPORTS.json",
    "WEB_WITH_COMFYUI_HELPER_PACKAGE_REQUIREMENTS.json",
    "WEB_WITH_COMFYUI_HELPER_UNRESOLVED_BRANCHES.json",
    "WEB_WITH_COMFYUI_HELPER_COVERAGE.json",
)
...
def select_web_with_comfyui_helper_context(source: dict[str, Any]) -> dict[str, Any]:
    matches = [
    ...
    if len(matches) != 1:
        raise ValueError(f"Expected exactly one Web With ComfyUI Helper context, found {len(matches)}")
    context = dict(matches[0])
    context["role"] = "web_with_comfyui_helper"
...
def enrich_closure(closure: dict[str, Any], parsed: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
    ...
    for edge in closure["edges"]:
        ...
        item = {
            **edge,
            ...
            "context_role": "web_with_comfyui_helper",
        }
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.web_with_comfyui_helper_first_party_closure.v1",
        "context_id": context.get("context_id"),
        "context_role": "web_with_comfyui_helper",
...
def build_conditional_report(parsed: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.web_with_comfyui_helper_conditional_imports.v1",
        "context_count": 1,
...
def build_package_requirements(
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.web_with_comfyui_helper_package_requirements.v1",
        "context_id": context.get("context_id"),
        "context_count": 1,
        **groups,
...
def build_unresolved_report(
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.web_with_comfyui_helper_unresolved_branches.v1",
        "record_count": len(records),
        "records": records,
...
def build_core_outputs(
    ...
    context_output = {
        "schema": f"{SCHEMA_PREFIX}.web_with_comfyui_helper_context.v1",
    ...
    coverage = {
        "schema": f"{SCHEMA_PREFIX}.web_with_comfyui_helper_coverage.v1",
        "mission_id": mission_id,
        "result": "web_with_comfyui_helper_dependency_closure_complete",
        ...
        "limitations": [
            ...
            "Only the Web With ComfyUI Helper protected context is covered by this mission.",
        ],
    }
    return {
        "WEB_WITH_COMFYUI_HELPER_CONTEXT.json": context_output,
        "WEB_WITH_COMFYUI_HELPER_FIRST_PARTY_CLOSURE.json": enriched,
        "WEB_WITH_COMFYUI_HELPER_CONDITIONAL_IMPORTS.json": conditional,
        "WEB_WITH_COMFYUI_HELPER_PACKAGE_REQUIREMENTS.json": requirements,
        "WEB_WITH_COMFYUI_HELPER_UNRESOLVED_BRANCHES.json": unresolved,
        "WEB_WITH_COMFYUI_HELPER_COVERAGE.json": coverage,
    }
...
def write_outputs(output_dir: Path, outputs: dict[str, Any], mission_id: str) -> None:
    ...
    coverage = outputs["WEB_WITH_COMFYUI_HELPER_COVERAGE.json"]
    receipt = {
        "schema": f"{SCHEMA_PREFIX}.web_with_comfyui_helper_closure_receipt.v1",
        "mission_id": mission_id,
        "result": "web_with_comfyui_helper_dependency_closure_complete",
        "source_outputs_verified_exact": True,
    ...
    (output_dir / "WEB_WITH_COMFYUI_HELPER_CLOSURE_RECEIPT.json").write_bytes(receipt_bytes)
...
def build(
    ...
    context = select_web_with_comfyui_helper_context(source)
...
def validate_output(index_dir: Path) -> None:
    required = set(OUTPUT_NAMES) | {"WEB_WITH_COMFYUI_HELPER_CLOSURE_RECEIPT.json"}
    ...
    receipt = read_json(index_dir / "WEB_WITH_COMFYUI_HELPER_CLOSURE_RECEIPT.json")
    coverage = read_json(index_dir / "WEB_WITH_COMFYUI_HELPER_COVERAGE.json")
    closure = read_json(index_dir / "WEB_WITH_COMFYUI_HELPER_FIRST_PARTY_CLOSURE.json")
    context = read_json(index_dir / "WEB_WITH_COMFYUI_HELPER_CONTEXT.json")
    conditional = read_json(index_dir / "WEB_WITH_COMFYUI_HELPER_CONDITIONAL_IMPORTS.json")
    unresolved = read_json(index_dir / "WEB_WITH_COMFYUI_HELPER_UNRESOLVED_BRANCHES.json")
    if receipt.get("result") != "web_with_comfyui_helper_dependency_closure_complete":
...
def show_context_requirements(index_dir: Path) -> None:
    context = read_json(index_dir / "WEB_WITH_COMFYUI_HELPER_CONTEXT.json")
    requirements = read_json(index_dir / "WEB_WITH_COMFYUI_HELPER_PACKAGE_REQUIREMENTS.json")
...
"""
    ...
    print("V1A3C2_ZERO_BYTE_SELF_TEST_OK")



EXPECTED_CONTEXT_ID = "CTX-E47F4DAA05CBCD6B"
EXPECTED_PATH_GROUP_ID = "PATHGROUP-25300F3FBF74CEEB"
EXPECTED_ARGUMENTS = ["--root", r"Z:\FOXAI", "--source", "webui"]
EXPECTED_FLAGS = ["-I", "-B", "-S"]
EXPECTED_RUNTIME = r"Z:\FOXAI\Runtime\Desktop\python\python.exe"
LINKED_LAUNCHER = r"Z:\FOXAI\START_FOXAI_WEB_PORTABLE.bat"
LINKED_LAUNCHER_LINE = 58
LINKED_BASELINE_MISSION_ID = "ENG-20260721-220855-64D244"
LINKED_BASELINE_CONTEXT_ID = "CTX-23820E95DB51A6C5"
LINKED_BASELINE_PATH_GROUP_ID = "PATHGROUP-8017393F74F3ACD3"
...
def select_web_with_comfyui_helper_context(source: dict[str, Any]) -> dict[str, Any]:
    ...
    if context.get("path_group_id") != EXPECTED_PATH_GROUP_ID:
        raise ValueError(f"Unexpected path group ID: {context.get('path_group_id')}")
    if [str(value) for value in context.get("arguments", [])] != EXPECTED_ARGUMENTS:
        raise ValueError(f"Unexpected helper arguments: {context.get('arguments')}")
    if [str(value) for value in context.get("flags", [])] != EXPECTED_FLAGS:
        raise ValueError(f"Unexpected helper flags: {context.get('flags')}")
    if win_case(context.get("resolved_interpreter_path")) != win_case(EXPECTED_RUNTIME):
        raise ValueError(f"Unexpected helper runtime: {context.get('resolved_interpreter_path')}")
    context["role"] = "web_with_comfyui_helper"
    context["context_selected_from_verified_evidence"] = True
    context["context_execution_proven"] = False
    return context
...
def select_called_launcher_relationship(source: dict[str, Any]) -> dict[str, Any]:
    records = [
        row for row in source["launcher_index"].get("records", [])
        if win_case(row.get("path")) == win_case(TARGET_LAUNCHER)
    ]
    if len(records) != 1:
        raise ValueError(f"Expected one launcher-index record for helper launcher, found {len(records)}")
    links = [
        row for row in records[0].get("called_launchers", [])
        if win_case(row.get("resolved_path")) == win_case(LINKED_LAUNCHER)
    ]
    if len(links) != 1:
        raise ValueError(f"Expected one called Web Portable launcher link, found {len(links)}")
    link = links[0]
    if int(link.get("line") or 0) != LINKED_LAUNCHER_LINE:
        raise ValueError(f"Unexpected called-launcher line: {link.get('line')}")
    return {
        "source_launcher_path": TARGET_LAUNCHER,
        "source_line": int(link.get("line") or 0),
        "reference_kind": link.get("reference_kind"),
        "target_launcher_path": win_norm(link.get("resolved_path")),
        "target_exists_in_immutable_capture": bool(link.get("exists")),
        "target_confirmation": link.get("confirmation"),
        "target_execution_proven": bool(link.get("confirmed_active_or_executed")),
        "relationship_source": "verified_v1a2r2_static_launcher_index",
    }


def verify_linked_web_portable_baseline(source_dir: Path) -> dict[str, Any]:
    required = (
    ...
    receipt_path = source_dir / "WEB_PORTABLE_CLOSURE_RECEIPT.json"
    if not receipt_path.is_file():
        raise FileNotFoundError(receipt_path)
    receipt = read_json(receipt_path)
    if receipt.get("mission_id") != LINKED_BASELINE_MISSION_ID:
        raise ValueError(f"Unexpected linked baseline mission: {receipt.get('mission_id')}")
    if receipt.get("result") != "web_portable_dependency_closure_complete":
        raise ValueError("Linked Web Portable baseline is not complete")
    if receipt.get("internal_deterministic_rebuild_match") is not True:
        raise ValueError("Linked Web Portable baseline is not deterministic")
    recorded = {str(row.get("name")): row for row in receipt.get("core_outputs_before_receipt", [])}
    verified_outputs = []
    for name in required:
        row = recorded.get(name)
        if row is None:
            raise ValueError(f"Linked baseline receipt omits {name}")
        ...
        size, digest = verify_recorded_size_and_hash(
            ...
            sha256_file(path),
            row,
            f"Linked Web Portable baseline {name}",
        )
        verified_outputs.append({"name": name, "size_bytes": size, "sha256": digest})
    context_path = source_dir / "WEB_PORTABLE_CONTEXT.json"
    context = read_json(context_path)
    if context.get("mission_id") != LINKED_BASELINE_MISSION_ID:
        raise ValueError("Linked baseline context mission mismatch")
    linked_context = context.get("context", {})
    if linked_context.get("context_id") != LINKED_BASELINE_CONTEXT_ID:
        raise ValueError("Linked baseline context ID changed")
    if linked_context.get("path_group_id") != LINKED_BASELINE_PATH_GROUP_ID:
        raise ValueError("Linked baseline path group changed")
    return {
        "baseline_mission_id": LINKED_BASELINE_MISSION_ID,
        "baseline_context_id": LINKED_BASELINE_CONTEXT_ID,
        "baseline_path_group_id": LINKED_BASELINE_PATH_GROUP_ID,
        "baseline_launcher_path": LINKED_LAUNCHER,
        "baseline_receipt_path": str(receipt_path),
        "baseline_receipt_sha256": sha256_file(receipt_path),
        "baseline_context_sha256": sha256_file(context_path),
        "baseline_node_count": int(receipt.get("preliminary_reached_source_count") or 0),
        "baseline_edge_count": int(receipt.get("preliminary_dependency_edge_count") or 0),
        "baseline_unresolved_count": int(receipt.get("preliminary_unresolved_branch_count") or 0),
        "verified_output_count": len(verified_outputs),
        "verified_outputs": verified_outputs,
        "linked_context_closure_rebuilt": False,
        "linked_context_records_duplicated": False,
    }


def decorate_outputs(
    outputs: dict[str, Any],
    relationship: dict[str, Any],
    linked_baseline: dict[str, Any],
) -> dict[str, Any]:
    context_output = outputs["WEB_WITH_COMFYUI_HELPER_CONTEXT.json"]
    context_output["called_launcher_relationship"] = relationship
    context_output["linked_protected_context"] = linked_baseline
    context_output["linked_context_relationship_only"] = True
    context_output["linked_context_closure_rebuilt"] = False

    closure = outputs["WEB_WITH_COMFYUI_HELPER_FIRST_PARTY_CLOSURE.json"]
    closure["linked_context_relationship_only"] = True
    closure["linked_context_id"] = linked_baseline["baseline_context_id"]
    closure["linked_context_nodes_or_edges_included"] = False

    coverage = outputs["WEB_WITH_COMFYUI_HELPER_COVERAGE.json"]
    coverage["called_launcher_link_count"] = 1
    coverage["called_launcher_target"] = relationship["target_launcher_path"]
    coverage["called_launcher_source_line"] = relationship["source_line"]
    coverage["linked_web_portable_baseline_verified"] = True
    coverage["linked_web_portable_baseline_mission_id"] = linked_baseline["baseline_mission_id"]
    coverage["linked_web_portable_baseline_context_id"] = linked_baseline["baseline_context_id"]
    coverage["linked_context_closure_rebuilt"] = False
    coverage["linked_context_records_duplicated"] = False
    coverage["zero_byte_safe_size_handling_inherited"] = True
    coverage.pop("zero_byte_indexed_source_accepted", None)
    coverage["limitations"] = [
        "Static reachability does not prove an import executed or succeeded.",
        "Function-local imports remain deferred candidates without call-graph execution inference.",
        "Dynamic import references are recorded but not executed or resolved by execution.",
        "Only the direct start_comfyui_quiet.py helper context is built by this mission.",
        "The called Web Portable launcher is linked to its completed V1A-3C1-R1 evidence and is not rebuilt or duplicated.",
    ]
    return outputs
...
def write_outputs(output_dir: Path, outputs: dict[str, Any], mission_id: str) -> None:
    ...
    second = {name: canonical_json_bytes(outputs[name]) for name in OUTPUT_NAMES}
    if first != second:
        raise ValueError("Internal deterministic helper output comparison failed")
    total_pre_receipt = sum(len(data) for data in first.values())
    if total_pre_receipt >= OUTPUT_CEILING_BYTES:
        raise ValueError(f"Output ceiling exceeded before receipt: {total_pre_receipt}")
    if output_dir.exists():
    ...
    receipt = {
        ...
        "source_buffer_ceiling_bytes": SOURCE_BUFFER_CEILING_BYTES,
        "known_good_candidates_all_match": coverage.get("known_good_candidates_all_match"),
        "called_launcher_link_count": coverage.get("called_launcher_link_count"),
        "called_launcher_target": coverage.get("called_launcher_target"),
        "called_launcher_source_line": coverage.get("called_launcher_source_line"),
        "linked_web_portable_baseline_verified": coverage.get("linked_web_portable_baseline_verified"),
        "linked_web_portable_baseline_mission_id": coverage.get("linked_web_portable_baseline_mission_id"),
        "linked_web_portable_baseline_context_id": coverage.get("linked_web_portable_baseline_context_id"),
        "linked_context_closure_rebuilt": False,
        "linked_context_records_duplicated": False,
        "zero_byte_safe_size_handling_inherited": True,
        "cartesian_expansion_used": False,
        "other_protected_contexts_built": 0,
        "imports_or_source_executed": False,
        "interpreter_child_processes": 0,
...
def build(
    ...
    source = load_verified_sources(v1a2r2_dir, v1a3a_r1_dir, v1a3b_dir)
    context = select_web_with_comfyui_helper_context(source)
    relationship = select_called_launcher_relationship(source)
    linked_baseline = verify_linked_web_portable_baseline(v1a3c1_r1_dir)
    catalogs = provider_catalogs(source)
    closure = preliminary_static_closure(source, context, catalogs)
    parsed = parse_reached_sources_once(project_root_path, project_root_win, source, closure, catalogs)
    protected_checks = verify_current_known_good(project_root_path, project_root_win, source["known_good"])
    outputs_a = decorate_outputs(
        build_core_outputs(mission_id, source, context, closure, parsed, protected_checks),
        relationship,
        linked_baseline,
    )
    outputs_b = decorate_outputs(
        build_core_outputs(mission_id, source, context, closure, parsed, protected_checks),
        relationship,
        linked_baseline,
    )
    ...
    bytes_b = {name: canonical_json_bytes(outputs_b[name]) for name in OUTPUT_NAMES}
    if bytes_a != bytes_b:
        raise ValueError("Immutable helper deterministic rebuild failed")
    write_outputs(output_dir, outputs_a, mission_id)


def verify_inputs(v1a2r2_dir: Path, v1a3a_r1_dir: Path, v1a3b_dir: Path, v1a3c1_r1_dir: Path) -> None:
    source = load_verified_sources(v1a2r2_dir, v1a3a_r1_dir, v1a3b_dir)
    context = select_web_with_comfyui_helper_context(source)
    relationship = select_called_launcher_relationship(source)
    linked = verify_linked_web_portable_baseline(v1a3c1_r1_dir)
    print(json.dumps({
        "status": "verified",
        "context_id": context.get("context_id"),
        "path_group_id": context.get("path_group_id"),
        "launcher": context.get("launcher_path"),
        "script": context.get("script_path"),
        "runtime": context.get("resolved_interpreter_path"),
        "arguments": context.get("arguments"),
        "flags": context.get("flags"),
        "called_launcher_target": relationship.get("target_launcher_path"),
        "called_launcher_line": relationship.get("source_line"),
        "linked_baseline_mission_id": linked.get("baseline_mission_id"),
        "linked_baseline_context_id": linked.get("baseline_context_id"),
        "linked_context_closure_rebuilt": linked.get("linked_context_closure_rebuilt"),
        "source_launcher_record_count": source["v1a3b_coverage"].get("source_launcher_record_count"),
        "source_code_file_count": source["v1a3b_coverage"].get("source_code_file_count"),
    }, sort_keys=True))


def verify_topology(v1a2r2_dir: Path, v1a3a_r1_dir: Path, v1a3b_dir: Path, v1a3c1_r1_dir: Path) -> None:
    source = load_verified_sources(v1a2r2_dir, v1a3a_r1_dir, v1a3b_dir)
    context = select_web_with_comfyui_helper_context(source)
    relationship = select_called_launcher_relationship(source)
    linked = verify_linked_web_portable_baseline(v1a3c1_r1_dir)
    closure = preliminary_static_closure(source, context, provider_catalogs(source))
    reached_count = len(closure["reached_paths"])
    edge_count = len(closure["edges"])
    unresolved_count = len(closure["unresolved"])
    if reached_count != EXPECTED_REACHED_NODE_COUNT:
    ...
    print(json.dumps({
        ...
        "dependency_edge_count": edge_count,
        "preliminary_unresolved_branch_count": unresolved_count,
        "preliminary_unresolved": closure.get("unresolved"),
        "called_launcher_target": relationship.get("target_launcher_path"),
        "called_launcher_line": relationship.get("source_line"),
        "linked_baseline_mission_id": linked.get("baseline_mission_id"),
        "linked_baseline_node_count": linked.get("baseline_node_count"),
        "linked_baseline_edge_count": linked.get("baseline_edge_count"),
        "linked_context_closure_rebuilt": False,
    }, sort_keys=True))
...
def validate_output(index_dir: Path) -> None:
    ...
    present = {path.name for path in index_dir.iterdir() if path.is_file()}
    if present != required:
        raise ValueError(f"Unexpected helper output set: present={sorted(present)}, required={sorted(required)}")
    receipt = read_json(index_dir / "WEB_WITH_COMFYUI_HELPER_CLOSURE_RECEIPT.json")
    coverage = read_json(index_dir / "WEB_WITH_COMFYUI_HELPER_COVERAGE.json")
    closure = read_json(index_dir / "WEB_WITH_COMFYUI_HELPER_FIRST_PARTY_CLOSURE.json")
    context_output = read_json(index_dir / "WEB_WITH_COMFYUI_HELPER_CONTEXT.json")
    conditional = read_json(index_dir / "WEB_WITH_COMFYUI_HELPER_CONDITIONAL_IMPORTS.json")
    unresolved = read_json(index_dir / "WEB_WITH_COMFYUI_HELPER_UNRESOLVED_BRANCHES.json")
    context = context_output.get("context", {})
    relationship = context_output.get("called_launcher_relationship", {})
    linked = context_output.get("linked_protected_context", {})
    if receipt.get("result") != "web_with_comfyui_helper_dependency_closure_complete":
        raise ValueError("Unexpected helper receipt result")
    if receipt.get("internal_deterministic_rebuild_match") is not True:
        raise ValueError("Helper deterministic rebuild did not match")
    if receipt.get("protected_direct_context_count") != 1 or context_output.get("protected_direct_context_count") != 1:
        raise ValueError("Expected exactly one protected helper context")
    if context.get("context_id") != EXPECTED_CONTEXT_ID or context.get("path_group_id") != EXPECTED_PATH_GROUP_ID:
        raise ValueError("Helper context identity changed")
    if win_case(context.get("launcher_path")) != win_case(TARGET_LAUNCHER):
        raise ValueError("Helper launcher changed")
    if win_case(context.get("script_path")) != win_case(TARGET_SCRIPT):
        raise ValueError("Helper script changed")
    if win_case(context.get("resolved_interpreter_path")) != win_case(EXPECTED_RUNTIME):
        raise ValueError("Helper runtime changed")
    if [str(value) for value in context.get("arguments", [])] != EXPECTED_ARGUMENTS:
        raise ValueError("Helper arguments changed")
    if [str(value) for value in context.get("flags", [])] != EXPECTED_FLAGS:
        raise ValueError("Helper flags changed")
    if relationship.get("source_line") != LINKED_LAUNCHER_LINE:
        raise ValueError("Called-launcher line changed")
    if win_case(relationship.get("target_launcher_path")) != win_case(LINKED_LAUNCHER):
        raise ValueError("Called-launcher target changed")
    if linked.get("baseline_mission_id") != LINKED_BASELINE_MISSION_ID:
        raise ValueError("Linked baseline mission changed")
    if linked.get("baseline_context_id") != LINKED_BASELINE_CONTEXT_ID:
        raise ValueError("Linked baseline context changed")
    if linked.get("linked_context_closure_rebuilt") is not False:
        raise ValueError("Linked Web Portable closure was unexpectedly rebuilt")
    if linked.get("linked_context_records_duplicated") is not False:
        raise ValueError("Linked Web Portable records were unexpectedly duplicated")
    if closure.get("node_count") != EXPECTED_REACHED_NODE_COUNT:
        raise ValueError(f"Unexpected helper node count: {closure.get('node_count')}")
    if closure.get("edge_count") != EXPECTED_DEPENDENCY_EDGE_COUNT:
        raise ValueError(f"Unexpected helper edge count: {closure.get('edge_count')}")
    if coverage.get("preliminary_reached_source_count") != EXPECTED_REACHED_NODE_COUNT:
        raise ValueError("Preliminary helper reached-source count changed")
    if coverage.get("preliminary_dependency_edge_count") != EXPECTED_DEPENDENCY_EDGE_COUNT:
        raise ValueError("Preliminary helper edge count changed")
    if coverage.get("preliminary_unresolved_branch_count") != EXPECTED_PRELIMINARY_UNRESOLVED_COUNT:
        raise ValueError("Preliminary helper unresolved count changed")
    if coverage.get("called_launcher_link_count") != 1:
        raise ValueError("Expected exactly one called-launcher link")
    if coverage.get("linked_context_closure_rebuilt") is not False:
        raise ValueError("Coverage reports linked closure rebuild")
    if coverage.get("cartesian_all_imports_by_all_contexts_expansion") is not False:
        raise ValueError("Unexpected Cartesian expansion")
    if int(coverage.get("maximum_reads_per_source_file") or 0) > 1:
        raise ValueError("A reached helper source was read more than once")
    if int(coverage.get("peak_source_bytes_in_memory") or 0) > SOURCE_BUFFER_CEILING_BYTES:
        raise ValueError("Helper source buffer ceiling exceeded")
    if closure.get("all_edges_have_exact_source_path_and_line") is not True:
        raise ValueError("Helper closure edge evidence incomplete")
    if conditional.get("conditionally_guarded_imports_labeled_unconditional") != 0:
        raise ValueError("Conditional helper import mislabeled")
    if unresolved.get("unresolved_branches_labeled_confirmed") != 0:
        raise ValueError("Unresolved helper branch falsely confirmed")
    for key in (
        "imports_or_source_executed", "network_used", "packages_installed", "models_loaded",
        "existing_foxai_source_modified",
    ):
        if receipt.get(key) is not False:
            raise ValueError(f"Unsafe helper receipt flag: {key}")
    if receipt.get("interpreter_child_processes") != 0 or receipt.get("shell_child_processes") != 0:
        raise ValueError("Helper payload child process boundary violated")
    for row in receipt.get("core_outputs_before_receipt", []):
        path = index_dir / str(row.get("name"))
        if not path.is_file():
            raise FileNotFoundError(path)
        verify_recorded_size_and_hash(
            ...
            sha256_file(path),
            row,
            f"Generated helper output {path}",
        )
    total = sum(path.stat().st_size for path in index_dir.rglob("*") if path.is_file())
    if total >= OUTPUT_CEILING_BYTES:
        raise ValueError(f"Helper output ceiling exceeded: {total}")
    print(json.dumps({
        "status": "valid",
        "context_id": EXPECTED_CONTEXT_ID,
        "path_group_id": EXPECTED_PATH_GROUP_ID,
        "reached_source_count": EXPECTED_REACHED_NODE_COUNT,
        "dependency_edge_count": EXPECTED_DEPENDENCY_EDGE_COUNT,
        "preliminary_unresolved_branch_count": EXPECTED_PRELIMINARY_UNRESOLVED_COUNT,
        "called_launcher_target": LINKED_LAUNCHER,
        "linked_baseline_mission_id": LINKED_BASELINE_MISSION_ID,
        "linked_context_closure_rebuilt": False,
...
def show_context_requirements(index_dir: Path) -> None:
    ...
    print(json.dumps({
        ...
        "called_launcher_relationship": context.get("called_launcher_relationship"),
        "linked_protected_context": context.get("linked_protected_context"),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Agent Fox V1A-3C2 bounded Web With ComfyUI helper static dependency closure"
    )
    sub = parser.add_subparsers(dest="command", required=True)
    sub.add_parser("self-test")

    for command_name in ("verify-inputs", "verify-topology"):
        command = sub.add_parser(command_name)
        command.add_argument("--v1a2r2-dir", required=True)
        command.add_argument("--v1a3a-r1-dir", required=True)
        command.add_argument("--v1a3b-dir", required=True)
        command.add_argument("--v1a3c1-r1-dir", required=True)
    ...
    build_parser.add_argument("--v1a3c1-r1-dir", required=True)
    ...
    if args.command == "self-test":
        ...
        verify_inputs(
            Path(args.v1a2r2_dir), Path(args.v1a3a_r1_dir), Path(args.v1a3b_dir), Path(args.v1a3c1_r1_dir)
        )
