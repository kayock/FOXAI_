...
SCHEMA_PREFIX = "foxai.agent_fox.technical_core.v1a3c3"
...
TARGET_SCRIPT = r"Z:\FOXAI\System\PortableRuntime\manage_comfyui_normal.py"
...
def select_workshop_comfyui_manager_context(source: dict[str, Any]) -> dict[str, Any]:
    matches = [
    ...
    if len(matches) != 1:
        raise ValueError(f"Expected exactly one Workshop ComfyUI Manager context, found {len(matches)}")
    context = dict(matches[0])
    context["role"] = "workshop_comfyui_manager_webui"
...
def enrich_closure(closure: dict[str, Any], parsed: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
    ...
    for edge in closure["edges"]:
        ...
        item = {
            **edge,
            ...
            "context_role": "workshop_comfyui_manager_webui",
        }
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.workshop_comfyui_manager_first_party_closure.v1",
        "context_id": context.get("context_id"),
        "context_role": "workshop_comfyui_manager_webui",
...
def build_conditional_report(parsed: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.workshop_comfyui_manager_conditional_imports.v1",
        "context_count": 1,
...
def build_package_requirements(
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.workshop_comfyui_manager_package_requirements.v1",
        "context_id": context.get("context_id"),
        "context_count": 1,
        **groups,
...
def build_unresolved_report(
    ...
    return {
        "schema": f"{SCHEMA_PREFIX}.workshop_comfyui_manager_unresolved_branches.v1",
        "record_count": len(records),
        "records": records,
...
def build_core_outputs(
    ...
    context_output = {
        "schema": f"{SCHEMA_PREFIX}.workshop_comfyui_manager_context.v1",
    ...
    coverage = {
        "schema": f"{SCHEMA_PREFIX}.workshop_comfyui_manager_coverage.v1",
        "mission_id": mission_id,
        "result": "workshop_comfyui_manager_dependency_closure_complete",
        ...
        "limitations": [
            ...
            "Only the Workshop ComfyUI Manager protected context is covered by this mission.",
        ],
    }
    return {
        "WORKSHOP_COMFYUI_MANAGER_CONTEXT.json": context_output,
        "WORKSHOP_COMFYUI_MANAGER_FIRST_PARTY_CLOSURE.json": enriched,
        "WORKSHOP_COMFYUI_MANAGER_CONDITIONAL_IMPORTS.json": conditional,
        "WORKSHOP_COMFYUI_MANAGER_PACKAGE_REQUIREMENTS.json": requirements,
        "WORKSHOP_COMFYUI_MANAGER_UNRESOLVED_BRANCHES.json": unresolved,
        "WORKSHOP_COMFYUI_MANAGER_COVERAGE.json": coverage,
    }
...
def write_outputs(output_dir: Path, outputs: dict[str, Any], mission_id: str) -> None:
    ...
    coverage = outputs["WORKSHOP_COMFYUI_MANAGER_COVERAGE.json"]
    receipt = {
        "schema": f"{SCHEMA_PREFIX}.workshop_comfyui_manager_closure_receipt.v1",
        "mission_id": mission_id,
        "result": "workshop_comfyui_manager_dependency_closure_complete",
        "source_outputs_verified_exact": True,
    ...
    (output_dir / "WORKSHOP_COMFYUI_MANAGER_CLOSURE_RECEIPT.json").write_bytes(receipt_bytes)
...
def build(
    ...
    context = select_workshop_comfyui_manager_context(source)
...
def validate_output(index_dir: Path) -> None:
    required = set(OUTPUT_NAMES) | {"WORKSHOP_COMFYUI_MANAGER_CLOSURE_RECEIPT.json"}
    ...
    receipt = read_json(index_dir / "WORKSHOP_COMFYUI_MANAGER_CLOSURE_RECEIPT.json")
    coverage = read_json(index_dir / "WORKSHOP_COMFYUI_MANAGER_COVERAGE.json")
    closure = read_json(index_dir / "WORKSHOP_COMFYUI_MANAGER_FIRST_PARTY_CLOSURE.json")
    context = read_json(index_dir / "WORKSHOP_COMFYUI_MANAGER_CONTEXT.json")
    conditional = read_json(index_dir / "WORKSHOP_COMFYUI_MANAGER_CONDITIONAL_IMPORTS.json")
    unresolved = read_json(index_dir / "WORKSHOP_COMFYUI_MANAGER_UNRESOLVED_BRANCHES.json")
    if receipt.get("result") != "workshop_comfyui_manager_dependency_closure_complete":
...
def show_context_requirements(index_dir: Path) -> None:
    context = read_json(index_dir / "WORKSHOP_COMFYUI_MANAGER_CONTEXT.json")
    requirements = read_json(index_dir / "WORKSHOP_COMFYUI_MANAGER_PACKAGE_REQUIREMENTS.json")
...
"""
    ...
    print("V1A3C3_ZERO_BYTE_SELF_TEST_OK")



EXPECTED_CONTEXT_ID = "CTX-EEC78B5B382B239D"
EXPECTED_PATH_GROUP_ID = "PATHGROUP-25300F3FBF74CEEB"
EXPECTED_LAUNCHER_LINE = 12
EXPECTED_ARGUMENTS = ["--root", r"Z:\FOXAI", "spawn", "--source", "workshop"]
EXPECTED_FLAGS = ["-I", "-B", "-S"]
EXPECTED_RUNTIME = r"Z:\FOXAI\Runtime\Desktop\python\python.exe"
EXPECTED_UNRESOLVED_MODULE = "psutil"
EXPECTED_UNRESOLVED_LINE = 190
RELATED_CONTEXT_ID = "CTX-68030A15EE97A526"
RELATED_PATH_GROUP_ID = "PATHGROUP-266CA7411F12A68D"
RELATED_SCRIPT = r"Z:\FOXAI\foxai.py"
RELATED_LINE = 24
RELATED_SOURCE_KIND = "nested_shell"
...
def select_workshop_comfyui_manager_context(source: dict[str, Any]) -> dict[str, Any]:
    ...
    if context.get("context_id") != EXPECTED_CONTEXT_ID:
        raise ValueError(f"Unexpected manager context ID: {context.get('context_id')}")
    if context.get("path_group_id") != EXPECTED_PATH_GROUP_ID:
        raise ValueError(f"Unexpected manager path group: {context.get('path_group_id')}")
    if int(context.get("line") or 0) != EXPECTED_LAUNCHER_LINE:
        raise ValueError(f"Unexpected manager launcher line: {context.get('line')}")
    if [str(value) for value in context.get("arguments", [])] != EXPECTED_ARGUMENTS:
        raise ValueError(f"Unexpected manager arguments: {context.get('arguments')}")
    if [str(value) for value in context.get("flags", [])] != EXPECTED_FLAGS:
        raise ValueError(f"Unexpected manager flags: {context.get('flags')}")
    if win_case(context.get("resolved_interpreter_path")) != win_case(EXPECTED_RUNTIME):
        raise ValueError(f"Unexpected manager runtime: {context.get('resolved_interpreter_path')}")
    context["role"] = "workshop_comfyui_manager_helper"
    context["context_selected_from_verified_evidence"] = True
    context["context_execution_proven"] = False
    return context


def select_related_pending_context(source: dict[str, Any]) -> dict[str, Any]:
    matches = [
        row for row in source["effective_contexts"].get("contexts", [])
        if row.get("context_id") == RELATED_CONTEXT_ID
        and win_case(row.get("launcher_path")) == win_case(TARGET_LAUNCHER)
        and win_case(row.get("script_path")) == win_case(RELATED_SCRIPT)
    ]
    if len(matches) != 1:
        raise ValueError(f"Expected exactly one related pending foxai.py context, found {len(matches)}")
    row = dict(matches[0])
    if row.get("path_group_id") != RELATED_PATH_GROUP_ID:
        raise ValueError(f"Unexpected related path group: {row.get('path_group_id')}")
    if int(row.get("line") or 0) != RELATED_LINE:
        raise ValueError(f"Unexpected related launcher line: {row.get('line')}")
    if str(row.get("source_kind")) != RELATED_SOURCE_KIND:
        raise ValueError(f"Unexpected related source kind: {row.get('source_kind')}")
    return {
        "context_id": row.get("context_id"),
        "path_group_id": row.get("path_group_id"),
        "launcher_path": row.get("launcher_path"),
        "launcher_line": int(row.get("line") or 0),
        ...
        "resolved_interpreter_path": row.get("resolved_interpreter_path"),
        "source_kind": row.get("source_kind"),
        "uncertainty": row.get("uncertainty", []),
        "status": "related_pending_direct_context",
...
def build_core_outputs(
    ...
    related = select_related_pending_context(source)
    context_output = outputs["WORKSHOP_COMFYUI_MANAGER_CONTEXT.json"]
    context_output["related_pending_direct_context"] = related
    context_output["related_context_built"] = False
    context_output["related_context_nodes_or_edges_included"] = False

    closure_output = outputs["WORKSHOP_COMFYUI_MANAGER_FIRST_PARTY_CLOSURE.json"]
    closure_output["related_pending_context_id"] = related["context_id"]
    closure_output["related_context_nodes_or_edges_included"] = False

    coverage = outputs["WORKSHOP_COMFYUI_MANAGER_COVERAGE.json"]
    coverage.pop("zero_byte_indexed_source_accepted", None)
    coverage["zero_byte_safe_size_handling_inherited"] = True
    coverage["manager_launcher_line"] = EXPECTED_LAUNCHER_LINE
    coverage["manager_arguments_verified"] = True
    coverage["related_pending_context_count"] = 1
    coverage["related_pending_context_id"] = related["context_id"]
    coverage["related_pending_context_path_group_id"] = related["path_group_id"]
    coverage["related_pending_context_launcher_line"] = related["launcher_line"]
    coverage["related_context_built"] = False
    coverage["related_context_nodes_or_edges_included"] = False
    coverage["limitations"] = [
        "Static reachability does not prove an import executed or succeeded.",
        "Function-local imports remain deferred candidates without call-graph execution inference.",
        "Dynamic import references are recorded but not executed or resolved by execution.",
        "Only the direct manage_comfyui_normal.py Workshop manager context is built by this mission.",
        "The separate foxai.py context from the same launcher is retained as a pending relationship and is not merged or built.",
    ]
    return outputs
...
def write_outputs(output_dir: Path, outputs: dict[str, Any], mission_id: str) -> None:
    ...
    second = {name: canonical_json_bytes(outputs[name]) for name in OUTPUT_NAMES}
    if first != second:
        raise ValueError("Internal deterministic Workshop manager output comparison failed")
    total_pre_receipt = sum(len(data) for data in first.values())
    if total_pre_receipt >= OUTPUT_CEILING_BYTES:
        raise ValueError(f"Output ceiling exceeded before receipt: {total_pre_receipt}")
    if output_dir.exists():
    ...
    receipt = {
        ...
        "source_buffer_ceiling_bytes": SOURCE_BUFFER_CEILING_BYTES,
        "known_good_candidates_all_match": coverage.get("known_good_candidates_all_match"),
        "related_pending_context_count": 1,
        "related_pending_context_id": RELATED_CONTEXT_ID,
        "related_pending_context_path_group_id": RELATED_PATH_GROUP_ID,
        "related_pending_context_launcher_line": RELATED_LINE,
        "related_context_built": False,
        "related_context_nodes_or_edges_included": False,
...
def verify_inputs(v1a2r2_dir: Path, v1a3a_r1_dir: Path, v1a3b_dir: Path) -> None:
    ...
    print(json.dumps({
        ...
        "related_pending_context": related,
        "source_launcher_record_count": source["v1a3b_coverage"].get("source_launcher_record_count"),
        "source_code_file_count": source["v1a3b_coverage"].get("source_code_file_count"),
    }, sort_keys=True))
...
def verify_topology(v1a2r2_dir: Path, v1a3a_r1_dir: Path, v1a3b_dir: Path) -> None:
    ...
    if len(unresolved) != 1:
        raise ValueError(f"Expected one preliminary unresolved branch, found {len(unresolved)}")
    record = unresolved[0]
    if str(record.get("module")) != EXPECTED_UNRESOLVED_MODULE:
        raise ValueError(f"Unexpected unresolved module: {record.get('module')}")
    if int(record.get("line") or 0) != EXPECTED_UNRESOLVED_LINE:
        raise ValueError(f"Unexpected unresolved line: {record.get('line')}")
    print(json.dumps({
        "status": "verified",
        "context_id": context.get("context_id"),
        "path_group_id": context.get("path_group_id"),
        ...
        "dependency_edge_count": edge_count,
        "preliminary_unresolved_branch_count": unresolved_count,
        "preliminary_unresolved": unresolved,
        "related_pending_context_id": related.get("context_id"),
        "related_pending_context_path_group_id": related.get("path_group_id"),
        "related_pending_context_launcher_line": related.get("launcher_line"),
        "related_context_built": False,
    }, sort_keys=True))
...
def validate_output(index_dir: Path) -> None:
    ...
    present = {path.name for path in index_dir.iterdir() if path.is_file()}
    if present != required:
        raise ValueError(f"Unexpected Workshop manager output set: present={sorted(present)}, required={sorted(required)}")
    receipt = read_json(index_dir / "WORKSHOP_COMFYUI_MANAGER_CLOSURE_RECEIPT.json")
    coverage = read_json(index_dir / "WORKSHOP_COMFYUI_MANAGER_COVERAGE.json")
    closure = read_json(index_dir / "WORKSHOP_COMFYUI_MANAGER_FIRST_PARTY_CLOSURE.json")
    context_output = read_json(index_dir / "WORKSHOP_COMFYUI_MANAGER_CONTEXT.json")
    conditional = read_json(index_dir / "WORKSHOP_COMFYUI_MANAGER_CONDITIONAL_IMPORTS.json")
    unresolved = read_json(index_dir / "WORKSHOP_COMFYUI_MANAGER_UNRESOLVED_BRANCHES.json")
    context = context_output.get("context", {})
    related = context_output.get("related_pending_direct_context", {})
    if receipt.get("result") != "workshop_comfyui_manager_dependency_closure_complete":
        raise ValueError("Unexpected Workshop manager receipt result")
    if receipt.get("internal_deterministic_rebuild_match") is not True:
        raise ValueError("Workshop manager deterministic rebuild did not match")
    if receipt.get("protected_direct_context_count") != 1 or context_output.get("protected_direct_context_count") != 1:
        raise ValueError("Expected exactly one protected Workshop manager context")
    if context.get("context_id") != EXPECTED_CONTEXT_ID or context.get("path_group_id") != EXPECTED_PATH_GROUP_ID:
        raise ValueError("Workshop manager context identity changed")
    if int(context.get("line") or 0) != EXPECTED_LAUNCHER_LINE:
        raise ValueError("Workshop manager launcher line changed")
    if win_case(context.get("launcher_path")) != win_case(TARGET_LAUNCHER):
        raise ValueError("Workshop manager launcher changed")
    if win_case(context.get("script_path")) != win_case(TARGET_SCRIPT):
        raise ValueError("Workshop manager script changed")
    if win_case(context.get("resolved_interpreter_path")) != win_case(EXPECTED_RUNTIME):
        raise ValueError("Workshop manager runtime changed")
    if [str(value) for value in context.get("arguments", [])] != EXPECTED_ARGUMENTS:
        raise ValueError("Workshop manager arguments changed")
    if [str(value) for value in context.get("flags", [])] != EXPECTED_FLAGS:
        raise ValueError("Workshop manager flags changed")
    if related.get("context_id") != RELATED_CONTEXT_ID:
        raise ValueError("Related pending context changed")
    if related.get("path_group_id") != RELATED_PATH_GROUP_ID:
        raise ValueError("Related pending path group changed")
    if int(related.get("launcher_line") or 0) != RELATED_LINE:
        raise ValueError("Related pending launcher line changed")
    if related.get("context_built_by_this_mission") is not False:
        raise ValueError("Related foxai.py context was unexpectedly built")
    if closure.get("related_context_nodes_or_edges_included") is not False:
        raise ValueError("Related foxai.py nodes or edges were unexpectedly included")
    if closure.get("node_count") != EXPECTED_REACHED_NODE_COUNT:
        raise ValueError(f"Unexpected manager node count: {closure.get('node_count')}")
    if closure.get("edge_count") != EXPECTED_DEPENDENCY_EDGE_COUNT:
        raise ValueError(f"Unexpected manager edge count: {closure.get('edge_count')}")
    if coverage.get("preliminary_reached_source_count") != EXPECTED_REACHED_NODE_COUNT:
        raise ValueError("Preliminary manager reached-source count changed")
    if coverage.get("preliminary_dependency_edge_count") != EXPECTED_DEPENDENCY_EDGE_COUNT:
        raise ValueError("Preliminary manager edge count changed")
    if coverage.get("preliminary_unresolved_branch_count") != EXPECTED_PRELIMINARY_UNRESOLVED_COUNT:
        raise ValueError("Preliminary manager unresolved count changed")
    if coverage.get("related_context_built") is not False:
        raise ValueError("Coverage reports related context build")
    if coverage.get("related_context_nodes_or_edges_included") is not False:
        raise ValueError("Coverage reports related context merge")
    ...
    if int(coverage.get("maximum_reads_per_source_file") or 0) > 1:
        raise ValueError("A reached manager source was read more than once")
    if int(coverage.get("peak_source_bytes_in_memory") or 0) > SOURCE_BUFFER_CEILING_BYTES:
        raise ValueError("Manager source buffer ceiling exceeded")
    if closure.get("all_edges_have_exact_source_path_and_line") is not True:
        raise ValueError("Manager closure edge evidence incomplete")
    if conditional.get("conditionally_guarded_imports_labeled_unconditional") != 0:
        raise ValueError("Conditional manager import mislabeled")
    if unresolved.get("unresolved_branches_labeled_confirmed") != 0:
        raise ValueError("Unresolved manager branch falsely confirmed")
    records = unresolved.get("records", [])
    if not any(str(row.get("module")) == EXPECTED_UNRESOLVED_MODULE and int(row.get("line") or 0) == EXPECTED_UNRESOLVED_LINE for row in records):
        raise ValueError("Expected psutil unresolved branch is missing")
    for key in (
        "imports_or_source_executed", "network_used", "packages_installed", "models_loaded",
        "existing_foxai_source_modified",
    ):
        if receipt.get(key) is not False:
            raise ValueError(f"Unsafe Workshop manager receipt flag: {key}")
    if receipt.get("interpreter_child_processes") != 0 or receipt.get("shell_child_processes") != 0:
        raise ValueError("Workshop manager payload child process boundary violated")
    for row in receipt.get("core_outputs_before_receipt", []):
        path = index_dir / str(row.get("name"))
        if not path.is_file():
            raise FileNotFoundError(path)
        verify_recorded_size_and_hash(
            ...
            sha256_file(path),
            row,
            f"Generated Workshop manager output {path}",
        )
    total = sum(path.stat().st_size for path in index_dir.rglob("*") if path.is_file())
    if total >= OUTPUT_CEILING_BYTES:
        raise ValueError(f"Workshop manager output ceiling exceeded: {total}")
    print(json.dumps({
        "status": "valid",
        "context_id": EXPECTED_CONTEXT_ID,
        "path_group_id": EXPECTED_PATH_GROUP_ID,
        ...
        "unresolved_module": EXPECTED_UNRESOLVED_MODULE,
        "unresolved_line": EXPECTED_UNRESOLVED_LINE,
        "related_pending_context_id": RELATED_CONTEXT_ID,
        "related_context_built": False,
...
def show_context_requirements(index_dir: Path) -> None:
    ...
    print(json.dumps({
        ...
        "related_pending_direct_context": context.get("related_pending_direct_context"),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Agent Fox V1A-3C3 bounded Workshop ComfyUI manager static dependency closure"
    )
    ...
    if args.command == "self-test":
        ...
        verify_topology(Path(args.v1a2r2_dir), Path(args.v1a3a_r1_dir), Path(args.v1a3b_dir))
