...
def boundary_watch(root: Path) -> dict:
    ...
    if process.returncode != 0 or "Ran 5 tests" not in process.stderr:
        raise VerifyError("Boundary Watch failed.")
    return {
        "passed": True,
        "tests": 5,
...
def main() -> int:
    ...
    receipt = {
        "action": "foxai_usb_commissioning_phase1_exact_preview_verify",
