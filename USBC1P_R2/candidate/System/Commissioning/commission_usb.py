    ...
    *,
    ...
    web_required = all(
        [
            ...
            embedded_probe.get("runs") is not False,
            pth.get("portable_base_ready"),
    ...
    if not structure_only:
        if not runtime_has(embedded_probe, "psutil"):
            web_notes.append(
    ...
    desktop_runtime = (
        venv_probe if venv_probe.get("runs") else embedded_probe
    )
    ...
    if venv_config.get("portability_warning"):
        desktop_notes.append(
    ...
    runtime_candidates = [embedded_probe, venv_probe, system_probe]
    torch_local = any(
    ...
    return {
        ...
        "portability": {
            ...
            "embedded_python_has_local_site_packages_directory": (
                root / "env/python/Lib/site-packages"
            ).is_dir(),
        },
        "storage": {
