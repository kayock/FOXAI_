...
if not exist "%VERIFY%" (
  echo [FAILED] Known-good verifier is missing: %VERIFY%
  pause
  exit /b 19
)
...
echo.
if "%RC%"=="0" (
  echo [COMPLETE] Current known-good baseline verification passed.
) else (
  echo [STOPPED] Verification failed closed with exit code %RC%.
)
pause
exit /b %RC%
