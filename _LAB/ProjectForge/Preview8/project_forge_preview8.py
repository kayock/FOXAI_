...
# Portable/embedded Python may run with an isolated sys.path that omits the script folder.
...
Do not explain the result in chat after the patch is written."""
    ...
    def _model_probe(self) -> dict[str, Any]:
        ...
        # Older/custom servers sometimes exposed runtime n_ctx in /v1/models.
        # Current llama.cpp documents runtime n_ctx under GET /props instead.
        context_candidates: list[tuple[str, Any]] = []
        top_meta = payload.get("meta")
        if isinstance(top_meta, dict):
            context_candidates.append(("/v1/models:meta.n_ctx", top_meta.get("n_ctx")))
        for item in model_items:
            if str(item.get("id", "")) != self.config.model_id:
                continue
            item_meta = item.get("meta")
            if isinstance(item_meta, dict):
                context_candidates.append(("/v1/models:data[].meta.n_ctx", item_meta.get("n_ctx")))

        endpoint_root = self.config.endpoint.rstrip("/")
        if endpoint_root.lower().endswith("/v1"):
            endpoint_root = endpoint_root[:-3]
        props_url = endpoint_root.rstrip("/") + "/props"
        props_error = None
        try:
            props_request = urllib.request.Request(props_url, headers={"Accept": "application/json"})
            with urllib.request.urlopen(props_request, timeout=10) as response:
                props_payload = json.loads(response.read().decode("utf-8", errors="replace"))
            settings = props_payload.get("default_generation_settings")
            if isinstance(settings, dict):
                context_candidates.append(("/props:default_generation_settings.n_ctx", settings.get("n_ctx")))
        except (OSError, urllib.error.URLError, json.JSONDecodeError) as exc:
            props_error = str(exc)
        ...
        for source, value in context_candidates:
            try:
                ...
                continue
            if candidate > 0:
                reported_context = candidate
                context_source = source
                break

        if reported_context is None:
            detail = f" /props error: {props_error}" if props_error else ""
            raise ForgeError(
                "The exact model is online, but its runtime context could not be read from /v1/models or /props."
                + detail
            )
        if reported_context != self.config.context_limit:
            raise ForgeError(
                "The local model context does not match Project Forge Preview 8. "
                f"Expected {self.config.context_limit}; reported {reported_context} from {context_source}."
            )
        return {
            "url": models_url,
            "props_url": props_url,
