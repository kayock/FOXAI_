...
class EngineerAgent:
    ...
    def resolve_exact_inspection_path(self, raw_path):
        """Resolve one project file from an absolute or FOXAI-relative path."""
        value = (raw_path or "").strip()
        if not value:
            return None, "No file path was supplied."

        if value.startswith("\\\\") or value.startswith("//"):
            return None, "Network paths are outside this read-only file request."
    ...
    def _brief_exact_file_summary(self, path, text):
        ...
        if suffix == ".py":
            try:
                ...
                return (
                    f"Python source file with {len(lines)} line(s). "
                    f"Python could not fully parse it: {error.msg} "
            ...
            parts = [f"Python source file with {len(lines)} line(s)."]
            module_doc = ast.get_docstring(tree)
            if module_doc:
                first_sentence = module_doc.strip().splitlines()[0]
                parts.append(f"Module description: {first_sentence}")
            ...
            if path.name.casefold() == "director.py":
                parts.append(
                    "Purpose: this is FOXAI's department selector. "
                    "Normal messages remain in Agent Fox chat. "
                    "An explicit /image command selects Red Canvas, and an "
                    "explicit /engineer command selects Engineer. The direct() "
                    "function returns the selected destination and payload."
                )
            elif classes or functions:
                parts.append(
                    "The names above show the file's main public structure "
                    "without searching the rest of the project."
                )
            else:
                parts.append(
                    "The file contains executable Python statements but no "
        ...
        parts = [f"{kind} with {len(lines)} line(s)."]
        if title:
