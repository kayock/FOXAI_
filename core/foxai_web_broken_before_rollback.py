...
HTML=r"""<!doctype html><html><head><meta charset=utf-8><title>FOXAI v9</title><meta name=viewport content="width=device-width,initial-scale=1"><style>
:root{--a:#00ff66;--w:#ffd166;--t:#eaffef;--m:#9db5a2;--l:#164c2c;--b:#050806;--r:#ff6b6b;--c:#8eeaff}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 10% 10%,#00ff6622,transparent 28%),linear-gradient(135deg,#020403,#07110a 55%,#020403);color:var(--t);font-family:Segoe UI,system-ui,sans-serif}.app{display:grid;grid-template-columns:330px 1fr;min-height:100vh}aside{border-right:1px solid var(--l);padding:22px;background:#0006;position:sticky;top:0;height:100vh;overflow:auto}main{padding:24px}.logo{width:96px;height:96px;border:1px solid var(--l);border-radius:24px;display:grid;place-items:center;color:var(--a);font-size:42px;font-weight:900}h1{color:var(--a);margin:16px 0 4px}.sub,.small{color:var(--m);font-size:13px}.nav{display:block;width:100%;text-align:left;margin:7px 0;padding:11px;border:1px solid #00ff6630;border-radius:14px;background:#0003;color:var(--t);font-weight:800}.nav.active,.nav:hover{background:#00ff661f}.hero,.card,.ops{border:1px solid #00ff6630;background:#0005;border-radius:22px;padding:18px;margin-bottom:16px}.hero h2{font-size:42px;margin:0 0 8px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(270px,1fr));gap:16px}.wide{grid-column:1/-1}button,select{border:1px solid #00ff6655;background:#005522;color:#fff;border-radius:12px;padding:10px 13px;margin:5px 5px 0 0;font-weight:800}button:hover{background:#007a33}select,input,textarea{width:100%;background:#07110a;color:var(--t);border:1px solid var(--l);border-radius:14px;padding:12px}textarea{min-height:92px;resize:vertical}.status{white-space:pre-wrap;font-family:Consolas,monospace;font-size:13px;color:#d8ffe0}.page{display:none}.page.active{display:block}.pill{display:inline-block;border:1px solid #00ff6638;border-radius:999px;padding:7px 10px;margin:4px;color:var(--m)}.ok{color:var(--a)}.warn{color:var(--w)}.bad{color:var(--r)}.path{font-family:Consolas,monospace;color:var(--w);overflow-wrap:anywhere}.row{display:grid;grid-template-columns:96px 1fr;gap:8px;border-bottom:1px solid #00ff6614;padding:5px 0;font-family:Consolas,monospace;font-size:13px}.lab{color:var(--m)}.meter{height:10px;border:1px solid var(--l);border-radius:999px;overflow:hidden}.fill{height:100%;background:linear-gradient(90deg,var(--a),var(--w))}.prof{min-height:210px}.prof h3{color:var(--a)}.activeProf{border-color:#ffd16688;box-shadow:0 0 28px #ffd16616}#chatLog{height:390px;overflow:auto;background:#050a07;border:1px solid var(--l);border-radius:16px;padding:14px}.user{color:#bfefff}.fox{color:#d8ffe0}.sys{color:var(--w)}.pulse{display:inline-block;width:10px;height:10px;border-radius:50%;background:var(--a);box-shadow:0 0 14px var(--a);animation:p 1.2s infinite}@keyframes p{50%{opacity:.3}}table{width:100%;border-collapse:collapse;font-family:Consolas,monospace;font-size:13px}td,th{border-bottom:1px solid #00ff6618;padding:9px;text-align:left}.link{background:transparent;border:0;color:var(--c);padding:0}.done{text-decoration:line-through;color:var(--m)}.tl{border-left:3px solid var(--a);padding:8px 0 8px 12px;margin:8px 0;background:#00ff660a;border-radius:0 12px 12px 0}.time{color:var(--w);font-family:Consolas,monospace;font-size:12px}#toast{position:fixed;right:20px;bottom:20px;background:#0b1b10;border:1px solid var(--l);padding:12px 16px;border-radius:14px;display:none;max-width:520px}@media(max-width:900px){.app{grid-template-columns:1fr}aside{position:relative;height:auto}}
</style></head><body><div class=app><aside><div class=logo>F</div><h1>FOXAI</h1><div class=sub>Cyber Operations Console v9</div><button class="nav active" onclick="pg('dash',this)">🏠 Dashboard</button><button class=nav onclick="pg('projects',this)">🗂 Projects</button><button class=nav onclick="pg('memory',this)">🧭 Mission Memory</button><button class=nav onclick="pg('mission',this)">💬 Mission Console</button><button class=nav onclick="pg('academy',this)">🧠 Academy</button><button class=nav onclick="pg('creative',this)">🎨 Creative Studio</button><button class=nav onclick="pg('library',this)">📚 Iron Library</button><button class=nav onclick="pg('repair',this)">🛠 Repair Bay</button><button class=nav onclick="pg('logs',this)">📜 Logs</button><button class=nav onclick="pg('settings',this)">⚙ Settings</button><div class=ops><div class=row><div class=lab>Operator</div><div>Eric Fox</div></div><div class=row><div class=lab>Professor</div><div id=ap>Agent Fox</div></div><div class=row><div class=lab>Project</div><div id=apro>None</div></div><div class=row><div class=lab>Mission</div><div id=ms>READY</div></div><div class=row><div class=lab>Model</div><div id=am>None</div></div><div class=row><div class=lab>Runtime</div><div id=rt>Checking</div></div></div><div id=quick></div><div class=ops><div class=row><div class=lab>CPU</div><div id=cpu>?</div></div><div class=row><div class=lab>RAM</div><div id=ram>?</div></div><div class=meter><div id=ramm class=fill></div></div></div></aside><main>
<section id=dash class="page active"><div class=hero><h2>Welcome back, Commander.</h2><p>Mission Memory is online. FOXAI can now remember project state.</p></div><div class=grid><div class=card><h3>Resume Mission</h3><div id=resume class=status>No active mission.</div><button onclick="goMemory()">Open Mission Memory</button><button onclick="resumeMission()">Resume in Console</button></div><div class=card><h3>Projects</h3><button onclick="go('projects')">Open Projects</button></div><div class=card><h3>Mission Control</h3><button onclick="go('mission')">Open Mission</button></div><div class="card wide"><h3>Status</h3><div id=status class=status>Loading...</div></div></div></section>
...
<section id=library class=page><div class=hero><h2>Iron Library</h2><p>Browse local reference files.</p></div><div class="card wide"><h3>Library Browser</h3><div id=libpath class="small path">Library</div><button onclick="libUp()">Up</button><button onclick="loadLib('')">Root</button><button onclick="api('/api/open/library')">Explorer</button><div id=liblist class=status>Loading...</div></div></section>
<section id=repair class=page><div class=hero><h2>Repair Bay</h2><p>Diagnostics first. Automated repair later.</p></div><div class=grid><div class=card><h3>Diagnostics</h3><button onclick="refresh()">Refresh Status</button></div><div class=card><h3>Config</h3><button onclick="api('/api/open/config')">Open Config</button></div></div></section>
...
function go(id){const map={dash:0,projects:1,memory:2,mission:3,academy:4,creative:5,library:6,repair:7,logs:8,settings:9};pg(id,document.querySelectorAll('.nav')[map[id]])}function goMemory(){go('memory')}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path=='/api/capabilities/launch': self.js(mission_bus.dispatch('capabilities.launch', {'key': d.get('key','')})); return
